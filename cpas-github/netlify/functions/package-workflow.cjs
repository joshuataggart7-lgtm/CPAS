  ...
  try {
    ...
    if (method === "GET" && path === "/list") {
      ...
      if (qs.role === "BC") {
        ...
        filters = `status=eq.${qs.status}&order=submitted_at.desc`;
      } else {
    ...
    if (method === "POST" && path.startsWith("/approve/")) {
      ...
      await dbUpdate("cpas_packages", id, {
        ...
        assigned_to_name: assignedToName || "", assigned_to_email: cleanEmail, assigned_at: now,
      });
      await dbInsert("cpas_assignment_log", {
        package_id: id, action: "APPROVED_AND_ASSIGNED", action_by: approvedBy || "",
        notes: `Assigned to ${assignedToName || cleanEmail}`,
      });

      if (cleanEmail) {
        await sendEmail(cleanEmail, `[CPAS] Acquisition Assigned to You — ${r.req_title}`,
    ...
    if (method === "POST" && path.startsWith("/reassign/")) {
      ...
      await dbInsert("cpas_assignment_log", {
        ...
        notes: `Reassigned from ${prevCO} to ${assignedToName || cleanEmail}`,
      });
