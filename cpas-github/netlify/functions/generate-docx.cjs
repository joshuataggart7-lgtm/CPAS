...
function parseContent(text) {
  ...
  for (const line of text.split("\n")) {
    ...
    // Divider lines — render as a horizontal rule paragraph
    if (/^[=─]{10,}$/.test(line.trim()) || /^={10,}$/.test(line.trim())) {
...
exports.handler = async (event) => {
  ...
  try {
    const { text, docType, intake, filename, title: directTitle, contractNumber, value: directValue, naics: directNaics, contractType: directContractType } = JSON.parse(event.body);
    const center = intake?.center || "NASA";
    const title = directTitle || intake?.reqTitle || "Requirement";
    const value = directValue ? "$"+Number(directValue).toLocaleString() : (intake?.value ? "$"+Number(intake.value).toLocaleString() : "TBD");
    const naics = directNaics || intake?.naics || "TBD";
    const contractType = directContractType || intake?.contractType || "FFP";
      ...
      sections:[{
        ...
        children:[
          ...
          new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:0,after:240},
            children:[new TextRun({text:"Estimated Value: "+value+" | NAICS: "+naics+" | Contract Type: "+contractType,font:"Arial",size:20,color:"666666"})]}),
