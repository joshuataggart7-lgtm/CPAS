...
const ADJECTIVAL_RATINGS = [
  { id: "EXCEPTIONAL",    label: "Exceptional",     color: "#0f6e56", bg: "#e8f7f0",
  ...
  { id: "VERY_GOOD",      label: "Very Good",       color: "#1a3a6e", bg: "#f5f7fa",
...
const CONFIDENCE_RATINGS = [
  { id: "HIGH",     label: "High Confidence",    color: "#0f6e56" },
  { id: "MODERATE", label: "Moderate Confidence", color: "#854f0b" },
  { id: "LOW",      label: "Low Confidence",      color: "#c05a00" },
  { id: "UNKNOWN",  label: "Unknown Risk",        color: "#6b7a99" },
];

const LPTA_RATINGS = [
  { id: "ACCEPTABLE",   label: "Acceptable",   color: "#0f6e56", bg: "#e8f7f0" },
  { id: "UNACCEPTABLE", label: "Unacceptable", color: "#a32d2d", bg: "#fff0f0" },
];
        ...
        <div key={item.id} style={{ background: C.bg3, border: `1px solid ${color}22`, borderRadius: 4, padding: "8px 10px", marginBottom: 6 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            ...
            <button onClick={() => onRemove(item.id)}
              style={{ background: "none", border: "none", color: "#6b7a99", cursor: "pointer", fontSize: 13 }}>×</button>
          </div>
    ...
    <div style={{ fontFamily: "FONT_COMP", color: C.text, background: C.bg }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "220px 1fr 360px", height: "calc(100vh - 140px)", overflow: "hidden" }}>
        ...
        <div style={{ borderRight: `1px solid ${C.border}`, overflow: "auto", padding: 12 }}>
          ...
          {factors.map((f, i) => {
            ...
            return (
              <div key={f.id} onClick={() => setActiveFactorId(f.id)}
                style={{ background: activeFactorId === f.id ? "#eef3fc" : C.bg3,
                         border: `1px solid ${activeFactorId === f.id ? C.blue : hasIssues ? "#854f0b" : C.border}`,
          ...
          <div style={{ background: C.bg3, border: `1px solid ${meta.overallRating ? C.blue : "#854f0b"}`, borderRadius: 4, padding: "8px 10px", marginBottom: 12 }}>
          ...
          {factorIssues.length > 0 && (
            <div style={{ background: "#fff8e6", border: "1px solid #4a4a14", borderRadius: 4, padding: "8px 10px" }}>
              <div style={{ fontSize: 9, color: C.yellow, marginBottom: 4 }}>⚠ INCOMPLETE ({factorIssues.length})</div>
              {factorIssues.map((issue, i) => (
                <div key={i} style={{ fontSize: 8.5, color: "#854f0b", marginBottom: 2 }}>• {issue}</div>
              ))}
            </div>
          )}
        </div>
        ...
        <div style={{ overflow: "auto", padding: 16 }}>
          {activeFactor ? (
            <>
                  ...
                  {ratingScale.map(r => (
                    <button key={r.id} onClick={() => updateFactor(activeFactor.id, "rating", r.id)}
                      style={{ padding: "6px 12px", borderRadius: 4, cursor: "pointer", fontSize: 10, fontWeight: "bold",
                               background: activeFactor.rating === r.id ? r.bg || "#eef3fc" : C.bg3,
              ...
              {/* Deficiencies */}
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: "#e05555", fontWeight: "bold", marginBottom: 6 }}>
              ...
              {factors.length > 1 && (
                <button onClick={() => { setFactors(f => f.filter(x => x.id !== activeFactor.id)); setActiveFactorId(factors[0]?.id); }}
                  style={{ background: "#fff0f0", border: "1px solid #4a1a1a", color: "#e05555", padding: "5px 14px",
        ...
        <div style={{ borderLeft: `1px solid ${C.border}`, overflow: "auto", padding: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10 }}>
            <div onClick={() => setMeta_("hasConflict", !meta.hasConflict)}
              style={{ width: 13, height: 13, border: `1px solid ${meta.hasConflict ? C.red : "#8896b0"}`, borderRadius: 2,
          ...
          <div style={{ background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 4, padding: "8px 10px", marginTop: 12, marginBottom: 10 }}>
            ...
            {factors.map(f => (
              <div key={f.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                ...
                {f.rating ? <RatingBadge rating={f.rating} scale={meta.isLPTA ? "LPTA" : "ADJECTIVAL"} /> : <span style={{ fontSize: 8, color: "#854f0b" }}>—</span>}
              </div>
            ))}
            {meta.pastPerfRating && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            ...
            <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 6, paddingTop: 6, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {meta.overallRating ? <RatingBadge rating={meta.overallRating} scale={meta.isLPTA ? "LPTA" : "ADJECTIVAL"} size="normal" /> : <span style={{ fontSize: 9, color: "#854f0b" }}>Not set</span>}
            </div>
          </div>
          ...
          <button onClick={() => onGenerated && onGenerated(text, meta.offerorName)}
            style={{ width: "100%", background: factorIssues.length === 0 ? "#e8f7f0" : C.bg3,
