    ...
    <div style={{ fontFamily: "FONT_COMP", color: C.text }}>
      <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", minHeight: 520 }}>
        <div style={{ padding: 16, borderRight: `1px solid ${C.border}`, overflow: "auto", maxHeight: "72vh" }}>
          ...
          {savings > 0 && (
            <div style={{ background: "#e8f7f0", border: "1px solid #1a6a3a", borderRadius: 3, padding: "8px 10px", marginTop: 6 }}>
          ...
          {PRICE_ANALYSIS_METHODS.map(m => (
            <div key={m.id} onClick={() => set("selectedMethods", toggle(p.selectedMethods, m.id))}
              style={{ display:"flex", gap:8, alignItems:"flex-start", padding:"4px 6px", cursor:"pointer",
                       background: p.selectedMethods.includes(m.id) ? "#e8f7f0" : "transparent",
                       border: `1px solid ${p.selectedMethods.includes(m.id) ? "#9fe1cb" : "transparent"}`,
                       borderRadius:3, marginBottom:3 }}>
              <div style={{ width:12, height:12, border:`1px solid ${p.selectedMethods.includes(m.id)?C.green:"#8896b0"}`,
            ...
            {COST_ANALYSIS_METHODS.map(m => (
              <div key={m.id} onClick={() => set("costAnalysisMethods", toggle(p.costAnalysisMethods, m.id))}
                style={{ display:"flex", gap:8, alignItems:"center", padding:"4px 6px", cursor:"pointer", marginBottom:3,
                         background: p.costAnalysisMethods.includes(m.id) ? "#e8f7f0" : "transparent",
                         border:`1px solid ${p.costAnalysisMethods.includes(m.id)?"#9fe1cb":"transparent"}`, borderRadius:3 }}>
                <div style={{ width:12,height:12,border:`1px solid ${p.costAnalysisMethods.includes(m.id)?C.green:"#8896b0"}`,
  ...
  return (
    <div style={{ fontFamily:"FONT_COMP", color:C.text }}>
      ...
      <div style={{ padding:16, overflow:"auto", maxHeight:"60vh" }}>
        ...
        {step === 0 && (
          <div>
            <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:12 }}>CONTRACT INFORMATION</div>
            ...
            <button onClick={()=>setStep(1)} style={{ width:"100%", marginTop:12, background:"#eef3fc", border:`1px solid ${C.blue}`, color:C.blue, padding:"8px", borderRadius:3, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>NEXT →</button>
          </div>
        )}

        {step === 1 && (
          <div>
            <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:12 }}>OPTION DETAILS</div>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:8 }}>
              <div onClick={()=>set("requirementsChanged",!opt.requirementsChanged)}
                style={{ width:13,height:13,border:`1px solid ${opt.requirementsChanged?C.yellow:"#8896b0"}`,borderRadius:2,
        ...
        {step === 2 && (
          <div>
            <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:12 }}>PRICE AND FUNDING</div>
            ...
            <div style={{ display:"flex", gap:8, marginTop:12 }}>
              ...
              <button onClick={()=>setStep(3)} style={{ flex:2, background:"#eef3fc", border:`1px solid ${C.blue}`, color:C.blue, padding:"7px", borderRadius:3, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>NEXT →</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:12 }}>D&F — GOVERNMENT INTEREST DETERMINATION (FAR 17.207)</div>
            <div style={{ background:"#eef1f6", border:"1px solid #1a3a6e", borderRadius:4, padding:"10px 12px", marginBottom:10, fontSize:10, color:C.dim }}>
            ...
            <div style={{ display:"flex", gap:8, marginTop:12 }}>
              ...
              <button onClick={()=>setStep(4)} style={{ flex:2, background:"#eef3fc", border:`1px solid ${C.blue}`, color:C.blue, padding:"7px", borderRadius:3, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>PREVIEW →</button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div>
            <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:8 }}>OPTION EXERCISE DOCUMENT</div>
            <div style={{ display:"flex", gap:8, marginBottom:10 }}>
              ...
              <button onClick={() => onGenerated && onGenerated(optText, "OPTION_EXERCISE_OY" + opt.optionYear)}
                style={{ background:"#e8f7f0", border:"1px solid #1a6a3a", color:C.green, padding:"6px 14px", borderRadius:3, cursor:"pointer", fontSize:11 }}>SAVE TO PACKAGE</button>
...
export function FPDSChecklist({ intake, onComplete }) {
  ...
  return (
    <div style={{ fontFamily:"FONT_COMP", color:C.text, padding:16 }}>
      <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:8 }}>FPDS-NG REQUIRED FIELDS — FAR 4.603</div>
      ...
      <div style={{ background:C.bg3, border:`1px solid ${C.border}`, borderRadius:4, padding:"10px 14px", marginBottom:12 }}>
        ...
        <div style={{ height:5, background:"#ffffff", borderRadius:3 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, maxHeight:"56vh", overflow:"auto" }}>
        {FPDS_FIELDS.map(f => (
          <div key={f.id} style={{ background:C.bg3, border:`1px solid ${f.required && !values[f.id] ? "#854f0b" : C.border}`, borderRadius:4, padding:"8px 10px" }}>
            <div style={{ fontSize:9, color: f.required ? C.yellow : C.muted, marginBottom:2 }}>
              {f.required ? "* " : ""}{f.label}
            </div>
            {f.hint && <div style={{ fontSize:8, color:"#6b7a99", marginBottom:3 }}>{f.hint}</div>}
            <input value={values[f.id]} onChange={e=>set(f.id,e.target.value)}
              style={{ ...inp, padding:"4px 7px", fontSize:10,
                border:`1px solid ${f.required && !values[f.id] ? "#854f0b" : "#1a3a6e"}` }} />
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginTop:12 }}>
        ...
        <button onClick={() => onComplete && onComplete(values)}
          style={{ background: allRequired ? "#e8f7f0" : C.bg3, border:`1px solid ${allRequired ? C.green : C.border}`,
