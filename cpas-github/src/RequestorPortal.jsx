...
export default function RequestorPortal({ onSubmit, onBack }) {
  ...
  const igceTotal = useMemo(() => {
    ...
    return labor + odc;
  }, [pkg.laborCats, pkg.odcTravel, pkg.odcMaterials, pkg.odcOther]);
  ...
  function handleSubmit() {
    ...
    setTimeout(() => {
      setSubmitting(false);
      setSubmitted(true);
      onSubmit && onSubmit({ ...pkg, sowText, igceTotal, submittedAt: new Date().toISOString() });
    }, 1200);
  }
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "28px 24px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <QCard q="How long do you need the contractor?" sub="Base period — we'll add options later if needed">
                <select style={inp} value={pkg.popMonths} onChange={e => set("popMonths", e.target.value)}>
                  {[["3","3 months"],["6","6 months"],["12","1 year"],["18","18 months"],["24","2 years"],["36","3 years"]].map(([v,l]) => <option key={v} value={v}>{l}</option>)}
                </select>
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ background: "#0a2a1a", border: "1px solid #1a6a3a", borderRadius: 6, padding: "10px 14px", marginBottom: 20, fontSize: 11, color: "#4aba6a" }}>
              💡 <strong>Tip:</strong> GSA rates, prior contract prices, and vendor quotes are all valid bases for your estimates. Document where your rates come from.
            </div>
            ...
            <div style={{ fontSize: 11, color: C.muted, marginBottom: 12 }}>
              Estimate the types of people needed and how many hours per year. Your CO will help refine this.
            </div>
            ...
            <div style={{ background: C.bg3, border: `2px solid ${pkg.nf1707Uploaded ? "#1a6a3a" : C.border}`, borderRadius: 8, padding: 20, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 3 }}>Required for all procurement requests. Must be signed by an authorized official.</div>
                </div>
                {pkg.nf1707Uploaded ? (
        ...
        {step === 6 && (
          <div>
            ...
            {/* Summary cards */}
            {[
              { label: "Requestor", items: [`${pkg.reqName} · ${pkg.reqTitle || ""}`, `${pkg.reqEmail}`, `${pkg.reqCenter} · ${pkg.reqOrg}`] },
              ...
              { label: "IGCE", items: [`Base year estimate: $${igceTotal.toLocaleString()}`, `Labor categories: ${pkg.laborCats.filter(l=>l.title).length}`, `Confidence: ${pkg.valueConfidence}`] },
            ...
            <div style={{ background: "#0a2a1a", border: "1px solid #1a6a3a", borderRadius: 6, padding: 14, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 11, color: C.dim }}>
                This package will be submitted to{" "}
...
function buildSOW(pkg) {
  ...
  const co     = pkg.reqName   || "[REQUESTOR]";
