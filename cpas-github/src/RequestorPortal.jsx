...
// Center → procurement office routing
const CENTER_BC_EMAILS = {
  "Ames (ARC)":          "cpas.branchchief@proton.me",
  "Armstrong (AFRC)":    "procurement@afrc.nasa.gov",
  "Glenn (GRC)":         "procurement@grc.nasa.gov",
  "Goddard (GSFC)":      "procurement@gsfc.nasa.gov",
  "Johnson (JSC)":       "procurement@jsc.nasa.gov",
  "Kennedy (KSC)":       "procurement@ksc.nasa.gov",
  "Langley (LaRC)":      "procurement@larc.nasa.gov",
  "Marshall (MSFC)":     "procurement@msfc.nasa.gov",
  "Stennis (SSC)":       "procurement@ssc.nasa.gov",
  "JPL":                 "procurement@jpl.nasa.gov",
  "HQ":                  "procurement@hq.nasa.gov",
};
        ...
        <React.Fragment key={i}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 64 }}>
            <div style={{
              ...
              background: i < current ? C.green : i === current ? C.blue : "#ffffff",
            ...
            <div style={{ fontSize: 9, color: i === current ? C.blue : i < current ? C.green : C.dim,
              marginTop: 4, textAlign: "center", maxWidth: 64, lineHeight: 1.2 }}>
              {s}
            </div>
          </div>
          {i < steps.length - 1 && (
            <div style={{ flex: 1, height: 2, background: i < current ? C.green : C.border,
              marginTop: 14, marginLeft: 2, marginRight: 2 }} />
          )}
        </React.Fragment>
      ))}
    </div>
...
export default function RequestorPortal({ onSubmit, onBack }) {
  ...
  function handleSubmit() {
    ...
    .finally(() => {
      // Log FAC-COR data to Supabase if COR info provided — captured for data calls and workforce tracking
      if (pkg.isCOR && pkg.reqEmail) {
        const SB_URL = "https://ylzdfcyiyznazvvbqdam.supabase.co";
        const SB_KEY = "sb_publishable_adMOxPm4Sd5fcUXRf9qKdw_VpwR382c";
        fetch(`${SB_URL}/rest/v1/cpas_cor_registry`, {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            organization: pkg.reqOrg || "",
            phone: pkg.reqPhone || "",
            fac_cor_level: pkg.corTraining || "",
            last_submission: new Date().toISOString(),
            is_designated_cor: pkg.isCOR || false,
          }),
        }).catch(() => {}); // silent fail
      }
  ...
  return (
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", background: C.bg, color: C.text, minHeight: "100vh" }}>
      {/* Header */}
      <div style={{ background: "#ffffff", borderBottom: `1px solid ${C.border}`, padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "28px 24px" }}>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                ...
                <select style={inp} value={pkg.reqCenter} onChange={e => {
                  const center = e.target.value;
                  set("reqCenter", center);
                  // Always update routing email when center changes
                  if (CENTER_BC_EMAILS[center]) {
                    setPkg(prev => ({ ...prev, reqCenter: center, branchChiefEmail: CENTER_BC_EMAILS[center] }));
                  }
                }}>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                <Lbl t="Procurement Office Routing Email" />
                <input style={{ ...inp, background: pkg.reqCenter ? "#f0f5ff" : "#fff" }}
                  value={pkg.branchChiefEmail}
                  onChange={e => set("branchChiefEmail", e.target.value)}
                  placeholder="Select your center above — routing email will auto-fill" />
                {pkg.reqCenter && pkg.branchChiefEmail && (
                  <div style={{ fontSize: 10, color: C.green, marginTop: 3 }}>
                    ✓ Your package will be routed to the {pkg.reqCenter} procurement office
                  </div>
                )}
              </div>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                {[["SERVICES","Professional Services","Engineering, IT support, operations, analysis"],
                  ...
                  <div key={val} onClick={() => set("reqType", val)}
                    style={{ background: pkg.reqType === val ? "#eef3fc" : C.bg3,
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  ...
                  {[["ROUGH","Rough estimate"],["MODERATE","Reasonably sure"],["CONFIDENT","Have IGCE"]].map(([val, lbl]) => (
                    <button key={val} onClick={() => set("valueConfidence", val)}
                      style={{ flex: 1, padding: "5px 4px", borderRadius: 3, cursor: "pointer", fontSize: 9,
                               background: pkg.valueConfidence === val ? "#eef3fc" : C.bg3,
              ...
              <QCard q="How long do you need the contractor?" sub="Base period — we'll add options later if needed">
                <select style={inp} value={pkg.popMonths} onChange={e => set("popMonths", e.target.value)}>
                  ...
                  <option value="60b">5 year base, no options</option>
                ...
                {[["NORMAL","Normal — no deadline pressure"],["MODERATE","Moderate — needed within 6 months"],["URGENT","Urgent — needed ASAP"]].map(([val, lbl]) => (
                  <button key={val} onClick={() => set("urgency", val)}
                    style={{ flex: 1, padding: "8px", borderRadius: 4, cursor: "pointer", fontSize: 11,
                             background: pkg.urgency === val ? (val === "URGENT" ? "#fff0f0" : "#eef3fc") : C.bg3,
                ...
                {[["YES","Yes"],["NO","No"],["UNKNOWN","Not sure"]].map(([val, lbl]) => (
                  <button key={val} onClick={() => set("priorContract", val)}
                    style={{ flex: 1, padding: "7px", borderRadius: 4, cursor: "pointer", fontSize: 11,
                             background: pkg.priorContract === val ? "#eef3fc" : C.bg3,
            ...
            {pkg.tasks.map((task, i) => (
              <div key={task.id} style={{ background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 6, padding: 14, marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  ...
                  {pkg.tasks.length > 1 && (
                    <button onClick={() => removeTask(task.id)} style={{ background: "none", border: "none", color: "#fff0f0", cursor: "pointer", fontSize: 14 }}>×</button>
                  )}
                </div>
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ background: "#e8f7f0", border: "1px solid #1a6a3a", borderRadius: 6, padding: "10px 14px", marginBottom: 20, fontSize: 11, color: "#0f6e56" }}>
            ...
            <div style={{ fontSize: 11, color: C.muted, marginBottom: 12 }}>
              <div style={{display:"flex",gap:12,marginBottom:12,alignItems:"center"}}>
                <span style={{fontSize:11,color:"#6b7a99"}}>IGCE Mode:</span>
                <button onClick={()=>set("igceMode","LABOR")}
                  style={{padding:"3px 10px",fontSize:10,borderRadius:3,cursor:"pointer",
                    background:pkg.igceMode!=="CATALOG"?"#4a7abf":"none",
                    border:"1px solid #2a4a7f",color:pkg.igceMode!=="CATALOG"?"#fff":"#6b7a99"}}>
                  Labor Hours
                </button>
              ...
              {pkg.igceMode === "CATALOG" ? (
                <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 70px 100px 70px 90px 28px",gap:6,padding:"6px 8px",background:"#f5f7fa",borderRadius:"4px 4px 0 0",border:`1px solid ${C.border}`}}>
                    ...
                    return (
                      <div key={cl.id} style={{display:"grid",gridTemplateColumns:"1fr 70px 100px 70px 90px 28px",gap:6,padding:"5px 8px",background:i%2===0?C.bg3:C.bg2,border:`1px solid ${C.border}`,borderTop:"none"}}>
                        ...
                        <button onClick={()=>setPkg(p=>({...p,catalogClins:p.catalogClins.filter(c=>c.id!==cl.id)}))}
                          style={{background:"none",border:"none",color:"#fff0f0",cursor:"pointer",fontSize:16}}>×</button>
                      </div>
                    );
                  })}
                ...
                <span style={{fontSize:11,color:"#6b7a99"}}>Estimate the types of people needed and how many hours per year. Your CO will help refine this.</span>
              )}
            </div>
              ...
              <div key={lc.id} style={{ background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 6, padding: 12, marginBottom: 8 }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 28px", gap: 8, alignItems: "end" }}>
                  ...
                  <button onClick={() => setPkg(p => ({ ...p, laborCats: p.laborCats.filter(l => l.id !== lc.id) }))}
                    style={{ background: "none", border: "none", color: "#fff0f0", cursor: "pointer", fontSize: 16, marginTop: 20 }}>×</button>
                </div>
                ...
                {[["YES","Yes — commercially available"],["PARTIAL","Partially — some commercial elements"],["NO","No — Government-unique requirement"],["UNKNOWN","Not sure"]].map(([val,lbl]) => (
                  <button key={val} onClick={() => set("commercialAvail", val)}
                    style={{ flex: 1, padding: "7px 4px", borderRadius: 4, cursor: "pointer", fontSize: 10,
                             background: pkg.commercialAvail === val ? "#eef3fc" : C.bg3,
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[["YES","Yes — small businesses can do this"],["MAYBE","Maybe — depends on requirements"],["NO","No — large business likely needed"],["UNKNOWN","Not sure"]].map(([val,lbl]) => (
                  <button key={val} onClick={() => set("smallBizPotential", val)}
                    style={{ flex: 1, padding: "7px 4px", borderRadius: 4, cursor: "pointer", fontSize: 10,
                             background: pkg.smallBizPotential === val ? "#eef3fc" : C.bg3,
        ...
        {step === 5 && (
          <div>
            ...
            {/* NF-1707 */}
            <div style={{ background: C.bg3, border: `2px solid ${pkg.nf1707Uploaded ? "#e8f7f0" : C.border}`, borderRadius: 8, padding: 20, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                ...
                {pkg.nf1707Uploaded ? (
                  <span style={{ background: "#e8f7f0", border: "1px solid #1a6a3a", color: C.green, fontSize: 10, padding: "3px 10px", borderRadius: 10 }}>✓ UPLOADED</span>
                ) : (
                  <span style={{ background: "#fff8e6", border: "1px solid #4a4a1a", color: C.yellow, fontSize: 10, padding: "3px 10px", borderRadius: 10 }}>REQUIRED</span>
                )}
              </div>
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <label style={{ background: "#eef3fc", border: `1px solid ${C.blue}`, color: C.blue,
        ...
        {step === 6 && (
          <div>
            ...
            {/* Routing info */}
            <div style={{ background: "#e8f7f0", border: "1px solid #1a6a3a", borderRadius: 6, padding: 14, marginBottom: 16 }}>
            ...
            {!pkg.nf1707Uploaded && (
              <div style={{ background: "#fff8e6", border: "1px solid #4a4a1a", borderRadius: 6, padding: 12, marginBottom: 16 }}>
            ...
            <button onClick={handleSubmit} disabled={submitting}
              style={{ width: "100%", background: submitting ? "#f5f7fa" : C.green,
                       border: "none", color: submitting ? C.muted : "#ffffff",
                       padding: "14px", borderRadius: 8, cursor: submitting ? "default" : "pointer",
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 28 }}>
          {step > 0 && (
            <button onClick={() => setStep(s => s - 1)}
              style={{ flex: 1, background: C.bg2, border: `1px solid ${C.border}`, color: C.muted,
                       padding: "11px", borderRadius: 8, cursor: "pointer", fontSize: 12,
                       fontFamily: "-apple-system,sans-serif" }}>
              ← Back
            </button>
          )}
          {step < STEPS.length - 1 && (
            <button onClick={() => setStep(s => s + 1)} disabled={!canAdvance}
              style={{ flex: 3, background: canAdvance ? C.blue : C.bg3,
                       border: "none",
                       color: canAdvance ? "#ffffff" : C.dim,
                       padding: "11px", borderRadius: 8,
                       cursor: canAdvance ? "pointer" : "default", fontSize: 13,
                       fontWeight: "500", fontFamily: "-apple-system,sans-serif" }}>
              Continue →
            </button>
          )}
        </div>
