...
function getFormType(p) {
  if (p.solType === "RFP_COMMERCIAL" || p.solType === "RFQ_SIMPLIFIED") return "SF-1449";
...
function buildSectionM(p) {
  ...
  if (isLPTA) {
    text += `M.1 BASIS FOR AWARD\nAward will be made to the offeror whose proposal represents the Lowest Price Technically Acceptable (LPTA) offer. The Government will award a contract to the responsible offeror whose offer is technically acceptable and has the lowest evaluated price.\n\n`;
    text += `M.2 EVALUATION FACTORS\n\nFactor 1 — Technical Acceptability (Acceptable/Unacceptable)\n    The offeror's technical approach will be evaluated to determine whether it meets the Government's minimum requirements as stated in Section C.\n\nFactor 2 — Past Performance (Acceptable/Unacceptable/Unknown)\n    The offeror's past performance will be evaluated for relevance and quality.\n\nFactor 3 — Price\n    Price will be evaluated for reasonableness. The lowest price among technically acceptable proposals will receive award.`;
  } else {
    text += `M.1 BASIS FOR AWARD\nAward will be made to the offeror whose proposal represents the Best Value to the Government, considering the factors identified below. [TECHNICAL / PAST PERFORMANCE / PRICE] are listed in descending order of importance.\n\n`;
    text += `M.2 EVALUATION FACTORS\n\nFactor 1 — Technical Approach (Most Important)\n    The Government will evaluate the offeror's understanding of the requirement, technical approach, and feasibility of the proposed solution.\n\nFactor 2 — Past Performance\n    The Government will evaluate relevance, recency (within [X] years), and quality of the offeror's past performance on contracts of similar scope and complexity.\n\nFactor 3 — Price/Cost\n    Price will be evaluated for reasonableness. ${v >= 2500000 ? "Cost or pricing data may be required in accordance with FAR 15.403 (TINA threshold: $2.5M)." : ""}\n\nM.3 RELATIVE IMPORTANCE\nTechnical Approach and Past Performance, when combined, are [EQUAL TO / MORE IMPORTANT THAN] Price.\n\n[NOTE TO CO: Customize evaluation factors per acquisition complexity and center evaluation templates. Simple low-risk acquisitions may use LPTA. Confirm with supervisor before releasing.]`;
  }
  return text;
}
