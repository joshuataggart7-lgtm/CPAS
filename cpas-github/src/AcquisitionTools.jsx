            ...
            {H_PROVISIONS.map(p => (
              <div key={p.id} onClick={() => setActive(a => ({ ...a, [p.id]: !a[p.id] }))}
                style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 6px", cursor: "pointer",
                         background: active[p.id] ? "#041a0e" : "transparent",
                         border: `1px solid ${active[p.id] ? "#1a4a2a" : "transparent"}`,
                         borderRadius: 3, marginBottom: 3 }}>
                <div style={{ width: 13, height: 13, border: `1px solid ${active[p.id] ? C.green : "#2a4a6a"}`,
          ...
          <div style={{ maxHeight: "58vh", overflow: "auto" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10 }}>
              <div onClick={() => set("urgency", !bpa.urgency)}
                style={{ width: 14, height: 14, border: `1px solid ${bpa.urgency ? C.red : "#2a4a6a"}`,
            ...
            <button onClick={() => setStep(1)}
              style={{ width:"100%", marginTop:12, background:"#0a2a4a", border:`1px solid ${C.blue}`, color:C.blue,
        ...
        {step === 2 && (
          <div style={{ maxHeight: "58vh", overflow: "auto" }}>
            ...
            <div style={{ display:"flex", gap:8, marginTop:12 }}>
              ...
              <button onClick={()=>setStep(3)} style={{ flex:2, background:"#0a2a4a", border:`1px solid ${C.blue}`, color:C.blue, padding:"7px", borderRadius:3, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>PREVIEW →</button>
            </div>
          </div>
        )}
...
const ANOSCA_THRESHOLDS = {
  competitiveAward:    50000000,  // NFS 1805.303 — competitive awards $50M+
  soleSource:          25000000,  // NFS 1805.303 — sole source awards $25M+
  modIncrease:         50000000,  // mods increasing competitive contract above $50M
  soleSourceMod:       25000000,  // mods increasing sole source above $25M
  indefiniteDelivery:  100000000, // IDIQ max value $100M+
  // Note: NFS 1805.303 also requires ANOSCA for actions with significant public interest
  // regardless of dollar value — CO judgment required for unusual/controversial actions
};
...
export function ANOSCABadge({ intake }) {
  ...
  return (
    <div style={{
      background: "#1a0a0a", border: "1px solid #6a1a1a", borderRadius: 4,
      padding: "8px 12px", marginBottom: 10,
      fontFamily: "'IBM Plex Mono', monospace",
    }}>
      <div style={{ fontSize: 10, color: C.red, fontWeight: "bold", marginBottom: 4 }}>
        ⚠ ANOSCA REQUIRED — NFS 1805.303
      </div>
      {check.reasons.map((r, i) => (
        <div key={i} style={{ fontSize: 10, color: "#c07070" }}>• {r}</div>
      ))}
      <div style={{ fontSize: 9, color: C.muted, marginTop: 4 }}>
        Submit to NASA HQ before or at time of award. See ANOSCA step in roadmap.
      </div>
    </div>
  );
}
