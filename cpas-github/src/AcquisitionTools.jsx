  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: C.red, fontWeight: "bold", marginBottom: 4 }}>
        ⚠ {check.level === "ANOSCA" ? "ANOSCA + HQ PUBLIC ANNOUNCEMENT" : "NASA HQ PUBLIC ANNOUNCEMENT"} REQUIRED — NFS 1805.303-71 (PCD 25-16)
      </div>
      {check.reasons.map((r, i) => (
        <div key={i} style={{ fontSize: 10, color: "#c07070" }}>• {r}</div>
      ))}
      <div style={{ fontSize: 9, color: C.muted, marginTop: 4 }}>
        Submit NASA Notification of Contract Action to HQ Procurement Analyst before award. See "NASA Notification of Contract Action / ANOSCA" step in roadmap.
      </div>
    </div>
  );
}
