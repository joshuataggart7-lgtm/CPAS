...
async function generateDoc(docType, intake, roadmap) {
  ...
  const comm = intake?.isCommercial;
    ...
    try {
      ...
      if (pp && pp.catalogClins && pp.catalogClins.some(c => c.qty && c.rate)) {
...
function BranchChiefDashboard({ onClose }) {
  ...
  async function load(em) {
    setLoading(true);
    try {
      const r = await fetch(`${PKG_API}/list?role=BC&email=${encodeURIComponent(em)}`);
...
// ── Tools Menu Dropdown ───────────────────────────────────────────
function ToolsMenu({ onMod, onForms, onBPA, onUCF, onAward, onAdmin, onCompliance, onPreSol, onReview, onRequestor, onTechEval, onBranchChief }) {
  const [open, setOpen] = React.useState(false);
      ...
      fetch(`${PKG_API}/package/${pkgId}`)
        ...
        .then(pkg => {
          if (pkg && (pkg.packageData || pkg.package_data)) {
            ...
            localStorage.setItem("cpas_portal_package", JSON.stringify(p));
