...
async function callAI(prompt, systemPrompt) {
  ...
  if (!apiKey) {
    apiKey = window.prompt("Enter your Anthropic API key (sk-ant-...). Saved locally in browser:");
...
async function downloadAsWord(docType, text, intake) {
  try {
    ...
    if (!window.docx) return "Word library not available. Try refreshing the page.";

    const { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType,
      BorderStyle, LevelFormat, Header } = window.docx;
    ...
    const docTypeLabels = {
      ...
      PNM:"Price Negotiation Memorandum", ACQ_PLAN:"Procurement Strategy Meeting",
      QASP:"Quality Assurance Surveillance Plan", COR_LETTER:"COR Appointment Letter",
      MARKET_RESEARCH:"Market Research Report", IGCE:"Independent Government Cost Estimate",
      SOURCES_SOUGHT:"Sources Sought Notice", CLOSEOUT:"Contract Closeout Checklist"
    };
    ...
    for (const line of text.split("\n")) {
      ...
      if (!t || t === "---") continue;
      if (t.startsWith("# ")) {
        body.push(new Paragraph({ heading: HeadingLevel.HEADING_1, spacing: { before: 280, after: 100 },
          children: [new TextRun({ text: t.slice(2), bold: true, font: "Arial", size: 28, color: "1F4E79" })] }));
      } else if (t.startsWith("## ")) {
        body.push(new Paragraph({ heading: HeadingLevel.HEADING_2, spacing: { before: 200, after: 80 },
          children: [new TextRun({ text: t.slice(3), bold: true, font: "Arial", size: 24, color: "2E75B6" })] }));
      } else if (t.startsWith("### ")) {
        body.push(new Paragraph({ heading: HeadingLevel.HEADING_3, spacing: { before: 160, after: 60 },
          children: [new TextRun({ text: t.slice(4), bold: true, font: "Arial", size: 22, color: "2E75B6" })] }));
      } else if (t.match(/^[-*] /)) {
        body.push(new Paragraph({ numbering: { reference: "bullets", level: 0 }, spacing: { before: 40, after: 40 },
          children: makeRuns(t.replace(/^[-*] /, "")) }));
      } else {
        body.push(new Paragraph({ spacing: { before: 60, after: 60 }, children: makeRuns(t) }));
      }
    }
      ...
      sections: [{ properties: { page: { size: { width: 12240, height: 15840 },
        ...
        children: [
          ...
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 240, after: 80 },
            children: [new TextRun({ text: label, font: "Arial", size: 36, bold: true, color: "1F4E79" })] }),
          ...
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 360 },
            children: [new TextRun({ text: "Value: " + value + "  |  NAICS: " + (intake?.naics || "TBD") + "  |  Type: " + (intake?.contractType || "FFP"), font: "Arial", size: 20, color: "666666" })] }),
    ...
    return "Word export error: " + (e?.message || e?.toString() || "Unknown error - check console");
  }
}
