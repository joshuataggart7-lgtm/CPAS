...
async function downloadAsWord(docType, text, intake) {
  try {
    ...
    const co      = intake?.co       || "Contracting Officer";
    const program = intake?.program  || title;
    ...
    const sigLine = (label, sublabel) => [
      ...
      ...(sublabel ? [new Paragraph({
        children: [new TextRun({ text: sublabel, size: 22, italics: true })],
        spacing: { before: 0, after: 120 },
      })] : []),
    ];

    // Helper: section header (bold)
    ...
    // Parse AI text into paragraphs (split on newlines)
    const bodyParas = (text || "").split("\n").map(line => {
      const t = line.trim();
      if (!t) return blank();
      if (/^\d+\.\s/.test(t)) return new Paragraph({
    ...
    // NASA document header
    const docHeader = [
      new Paragraph({
        children: [new TextRun({ text: "NATIONAL AERONAUTICS AND SPACE ADMINISTRATION", bold: true, size: 28, allCaps: true })],
        alignment: AlignmentType.CENTER,
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [new TextRun({ text: center, bold: true, size: 24 })],
        alignment: AlignmentType.CENTER,
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [new TextRun({ text: docType + (docType === "JOFOC" ? " — JUSTIFICATION FOR OTHER THAN FULL AND OPEN COMPETITION" : ""), bold: true, size: 26, allCaps: true })],
        alignment: AlignmentType.CENTER,
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [new TextRun({ text: "For: " + title, bold: true, size: 22 })],
        alignment: AlignmentType.CENTER,
        spacing: { after: 80 },
      }),
      new Paragraph({
        children: [new TextRun({ text: date, size: 22, italics: true })],
        alignment: AlignmentType.CENTER,
        spacing: { after: 400 },
      }),
      blank(),
    ...
    // Signature page logic based on value and docType
    const buildSigPage = () => {
      const v = parseFloat(value) || 0;
      const paras = [
        new Paragraph({ children: [new PageBreak()] }),
        new Paragraph({
          children: [new TextRun({ text: program, bold: true, size: 24 })],
          alignment: AlignmentType.CENTER,
          spacing: { after: 80 },
        }),
        new Paragraph({
          children: [new TextRun({ text: docType + " SIGNATURE PAGE", bold: true, size: 24, allCaps: true })],
          alignment: AlignmentType.CENTER,
          spacing: { after: 200 },
        }),
      ];
      ...
      if (docType === "JOFOC") {
        let thresholdNote = "";
        if (v <= 900000) thresholdNote = "Signature authority for actions ≤ $900K";
        else if (v <= 20000000) thresholdNote = "Signature authority for actions > $900K and ≤ $20M";
        else if (v <= 150000000) thresholdNote = "Signature authority for actions > $20M and ≤ $150M";
        else thresholdNote = "Signature authority for actions > $150M";

        paras.push(bodyPara(thresholdNote));
        paras.push(blank());
        paras.push(bodyPara("Technical Representative: I certify that the facts presented in this justification are accurate and complete."));
        paras.push(...sigLine("(Insert Name)", "Technical Representative"));
        paras.push(blank());
        paras.push(bodyPara("Contracting Officer: I hereby certify that the above justification is complete and accurate to the best of my knowledge and belief."));
        paras.push(...sigLine(co, "Contracting Officer"));

        if (v > 900000) {
          paras.push(blank());
          paras.push(v <= 20000000 ? bodyPara("APPROVED:") : bodyPara("CONCURRENCES:"));
          paras.push(...sigLine("(Insert Name)", "Competition Advocate, " + center));
        }
        if (v > 20000000) {
          paras.push(...sigLine("(Insert Name)", "Procurement Officer [NOJMO, ESDMD, SOMD actions only]"));
          paras.push(blank());
          paras.push(bodyPara("APPROVAL:"));
          paras.push(...sigLine("(Insert Name)", "Head of Contracting Activity, " + center));
        }
        if (v > 150000000) {
          paras.push(...sigLine("(Insert Name)", "Office of the General Counsel at Headquarters"));
          paras.push(...sigLine("(Insert Name)", "Agency Competition Advocate"));
          paras.push(blank());
          paras.push(bodyPara("APPROVAL:"));
          paras.push(...sigLine("(Insert Name)", "Senior Procurement Executive"));
        }
      } else if (docType === "ACQ_PLAN") {
        // PSM signature page
        paras.push(bodyPara("PSM STAKEHOLDER CONCURRENCE"));
        paras.push(...sigLine("(Insert Name)", "Procurement Officer / Center Procurement Office Representative"));
        paras.push(...sigLine("(Insert Name)", "Chief Financial Officer / IGCE Approving Official"));
        paras.push(...sigLine("(Insert Name)", "Center OSBP Representative"));
        paras.push(...sigLine("(Insert Name)", "Center Requirements Office Representative"));
        paras.push(blank());
        paras.push(bodyPara("APPROVAL:"));
        paras.push(...sigLine(co, "Contracting Officer"));
      } else {
        // Generic CO signature block
        paras.push(bodyPara("I certify that this document is complete and accurate to the best of my knowledge and belief."));
        paras.push(...sigLine(co, "Contracting Officer"));
        paras.push(...sigLine("(Insert Name)", "Technical Representative / COR"));
        paras.push(blank());
        paras.push(bodyPara("APPROVED:"));
        paras.push(...sigLine("(Insert Name)", "Supervisor / Branch Chief"));
      }
    ...
    a.href = url;
    a.download = docType.replace(/[^a-zA-Z0-9]/g,"_") + "_" + title.replace(/[^a-zA-Z0-9 ]/g,"").replace(/ /g,"_").slice(0,30) + ".docx";
    a.click();
