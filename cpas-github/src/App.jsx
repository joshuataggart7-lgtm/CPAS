...
function getAcqLane(value, isCommercial) {
...
const CHECKLIST = {"P1":[{"id":11,"q":"Funds budgeted and available for the intended action?","app":"All actions obligating funding"},{"id":14,"q":"Approved SOW/PWS included in the file?","app":"All actions"},{"id":15,"q":"QASP or written determination that no QASP is required?","app":"All new contracts for services"},{"id":19,"q":"Complete, signed, dated IGCE prepared by requiring office?","app":"All new actions above the SAT"},{"id":49,"q":"Determination that no SOW requirements include inherently governmental functions?","app":"All new awards"}],"P2":[{"id":23,"q":"Approved written Acquisition Plan on file?","app":"All new contracts meeting thresholds"},{"id":25,"q":"Approved SSP signed prior to solicitation release?","app":"Competitive FAR Part 15 actions >= $10M"},{"id":33,"q":"Approved JOFOC using appropriate template with required signatures?","app":"Above SAT - other than full and open competition"},{"id":35,"q":"Completed and signed Limited Source Justification?","app":"Actions using Limited Source Justification IAW FAR Part 13"},{"id":27,"q":"Appropriate approvals obtained IAW authority matrix?","app":"All actions per MDA 5013.01"}],"P3":[{"id":28,"q":"Signed and fully coordinated NF 1787 Small Business Coordination Record?","app":"All new contracts meeting thresholds"},{"id":29,"q":"Market Research Report (MRR) within 18 months on file?","app":"All new awards per FAR 10.001"},{"id":71,"q":"Documentation if Industry Day or Small Business Conference was held?","app":"New competitive contracts with industry exchanges"},{"id":77,"q":"Policy Compliance Review (PCR) completed IAW PS-OWI-05 and CAM?","app":"All actions per PS-OWI-05"}],"P4":[{"id":29,"q":"Recent Market Research Report (within 18 months) in file?","app":"All new awards per FAR 10.001"},{"id":30,"q":"If prior MRR reused, rationale documented?","app":"Orders using previous MRR"},{"id":31,"q":"Requirement publicized on SAM.gov with dated synopsis?","app":"Actions meeting FAR 5.101 threshold"}],"P5":[{"id":2,"q":"All documents filed under correct NEAR File Element Tab?","app":"All actions"},{"id":3,"q":"Duplicate and unnecessary drafts removed from file?","app":"All actions"},{"id":13,"q":"Final version of SOW/PWS included in file?","app":"All actions"},{"id":31,"q":"Requirement publicized on SAM.gov?","app":"Actions meeting FAR 5.101 threshold"},{"id":54,"q":"Responsibility/non-responsibility determination for awardee?","app":"All new awards above SAT"},{"id":66,"q":"Personal services determination documented?","app":"All new contracts for services"},{"id":67,"q":"Waivers or deviations from standard clauses documented?","app":"When waiver or deviation approved"}],"P6":[{"id":99,"q":"All proposals, revisions, and attachments included?","app":"All actions requiring proposals"},{"id":100,"q":"CBPI/SSI documents properly marked and controlled?","app":"New competitive contracts"},{"id":101,"q":"Proposals marked with date and time of receipt?","app":"New competitive contracts"},{"id":104,"q":"Dated SAM exclusion record generated after proposal receipt?","app":"All actions requiring proposals"},{"id":105,"q":"Contractor reps and certs retrieved from SAM.gov?","app":"All actions unless FAR 4.1102 exception applies"},{"id":112,"q":"Complete, signed, dated technical evaluation in file?","app":"All actions requiring technical expert input"}],"P7":[{"id":7,"q":"All documents uploaded into NEAR system?","app":"All actions"},{"id":34,"q":"Redacted JOFOC approved and posted to SAM.gov if required?","app":"Actions using other than full and open competition"},{"id":168,"q":"Signed copy of award/agreement in file?","app":"All actions"},{"id":171,"q":"Effective Date and PoP start on or after Signature Date?","app":"All actions except those with approved Advance Agreement"},{"id":179,"q":"Finalized CAR approved within 3 business days and uploaded?","app":"All actions"},{"id":185,"q":"Dated SAM.gov award synopsis publicized?","app":"All new contracts above FAR Part 5 threshold"},{"id":186,"q":"Award Notification Letters on file?","app":"New competitive contracts"},{"id":196,"q":"COR reports completed and filed?","app":"Contracts with a COR"}]};
      ...
      <div style={S.panelBody}>
        ...
        {step.type === "CHECK" && (
          <div style={S.panelSection}>
            ...
            <textarea
              style={S.textarea}
              placeholder="Document what you found, file location, ..."
              value={notes}
...
function IntakeWizard({ onComplete }) {
  ...
  function next() {
    ...
    if (isLast) onComplete({ ...ans, naics:"541330", psc:"R499", pop:"Base year + 4 option years" });
    else setStep(s=>s+1);
  }
...
const REVIEW_PROMPTS = {
  SOW: (ctx) => "You are a NASA Contracting Officer reviewing a Statement of Work (SOW) or Performance Work Statement (PWS). Acquisition context: "+(ctx)+"\n\nReturn ONLY JSON (no markdown): {\"docType\":\"SOW/PWS\",\"overallRating\":\"ACCEPTABLE|NEEDS_WORK|MAJOR_ISSUES\",\"summary\":\"...\",\"findings\":[{\"severity\":\"CRITICAL|MAJOR|MINOR|INFO\",\"category\":\"...\",\"issue\":\"...\",\"recommendation\":\"...\"}]}\nCheck: (1) Measurable deliverables/acceptance criteria, (2) PoP/place of performance, (3) PWS outcomes not methods, (4) GFP/GFE identified, (5) Security requirements, (6) Section 508 for IT, (7) Inherently governmental functions, (8) QASP-able standards, (9) Shall/will consistency, (10) Clause hooks (travel/IP/key personnel).",

  JOFOC: (ctx) => "NASA CO JOFOC/J&A review. Context: "+(ctx)+". Return ONLY JSON: {\"docType\":\"JOFOC/J&A\",\"overallRating\":\"ACCEPTABLE|NEEDS_WORK|MAJOR_ISSUES\",\"summary\":\"...\",\"findings\":[{\"severity\":\"CRITICAL|MAJOR|MINOR|INFO\",\"category\":\"...\",\"issue\":\"...\",\"recommendation\":\"...\"}]}\nCheck all 8 FAR 6.303-2 elements: (1) Agency/contracting activity, (2) Nature of action, (3) Description/cost, (4) FAR 6.302-X authority, (5) Only-one-source justification with technical facts, (6) Actions to maximize competition, (7) Actions to remove barriers, (8) CO/technical certification. Flag: FAR 6.304 approval thresholds, correct 6.302 subsection.",

  IGCE: (ctx) => "NASA CO IGCE review. Context: "+(ctx)+". Return ONLY JSON: {\"docType\":\"IGCE\",\"overallRating\":\"ACCEPTABLE|NEEDS_WORK|MAJOR_ISSUES\",\"summary\":\"...\",\"findings\":[{\"severity\":\"CRITICAL|MAJOR|MINOR|INFO\",\"category\":\"...\",\"issue\":\"...\",\"recommendation\":\"...\"}]}\nCheck: (1) Full cost breakdown (labor/ODCs/travel/G&A/fee), (2) Rate sources documented, (3) Basis of estimate explained, (4) Option year escalation ~3-4%/yr, (5) Marked Source Selection Sensitive, (6) Prepared by tech office not CO, (7) Within 18 months of award, (8) Total matches funded PR, (9) Assumptions stated, (10) Labor categories match SOW tasks.",

  CLAUSES: (ctx) => "NASA CO solicitation/clause review. Context: "+(ctx)+". Return ONLY JSON: {\"docType\":\"Solicitation/Clauses\",\"overallRating\":\"ACCEPTABLE|NEEDS_WORK|MAJOR_ISSUES\",\"summary\":\"...\",\"findings\":[{\"severity\":\"CRITICAL|MAJOR|MINOR|INFO\",\"category\":\"...\",\"issue\":\"...\",\"recommendation\":\"...\"}]}\nCheck: (1) FAR 52.212-1 through 52.212-5 for commercial, (2) NFS 1852.2xx clauses present, (3) All fill-ins complete no blank [], (4) Clause dates current, (5) Section B CLINs map to Section C, (6) Section H matches Section I authority, (7) Option CLINs have unilateral language, (8) Section M aligns with Section L, (9) FAR 52.222-x labor clauses correct, (10) NFS 1852.227 data rights appropriate."
};
...
function NEARPackage({ intake, roadmap }) {
  ...
  return (
    <div style={{ padding:"20px", maxWidth:"860px", margin:"0 auto" }}>
      ...
      <div style={{ fontSize:"11px", color:"#4a7aaa",marginBottom:"16px"}}>${(intake.value||0).toLocaleString()} | {intake.center} | {intake.contractType}</div>
...
function buildRoadmap(intake) {
  ...
  const lane = getAcqLane(value, isCommercial);
  ...
  try {
    if (!window.docxLoaded) {
      await new Promise((resolve, reject) => {
        ...
        s.onload = () => { window.docxLoaded = true; resolve(); };
        s.onerror = () => reject(new Error("Failed to load docx library"));
        document.head.appendChild(s);
      });
    }
    if (!window.docx) return "Word library not available.";
    const { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType, BorderStyle, LevelFormat, Header } = window.docx;
    ...
    const docTypeLabels = { JOFOC:"Justification for Other Than Full and Open Competition", PNM:"Price Negotiation Memorandum", ACQ_PLAN:"Procurement Strategy Meeting", QASP:"Quality Assurance Surveillance Plan", COR_LETTER:"COR Appointment Letter", MARKET_RESEARCH:"Market Research Report", IGCE:"Independent Government Cost Estimate", SOURCES_SOUGHT:"Sources Sought Notice", CLOSEOUT:"Contract Closeout Checklist" };
    const label = docTypeLabels[docType] || docType;
    function makeRuns(txt) {
      return txt.split(/(\*\*[^*]+\*\*)/).filter(p=>p).map(p =>
        (p.startsWith("**")&&p.endsWith("**")) ? new TextRun({text:p.slice(2,-2),bold:true,font:"Arial",size:22}) : new TextRun({text:p,font:"Arial",size:22})
      );
    }
    const body = [];
    for (const line of text.split("\n")) {
      const t = line.trim();
      if (!t || t==="---") continue;
      if (t.startsWith("# ")) body.push(new Paragraph({heading:HeadingLevel.HEADING_1,spacing:{before:280,after:100},children:[new TextRun({text:t.slice(2),bold:true,font:"Arial",size:28,color:"1F4E79"})]}));
      else if (t.startsWith("## ")) body.push(new Paragraph({heading:HeadingLevel.HEADING_2,spacing:{before:200,after:80},children:[new TextRun({text:t.slice(3),bold:true,font:"Arial",size:24,color:"2E75B6"})]}));
      else if (t.startsWith("### ")) body.push(new Paragraph({heading:HeadingLevel.HEADING_3,spacing:{before:160,after:60},children:[new TextRun({text:t.slice(4),bold:true,font:"Arial",size:22,color:"2E75B6"})]}));
      else if (t.match(/^[-*] /)) body.push(new Paragraph({numbering:{reference:"bullets",level:0},spacing:{before:40,after:40},children:makeRuns(t.replace(/^[-*] /,""))}));
      else body.push(new Paragraph({spacing:{before:60,after:60},children:makeRuns(t)}));
    }
    const doc = new Document({
      numbering:{config:[{reference:"bullets",levels:[{level:0,format:LevelFormat.BULLET,text:"\u2022",alignment:AlignmentType.LEFT,style:{paragraph:{indent:{left:720,hanging:360}},run:{font:"Arial",size:22}}}]}]},
      styles:{default:{document:{run:{font:"Arial",size:22}}},paragraphStyles:[
        {id:"Heading1",name:"Heading 1",basedOn:"Normal",next:"Normal",quickFormat:true,run:{size:28,bold:true,font:"Arial",color:"1F4E79"},paragraph:{spacing:{before:280,after:100},outlineLevel:0}},
        {id:"Heading2",name:"Heading 2",basedOn:"Normal",next:"Normal",quickFormat:true,run:{size:24,bold:true,font:"Arial",color:"2E75B6"},paragraph:{spacing:{before:200,after:80},outlineLevel:1}},
        {id:"Heading3",name:"Heading 3",basedOn:"Normal",next:"Normal",quickFormat:true,run:{size:22,bold:true,font:"Arial",color:"2E75B6"},paragraph:{spacing:{before:160,after:60},outlineLevel:2}}
      ]},
      sections:[{properties:{page:{size:{width:12240,height:15840},margin:{top:1440,right:1440,bottom:1440,left:1440}}},
        headers:{default:new Header({children:[new Paragraph({border:{bottom:{style:BorderStyle.SINGLE,size:6,color:"1F4E79",space:4}},spacing:{after:120},children:[new TextRun({text:"NASA "+center+" — ACQUISITION DOCUMENT",font:"Arial",size:18,color:"1F4E79",bold:true}),new TextRun({text:"    SOURCE SELECTION SENSITIVE",font:"Arial",size:18,color:"CC0000",bold:true})]})  ]})},
        children:[
          new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:480,after:80},children:[new TextRun({text:"NATIONAL AERONAUTICS AND SPACE ADMINISTRATION",font:"Arial",size:20,bold:true,color:"1F4E79"})]}),
          new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:0,after:80},children:[new TextRun({text:center,font:"Arial",size:20,color:"1F4E79"})]}),
          new Paragraph({border:{bottom:{style:BorderStyle.SINGLE,size:12,color:"1F4E79",space:4}},spacing:{before:0,after:240}}),
          new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:240,after:80},children:[new TextRun({text:label,font:"Arial",size:36,bold:true,color:"1F4E79"})]}),
          new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:0,after:120},children:[new TextRun({text:title,font:"Arial",size:26,color:"333333"})]}),
          new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:0,after:360},children:[new TextRun({text:"Value: "+value+"  |  NAICS: "+(intake?.naics||"TBD")+"  |  Type: "+(intake?.contractType||"FFP"),font:"Arial",size:20,color:"666666"})]}),
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = label.replace(/[^a-zA-Z0-9]/g,"_").slice(0,25)+"_"+title.replace(/[^a-zA-Z0-9]/g,"_").slice(0,25)+".docx";
    a.click();
    URL.revokeObjectURL(url);
    return null;
  } catch(e) { console.error("Word export:",e); return "Word export error: "+(e?.message||String(e)); }
}
