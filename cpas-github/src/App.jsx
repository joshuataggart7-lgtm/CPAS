  ...
  if (isTaskOrder) {
    phases.push({ id:"P2", title:"Vehicle Identification & Fair Opportunity",
      steps:[
        ...
        { id:"P2S2", title:"Confirm Ordering Period is Active", type:"CHECK", nfs:"RFO FAR 16.507-3 / FAR 16.505(a)(1)" },
        { id:"P2S3", title:"NAICS Code & Size Standard", type:"FORM", formType:"NAICS" },
        { id:"P2S4", title:"Fair Opportunity Decision", type:"DECISION", nfs:"RFO FAR 16.507-6(b) [formerly FAR 16.505(b)(2)] — Fair opportunity exception D&F required if not using full FO",
          decisionOptions:[
            { label:"Full Fair Opportunity - RFQ to all awardees", sub:"RFO FAR 16.507-6(a) — Standard approach; no D&F required." },
            { label:"Exception: Only One Capable", sub:"RFO FAR 16.507-6(b)(1)(i) — Document why only one source capable." },
            { label:"Exception: Unusual Urgency", sub:"RFO FAR 16.507-6(b)(1)(ii) — Urgency precludes fair opportunity." },
            { label:"Exception: Logical Follow-On", sub:"RFO FAR 16.507-6(b)(1)(iii) — Continuation of prior order." },
            { label:"Exception: Best Interest of Government", sub:"RFO FAR 16.507-6(b)(1)(iv) — HCA approval required." },
          ]},
...
import ContractRecord from "./ContractRecord.jsx";
import MyAcquisitions from "./MyAcquisitions.jsx";
import StandaloneClauseMatrix from "./StandaloneClauseMatrix.jsx";
import DataRightsAttachment from "./DataRightsAttachment.jsx";
import AttachmentsBuilder from "./AttachmentsBuilder.jsx";
import RegulatorySearch from "./RegulatorySearch.jsx";
...
function AwardRecordModal({ intake, acquisitionId, sbUrl, sbKey, onClose, onSaved }) {
  const FONT = "-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif";
  const C = { blue:"#1a3a6e", green:"#0f6e56", border:"#dde3ef", muted:"#6b7a99", text:"#1a2332" };
  const inp = { background:"#fff", border:`1px solid ${C.border}`, color:C.text,
    padding:"8px 12px", borderRadius:8, fontSize:12, width:"100%",
    boxSizing:"border-box", fontFamily:FONT, outline:"none", marginBottom:2 };
  const lbl = (t) => <div style={{fontSize:10,color:C.muted,fontWeight:"600",
    letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:4,marginTop:10,fontFamily:FONT}}>{t}</div>;
  ...
  async function save() {
    if (!d.contractNumber.trim()) { setMsg("Contract number is required."); return; }
    setSaving(true);
    try {
      // Update cpas_acquisitions status to Awarded
      if (acquisitionId) {
        await fetch(`${sbUrl}/rest/v1/cpas_acquisitions?id=eq.${acquisitionId}`, {
          method: "PATCH",
      ...
      // Update cpas_contract_records — find matching pre-award record and flip to Post-Award
      const matchRes = await fetch(
        `${sbUrl}/rest/v1/cpas_contract_records?co_email=eq.${encodeURIComponent((intake?.coEmail||"").toLowerCase())}&status=eq.Pre-Award&order=created_at.desc&limit=1`,
        { headers: { "apikey":sbKey, "Authorization":`Bearer ${sbKey}` } }
      );
      const matches = await matchRes.json();
      if (Array.isArray(matches) && matches[0]) {
        await fetch(`${sbUrl}/rest/v1/cpas_contract_records?id=eq.${matches[0].id}`, {
          method: "PATCH",
          headers: { "Content-Type":"application/json", "apikey":sbKey, "Authorization":`Bearer ${sbKey}`, "Prefer":"return=minimal" },
          body: JSON.stringify({
            status: "Active",
            ...
            contractor_uei: d.contractorUEI,
            award_date: d.awardDate,
            current_value: parseFloat(d.finalValue) || null,
            potential_value: parseFloat(d.finalValue) || null,
            naics: d.naics,
            psc: d.psc,
          }),
        });
      }
      onSaved(d.contractNumber);
    } catch(e) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FONT}}>
      <div style={{background:"#f5f7fa",borderRadius:12,padding:"24px 28px",maxWidth:520,width:"94%",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 8px 40px rgba(26,58,110,0.18)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
          <div style={{fontSize:15,fontWeight:"600",color:C.text}}>🏆 Record Contract Award</div>
        ...
        <div style={{fontSize:11,color:C.muted,marginBottom:20,lineHeight:1.5}}>
          Award Document step completed for <strong style={{color:C.text}}>{intake?.reqTitle || "this acquisition"}</strong>.
          Record the award details to update the contract lifecycle record.
        </div>

        <div style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:10,padding:"16px 18px"}}>
          <div style={{fontSize:11,fontWeight:"600",color:C.blue,textTransform:"uppercase",letterSpacing:"0.6px",marginBottom:4}}>Contract Award Details</div>

          {lbl("Contract Number (PIID) *")}
          <input style={inp} value={d.contractNumber} onChange={e=>set("contractNumber",e.target.value)} placeholder="e.g., 80ARC025DA006" />

          {lbl("Award Date")}
          <input type="date" style={inp} value={d.awardDate} onChange={e=>set("awardDate",e.target.value)} />

          {lbl("Final Award Value ($)")}
          <input type="number" style={inp} value={d.finalValue} onChange={e=>set("finalValue",e.target.value)} placeholder="e.g., 13500000" />

          {lbl("Contractor Name")}
          <input style={inp} value={d.contractorName} onChange={e=>set("contractorName",e.target.value)} placeholder="e.g., Dynamic Aviation Group, Inc." />

          {lbl("Contractor Address")}
          <input style={inp} value={d.contractorAddress} onChange={e=>set("contractorAddress",e.target.value)} placeholder="Street, City, State ZIP" />

          {lbl("Contractor UEI")}
          <input style={inp} value={d.contractorUEI} onChange={e=>set("contractorUEI",e.target.value)} placeholder="12-character UEI from SAM.gov" />

          {lbl("NAICS")}
          <input style={inp} value={d.naics} onChange={e=>set("naics",e.target.value)} />

          {lbl("PSC")}
          <input style={inp} value={d.psc} onChange={e=>set("psc",e.target.value)} />
        </div>

        {msg && <div style={{fontSize:11,color:"#a32d2d",marginTop:10}}>{msg}</div>}

        <div style={{display:"flex",gap:10,marginTop:16}}>
          <button onClick={save} disabled={saving}
            style={{flex:1,background:saving?"#8896b0":C.green,border:"none",color:"#fff",
              padding:"11px",borderRadius:8,cursor:saving?"default":"pointer",
              fontSize:13,fontWeight:"500",fontFamily:FONT}}>
            {saving ? "Saving..." : "Save Award Record"}
          </button>
          <button onClick={onClose}
            style={{background:"none",border:`1px solid ${C.border}`,color:C.muted,
              padding:"11px 16px",borderRadius:8,cursor:"pointer",fontSize:12,fontFamily:FONT}}>
            Skip
          </button>
        </div>
        <div style={{fontSize:10,color:C.muted,marginTop:10,lineHeight:1.5}}>
          This updates the contract record from Pre-Award to Active status and records award details for the post-award lifecycle workspace. You can also update these fields later via Tools → Contract Admin.
        </div>
      </div>
    </div>
  );
...
function BranchChiefDashboard({ onClose }) {
  ...
  const statusColor = { PENDING: "#854f0b", ASSIGNED: "#2db87a", REJECTED: "#e05555", IN_PROGRESS: "#5ba3ff" };
  const urgencyColor = { URGENT: "#a32d2d", MODERATE: "#854f0b", NORMAL: "#2db87a" };

  const inp = { background: "#ffffff", border: "1px solid #dde3ef", color: "#1a2332", padding: "8px 12px",
    borderRadius: 7, fontSize: 12, width: "100%", boxSizing: "border-box", fontFamily: "system-ui, sans-serif" };

  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:997, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#ffffff", border:"1px solid #2a4a7a", borderRadius:8, width:700, maxWidth:"96vw", maxHeight:"92vh", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding:"16px 20px", borderBottom:"0.5px solid #dde3ef", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {email && <button onClick={()=>{setEmail("");localStorage.removeItem("cpas_bc_email");}}
              style={{ background:"none", border:"1px solid #1a2a4a", color:"#6b7a99", padding:"4px 10px", borderRadius:4, cursor:"pointer", fontSize:10 }}>
              SWITCH USER
            </button>}
        ...
        <div style={{ flex:1, overflow:"auto", padding:20 }}>
          ...
          {!email ? (
            <div>
              <div style={{ color:"#6b7a99", fontSize:12, marginBottom:12 }}>Enter your email to see the acquisition package queue.</div>
              ...
              <button onClick={login} style={{ width:"100%", background:"#1a3a6e", border:"1px solid #1a3a6e", color:"#ffffff",
            ...
            <div>
              {successMsg && (
                <div style={{ background:"#e8f7f0", border:"1px solid #9fe1cb", borderRadius:4, padding:"10px 14px", marginBottom:14, color:"#2db87a", fontSize:11 }}>
                  ✓ {successMsg}
              ...
              {packages.length === 0 ? (
                ...
                <div key={pkg.id} style={{ background: selected===pkg.id?"#1a3a5e":"#eef3fc", border:`1px solid ${selected===pkg.id?"#5ba3ff":"#2255a4"}`,
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:6 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ color:"#1a2332", fontWeight:"bold", fontSize:13 }}>{pkg.reqTitle}</div>
                    ...
                    <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
                      ...
                      <span style={{ background: pkg.daysWaiting >= 5 ? "#cc333322" : "#eef3fc",
                        color: pkg.daysWaiting >= 5 ? "#a32d2d" : "#7090b8",
                        border:`1px solid ${pkg.daysWaiting >= 5 ? "#a32d2d" : "#2255a4"}`,
                  ...
                  {selected === pkg.id && (
                    <div style={{ marginTop:12, borderTop:"1px solid #1a3a6e", paddingTop:12 }}>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12, fontSize:11 }}>
                        <div><span style={{ color:"#6b7a99" }}>Submitted: </span><span style={{ color:"#1a2332" }}>{new Date(pkg.submitted_at||pkg.submittedAt).toLocaleDateString()}</span></div>
                        <div><span style={{ color:"#6b7a99" }}>COR Email: </span><span style={{ color:"#1a2332" }}>{pkg.cor_email||pkg.corEmail}</span></div>
                        {(pkg.package_data?.sowObjective||pkg.packageData?.sowObjective) && (
                          <div style={{ gridColumn:"1/-1" }}>
                            <span style={{ color:"#6b7a99" }}>Objective: </span>
                            <span style={{ color:"#1a2332" }}>{(pkg.package_data?.sowObjective||pkg.packageData?.sowObjective||"").slice(0,120)}...</span>
                          </div>
                        )}
                        ...
                        {(pkg.rejection_reason||pkg.rejectionReason) && (
                          <div style={{ gridColumn:"1/-1" }}><span style={{ color:"#e05555" }}>Rejection reason: </span><span style={{ color:"#1a2332" }}>{pkg.rejection_reason||pkg.rejectionReason}</span></div>
                        )}
                      </div>

                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        <button onClick={e=>{e.stopPropagation();setViewPkg(pkg);}}
                          style={{ background:"#ffffff", border:"1px solid #4a7aaa", color:"#6b7a99",
                        ...
                        {pkg.status === "PENDING" && (<>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"approve",pkg});}}
                            style={{ background:"#e8f7f0", border:"1px solid #9fe1cb", color:"#2db87a",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ✓ APPROVE & ASSIGN
                          </button>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reject",pkg});}}
                            style={{ background:"#fff0f0", border:"1px solid #cc4a4a", color:"#e05555",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ✗ RETURN TO COR
                          </button>
                        </>)}
                        {pkg.status === "ASSIGNED" && (<>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reassign",pkg});}}
                            style={{ background:"#ffffff", border:"1px solid #4a9eff", color:"#2a5aaa",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ↺ REASSIGN
                          </button>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reject",pkg});}}
                    ...
                    return (
                      <div key={i} style={{ background:"#ffffff", border:"1px solid #1a3a6e", borderRadius:6, padding:"12px 16px", marginBottom:8 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
                          <div style={{ flex:1 }}>
                            <div style={{ color:"#1a2332", fontWeight:"bold", fontSize:13 }}>{pkg.req_title}</div>
                        ...
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginTop:10, fontSize:11 }}>
                          <div style={{ background:"#ffffff", borderRadius:4, padding:"8px 10px" }}>
                            ...
                            <div style={{ color:"#1a2332", fontWeight:"bold" }}>{pkg.assigned_to_name||"—"}</div>
                          ...
                          <div style={{ background:"#ffffff", borderRadius:4, padding:"8px 10px" }}>
                            ...
                            <div style={{ color: daysSinceAssign>=14?"#a32d2d":daysSinceAssign>=7?"#854f0b":"#e8eef8",
                              fontWeight:"bold", fontSize:18 }}>{daysSinceAssign??0}</div>
                          </div>
                          <div style={{ background:"#ffffff", borderRadius:4, padding:"8px 10px" }}>
                            <div style={{ color:"#6b7a99", fontSize:9, marginBottom:2 }}>DAYS SINCE STARTED</div>
                            <div style={{ color:"#1a2332", fontWeight:"bold", fontSize:18 }}>{daysSinceStart??0}</div>
                          </div>
                        </div>
                      </div>
                    );
      ...
      {viewPkg && (() => {
        ...
        const field = (label, val) => val ? (
          <div style={{ marginBottom:10 }}>
            <div style={{ color:"#2a5aaa", fontSize:10, letterSpacing:"0.5px", textTransform:"uppercase", marginBottom:3 }}>{label}</div>
            <div style={{ color:"#1a2332", fontSize:12, lineHeight:1.5, whiteSpace:"pre-wrap" }}>{val}</div>
          </div>
        ) : null;
        const sect = (title) => (
        ...
        return (
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:10 }}>
            <div style={{ background:"#ffffff", border:"1px solid #2a4a7a", borderRadius:8, width:680, maxWidth:"96vw",
              ...
              <div style={{ padding:"16px 20px", borderBottom:"0.5px solid #dde3ef", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
                <div>
                  <div style={{ color:"#1a2332", fontWeight:"bold", fontSize:14 }}>{viewPkg.req_title || p.reqtitle}</div>
              ...
              <div style={{ flex:1, overflow:"auto", padding:20 }}>
                ...
                {p.tasks && p.tasks.length > 0 && (
                  <div style={{ marginBottom:10 }}>
                    <div style={{ color:"#2a5aaa", fontSize:10, letterSpacing:"0.5px", textTransform:"uppercase", marginBottom:6 }}>WORK TASKS</div>
                    {p.tasks.map((t,i) => (
                      <div key={i} style={{ background:"#ffffff", border:"1px solid #dde3ef", borderRadius:7, padding:"8px 12px", marginBottom:6 }}>
                        <div style={{ color:"#1a2332", fontWeight:"bold", fontSize:12 }}>{t.title}</div>
                        <div style={{ color:"#6b7a99", fontSize:11, marginTop:2 }}>{t.desc}</div>
                ...
                {p.igceMode === "CATALOG" && p.catalogClins && p.catalogClins.length > 0 && (
                  <div style={{ marginBottom:10 }}>
                    <div style={{ color:"#2a5aaa", fontSize:10, letterSpacing:"0.5px", textTransform:"uppercase", marginBottom:6 }}>CATALOG LINE ITEMS</div>
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                      <thead>
                        <tr style={{ background:"#ffffff" }}>
                          {["DESCRIPTION","UNIT","RATE","QTY/YR","TOTAL"].map(h => (
                            <td key={h} style={{ padding:"5px 8px", color:"#2a5aaa", borderBottom:"0.5px solid #dde3ef" }}>{h}</td>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {p.catalogClins.filter(c=>c.qty).map((c,i) => (
                          <tr key={i} style={{ background: i%2===0?"#eef3fc":"#0a1f38" }}>
                            <td style={{ padding:"5px 8px", color:"#1a2332", borderBottom:"1px solid #0d1f38" }}>{c.title}</td>
                            <td style={{ padding:"5px 8px", color:"#6b7a99", borderBottom:"1px solid #0d1f38" }}>{c.unit}</td>
                            <td style={{ padding:"5px 8px", color:"#6b7a99", borderBottom:"1px solid #0d1f38" }}>${parseFloat(c.rate||0).toFixed(2)}</td>
                            <td style={{ padding:"5px 8px", color:"#6b7a99", borderBottom:"1px solid #0d1f38" }}>{c.qty}</td>
                            <td style={{ padding:"5px 8px", color:"#1a2332", borderBottom:"1px solid #0d1f38" }}>${(parseFloat(c.rate||0)*parseFloat(c.qty||0)).toLocaleString(undefined,{maximumFractionDigits:0})}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div style={{ padding:"12px 20px", borderTop:"1px solid #1a2a4a", display:"flex", gap:8, flexShrink:0 }}>
                ...
                <button onClick={()=>setViewPkg(null)}
                  style={{ background:"#ffffff", border:"1px solid #1a2a4a", color:"#6b7a99",
      ...
      {(action?.type === "approve" || action?.type === "reassign") && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#ffffff", border:`1px solid ${action.type === "reassign" ? "#4a7abf" : "#9fe1cb"}`, borderRadius:8, padding:28, width:400, maxWidth:"95vw" }}>
            ...
            {action.type === "reassign" && (
              <div style={{ background:"#ffffff", border:"1px solid #dde3ef", borderRadius:7, padding:"8px 12px", marginBottom:12, fontSize:11, color:"#6b7a99" }}>
                Currently assigned to: <strong style={{ color:"#1a2332" }}>{action.pkg.assignedToName || action.pkg.assignedToEmail || action.pkg.assigned_to_name || action.pkg.assigned_to_email}</strong>
              </div>
            )}
            <div style={{ color:"#8896b0", fontSize:11, marginBottom:16 }}>{action.pkg.reqTitle || action.pkg.req_title}</div>
            <div style={{ color:"#6b7a99", fontSize:11, marginBottom:4 }}>CO / CS NAME</div>
            <input placeholder="Joshua Taggart" value={assignName} onChange={e=>setAssignName(e.target.value)}
              style={{ ...inp, marginBottom:10 }} />
            <div style={{ color:"#6b7a99", fontSize:11, marginBottom:4 }}>CO / CS EMAIL *</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={doApprove} disabled={submitting || !assignEmail.includes("@")}
                style={{ flex:1, background: action.type === "reassign" ? "#1a3a6e" : "#0f6e56",
                  border:"none",
                  color:"#ffffff",
                  padding:"9px", borderRadius:7, cursor:"pointer", fontSize:12, fontWeight:"500",
              ...
              <button onClick={()=>setAction(null)} style={{ flex:1, background:"#f5f7fa", border:"1px solid #dde3ef",
                color:"#6b7a99", padding:"9px", borderRadius:7, cursor:"pointer", fontSize:12 }}>
                Cancel
              </button>
            </div>
          </div>
      ...
      {action?.type === "reject" && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#ffffff", border:"1px solid #7a2a2a", borderRadius:8, padding:28, width:400, maxWidth:"95vw" }}>
            ...
            <div style={{ color:"#6b7a99", fontSize:11, marginBottom:4 }}>REASON FOR RETURN *</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={doReject} disabled={submitting || !rejectReason.trim()}
                style={{ flex:1, background:"#a32d2d", border:"none", color:"#ffffff",
                  padding:"9px", borderRadius:7, cursor:"pointer", fontSize:12, fontWeight:"500",
...
function ReviewChain({ docKey, value, center }) {
  ...
  const actionColors = { A:"#2db87a", R:"#5ba3ff", C:"#854f0b", CE:"#5a3a9e", Awareness:"#7a9ab8" };
    ...
    <div style={{ marginBottom:"20px" }}>
      ...
      {chain.map((item, i) => (
        <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:"10px",marginBottom:"6px"}}>
          <div style={{ width:"20px", height:"20px", borderRadius:"50%", background:"#eef3fc", border:"1px solid "+actionColors[item.action]||"#7090b8", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"9px", color:actionColors[item.action]||"#7090b8", flexShrink:0, marginTop:"2px" }}>
            {i+1}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ display:"flex", gap:"8px", alignItems:"center", flexWrap:"wrap" }}>
              <span style={{ fontSize:"12px", color:"#1a2332", fontWeight:"600" }}>{item.role}</span>
              <span style={{ background: "#eef3fc", border:"1px solid "+actionColors[item.action]||"#7090b8", color:actionColors[item.action]||"#7090b8", padding:"1px 7px", fontSize:"9px", letterSpacing:"1px", borderRadius:"2px" }}>
...
function DownloadPDFButton({ endpoint, buildPayload, filename, label }) {
  ...
  return (
    <div style={{ flex:1 }}>
      <button
        style={{ background: status === "error" ? "#fff0f0" : "#1a3a6e", border: status === "error" ? "1px solid #f5a0a0" : "none", color: status === "error" ? "#a32d2d" : "#ffffff", padding:"10px", cursor:"pointer", borderRadius:"7px", fontSize:"12px", fontWeight:"500", width:"100%", opacity: status === "loading" ? 0.6 : 1 }}
        onClick={handleClick}
        disabled={status === "loading"}
      >
        {status === "loading" ? "Generating PDF..." : status === "error" ? "API Error — Retry" : label}
      </button>
      {status === "error" && (
        <div style={{ fontSize:"10px", color:"#7a2252", marginTop:"4px", lineHeight:1.5 }}>
          {errMsg}
          {isLocal && <span> - Start API: python api/app.py</span>}
        </div>
      )}
      {isLocal && status === "idle" && (
        <div style={{ fontSize:"9px", color:"#4a7abf", marginTop:"3px" }}>
  ...
  return (
    <div style={{ padding:"16px" }}>
      ...
      <div style={{ background:"#ffffff", border:"1px solid #1a5aaa", borderRadius:"3px", padding:"12px 14px",marginBottom:"12px"}}>
        <div style={{ fontSize:"10px", color:"#2a5aaa",marginBottom:"8px"}}>Acquisition Context</div>
        {fields.map((f,i) => f.val ? (
          <div key={i} style={{ display:"flex", gap:"12px", padding:"4px 0", borderBottom:"1px solid #061428" }}>
            ...
            <div style={{ fontSize:"10px", color:"#1a2332" }}>{f.val}</div>
          </div>
        ) : null)}
      </div>
      <div style={{ fontSize:"11px", color:"#1a2332", lineHeight:1.7,marginBottom:"14px"}}>
      ...
      <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
        <a href="https://www.nasa.gov/sites/default/files/atoms/files/nf1787.pdf" target="_blank" rel="noopener noreferrer"
          style={{ fontSize:"10px", padding:"8px 14px", borderRadius:"3px", background:"#ffffff", border:"1px solid #1a5aaa", color:"#2a5aaa", textDecoration:"none", letterSpacing:"1px" }}>
          DOWNLOAD NF 1787 PDF
        </a>
        <button onClick={() => { onSave && onSave({completed:true}); setSaved(true); }}
          style={{ fontSize:"10px", padding:"8px 14px", cursor:"pointer", borderRadius:"3px", background:"#e8f7f0", border:"1px solid #9fe1cb", color:"#2db87a", letterSpacing:"1px" }}>
...
function NF1787AForm({ intake, onSave, savedData }) {
  ...
  return (
    <div style={{ padding:"16px" }}>
      ...
      <div style={{ background:"#ffffff", border:"1px solid #1a3a6e", borderRadius:"3px", padding:"10px 14px", marginBottom:"14px", fontSize:"10px", color:"#1a2332" }}>
        <div style={{ color:"#2a5aaa", marginBottom:"6px", fontSize:"9px", letterSpacing:"1px" }}>MARKET RESEARCH SOURCES TO DOCUMENT</div>
    ...
    <div style={S.panel}>
                  ...
                  return (
                    <div key={qi} onClick={()=>setFormData(d=>({...d,[step.id+"_qc_"+qi]:!ck}))}
                      style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",
                              background:ck?"#e8f7f0":"#ffffff",border:"1px solid "+(ck?"#e8f7f0":"#eef1f6"),
                              borderRadius:3,cursor:"pointer",marginBottom:4}}>
                      <div style={{width:14,height:14,border:"1px solid "+(ck?"#0f6e56":"#6b7a99"),
                                   borderRadius:2,background:ck?"#0f6e56":"transparent",flexShrink:0,
                                   display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#fff"}}>
                        {ck?"v":""}
                      </div>
                      <span style={{fontSize:11,color:ck?"#0f6e56":"#4a5a78"}}>{item}</span>
                    </div>
                  );
                })}
              </div>
        ...
        {step.type === "FORM" && step.formType === "NAICS" && (
          <div style={S.panelSection}>
            ...
            <input placeholder="Search by code or keyword..." value={naicsSearch}
              ...
              style={{ background:"#ffffff", border:"1px solid #1a3a6e", color:"#1a2332", padding:"7px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", marginBottom:"8px", fontFamily:"IBM Plex Mono,monospace" }} />
            ...
            <input placeholder="e.g. 481211" value={naicsCustom} onChange={e=>setNaicsCustom(e.target.value)}
              style={{ background:"#ffffff", border:"1px solid " + (naicsCustom?"#1a3a6e":"#1a3a6e"), color:"#1a2332", padding:"7px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", marginBottom:"8px", fontFamily:"IBM Plex Mono,monospace" }} />
            ...
            <div style={{ fontSize:"9px", color:"#6b7a99",marginBottom:"10px"}}>Verify at sba.gov/size-standards if using a manual code</div>
        ...
        {step.type === "FORM" && step.formType === "PSC" && (
          <div style={S.panelSection}>
            ...
            <input placeholder="e.g. V231" value={pscCustom} onChange={e=>setPscCustom(e.target.value)}
              style={{ background:"#ffffff", border:"1px solid " + (pscCustom?"#1a3a6e":"#1a3a6e"), color:"#1a2332", padding:"7px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", marginBottom:"8px", fontFamily:"IBM Plex Mono,monospace" }} />
                ...
                {!docContent[curPkg.key] ? (
                  ...
                  <>
                    ...
                    <div style={{ display:"flex", gap:"8px", marginTop:"8px", flexWrap:"wrap" }}>
                      ...
                      <button style={{ ...S.copyBtn, background:"#ffffff", border:"1px solid #2a6aaa", color:"#1a3a6e" }}
              ...
              <>
                {curPkg.formType === "NF1787" ? (
                  <>
                    ...
                    {formSaved[curPkg.key] && <div style={{ marginTop:"8px", fontSize:"10px", color:"#0f6e56" }}>v Form data saved. Mark step complete when PCR response is received.</div>}
                  </>
                ) : curPkg.formType === "NF1787A" ? (
                  <>
                    <NF1787AForm intake={intake} savedData={formData[curPkg.key]}
                      onSave={(d)=>{ setFormData(fd=>({...fd,[curPkg.key]:d})); setFormSaved(fs=>({...fs,[curPkg.key]:true})); }} />
                    {formSaved[curPkg.key] && <div style={{ marginTop:"8px", fontSize:"10px", color:"#0f6e56" }}>v Form data saved.</div>}
                  </>
                ) : !docContent[curPkg.key] ? (
                  <button style={S.primaryBtn} onClick={()=>genDoc(curPkg.docType, curPkg.key)} disabled={loading[curPkg.key]}>
      ...
      {step.reviewType && (
        <div style={{ padding:"20px 24px 0", borderTop:"1px solid #1a3a6e", marginTop:"8px" }}>
          <div style={{ fontSize:"10px", color:"#1a3a6e", letterSpacing:"0.5px",marginBottom:"10px"}}>DRAFT DOCUMENT REVIEW</div>
        ...
        {items.map((t,i) => {
          ...
          return (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"5px 0", borderBottom:"1px solid #061428" }}>
              <span style={{ fontSize:"9px", color:"#8896b0", minWidth:"28px" }}>FE {t.uid}</span>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:"10px", color:"#1a2332" }}>{t.name}</div>
              ...
              {driveEntry ? (
                <a href={"https://docs.google.com/document/d/" + driveEntry.id + "/edit"}
                  ...
                  style={{ fontSize:"9px", padding:"3px 8px", borderRadius:"2px", background:"#ffffff", border:"1px solid #1a5aaa", color:"#1a3a6e", fontWeight:"600", letterSpacing:"0.5px", textDecoration:"none", whiteSpace:"nowrap" }}>
                  OPEN
                </a>
              ) : (
                <span style={{ fontSize:"9px", color:"#8896b0", padding:"3px 8px" }}>NEAR</span>
              )}
            </div>
          );
        })}
      </div>
      <div style={{ padding:"6px 12px", background:"#f5f7fa", fontSize:"9px", color:"#8896b0" }}>
...
function NearCoverSheet({ intake, docs, roadmap }) {
  ...
  return (
    <button onClick={generate}
      style={{background:"#e8f7f0",border:"1px solid #9fe1cb",color:"#0f6e56",
...
function NEARPackage({ intake, roadmap }) {
  ...
  return (
    <div style={{ padding:"20px", maxWidth:"860px", margin:"0 auto" }}>
      ...
      <div style={{ fontSize:"18px", color:"#1a2332", fontWeight:700,marginBottom:"4px"}}>{intake.reqTitle}</div>
      ...
      <div style={{ background:"#ffffff", border:"1px solid #1a3a6e", borderRadius:"4px", padding:"12px 16px",marginBottom:"20px"}}>
        <div style={{ display:"flex", justifyContent:"space-between",marginBottom:"6px"}}>
          ...
          <span style={{ fontSize:"11px", color:"#1a2332" }}>{genCount}/{expected.length}</span>
        </div>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"5px" }}>
          {expected.map(e => { const done=genTypes.has(e.docType); const m=NEAR_MAP[e.docType]||{};
            return <div key={e.docType} style={{ fontSize:"9px", padding:"2px 6px", borderRadius:"2px", background:done?"#e8f7f0":"transparent", border:"1px solid "+(done?"#2db87a":"#2255a4"), color:done?"#2db87a":"#8896b0" }}>
      ...
      {docs.length > 0 && (
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          <button onClick={dlAll}
            style={{background:"#ffffff",border:"1px solid #2a6aaa",color:"#2a5aaa",
                    padding:"7px 16px",borderRadius:4,cursor:"pointer",fontSize:11,fontWeight:"bold"}}>
            ↓ DOWNLOAD ALL (.docx)
          </button>
      ...
      {docs.length===0 ? (
        ...
        return <div key={cat} style={{ marginBottom:"14px" }}>
          ...
          {cd.map((doc,i) => <div key={i} style={{ display:"flex", alignItems:"center", gap:"10px", padding:"8px 10px", marginBottom:"4px", background:"#ffffff", border:"1px solid #1a3a6e", borderRadius:"3px" }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontSize:"10px", color:"#1a2332", fontWeight:600 }}>{doc.folder}</div>
              <div style={{ fontSize:"9px", color:"#6b7a99" }}>{doc.label||doc.docType}</div>
            </div>
            <button onClick={()=>dl(doc)} style={{ fontSize:"9px", padding:"3px 9px", cursor:"pointer", borderRadius:"2px", background:"#ffffff", border:"1px solid #1a5aaa", color:"#1a3a6e", fontWeight:"600", letterSpacing:"0.5px" }}>DL</button>
          </div>)}
        </div>;
      })}
      <div style={{ fontSize:"9px", color:"#8896b0", marginTop:"12px", fontStyle:"italic" }}>Documents saved locally. Generate from roadmap steps - each auto-files to the correct NEAR folder.</div>
    </div>
  );
}
    ...
    <div style={{ marginTop:"12px", border:`1px solid ${allOk?"#9fe1cb":nos>0?"#f5a0a0":"#dde3ef"}`, borderRadius:"8px" }}>
      <div style={{ padding:"8px 14px", display:"flex", alignItems:"center", gap:"8px", cursor:"pointer", background:allOk?"#f0faf6":nos>0?"#fff0f0":"#f5f7fa", borderRadius:"8px" }}
        ...
        <div style={{ flex:1, height:"3px", background:"#dde3ef", borderRadius:"2px" }}>
          <div style={{ width:(pct)+"%", height:"100%", background:nos>0?"#a32d2d":allOk?"#0f6e56":"#2255a4", borderRadius:"2px" }}/>
        </div>
...
function RouteAdvisor({ onUseRecommendation }) {
  ...
  const inp = { background:"#ffffff", border:"1px solid #dde3ef", color:"#1a2332", padding:"8px 12px", borderRadius:"7px", fontSize:"12px", width:"100%", fontFamily:"system-ui,sans-serif", outline:"none" };
  const btn = (active) => ({ padding:"8px 16px", cursor:"pointer", borderRadius:"7px", fontSize:"11px", border:"1px solid "+(active?"#1a3a6e":"#dde3ef"), background:active?"#1a3a6e":"#ffffff", color:active?"#ffffff":"#6b7a99", fontWeight:active?"500":"400" });
  ...
  return (
    <div style={{ padding:"24px", maxWidth:"820px", margin:"0 auto" }}>
      ...
      <div style={{ fontSize:"20px", color:"#1a2332", fontWeight:700,marginBottom:"4px"}}>Acquisition Approach Coach <span style={{fontSize:"13px",color:"#6b7a99",fontWeight:400}}>(your acquisition life coach)</span></div>
      ...
      {mode === "QUICK" ? (
        <div>
          <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>DESCRIBE THE REQUIREMENT</div>
          <div style={{ fontSize:"10px", color:"#8896b0", marginBottom:"6px", fontStyle:"italic" }}>Be specific - what is the work, who performs it, any technical details, what makes it unique.</div>
          ...
          <div style={S.grid2}>
            <div>
              <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>ESTIMATED TOTAL VALUE</div>
              <input value={dollars} onChange={e=>setDollars(e.target.value)} placeholder="e.g. 5000000" style={{ ...inp,marginBottom:"10px"}} />
            </div>
            <div>
              <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>PERIOD OF PERFORMANCE</div>
          ...
          <div style={S.grid2}>
            <div>
              <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>INCUMBENT CONTRACT?</div>
            ...
            <div>
              <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>URGENCY / NEED DATE</div>
          ...
          <div style={S.grid2}>
            <div>
              <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>EXISTING CONTRACT VEHICLE?</div>
            ...
            <div>
              <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>SMALL BUSINESS GOAL</div>
        ...
        <div style={{ marginBottom:"16px" }}>
          <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"6px",fontWeight:"500"}}>UPLOAD PACKAGE DOCUMENTS (.docx, .txt, .pdf text)</div>
          <label style={{ display:"block", border:"2px dashed #1a3a6e", borderRadius:"4px", padding:"20px", textAlign:"center", cursor:"pointer", color:"#6b7a99", fontSize:"11px" }}>
            {fileText ? <span style={{ color:"#2db87a" }}>v {fileName} - ready ({fileText.length.toLocaleString()} chars extracted)</span>
              : <span>Click to upload NF 1707, SOW, IGCE, or any package document<br/><span style={{ fontSize:"9px", color:"#8896b0" }}>Multiple documents: combine into one .txt file or upload the most complete one</span></span>}
            <input type="file" accept=".docx,.txt,.pdf,.md" style={{ display:"none" }} onChange={handleFileUpload} />
          </label>
        </div>
      )}

      {error && <div style={{ color:"#a32d2d", fontSize:"12px", marginBottom:"12px", padding:"10px 12px", background:"#fff0f0", border:"1px solid #f5a0a0", borderRadius:"7px" }}>{error}</div>}

      <button onClick={runAdvisor} disabled={loading || (mode==="QUICK"?(!desc||!dollars):(!fileText))}
        style={{ background:loading?"#eef1f6":"#1a3a6e", border:"none", color:loading?"#6b7a99":"#ffffff", padding:"11px 24px", cursor:loading?"not-allowed":"pointer", borderRadius:"8px", fontSize:"12px", fontWeight:"500", opacity:loading?0.6:1, marginBottom:"24px" }}>
        {loading ? "Analyzing requirement..." : "Analyze & Recommend"}
      </button>

      {result && result.primary && (
          <div>
            <div style={{ background:"#e8f7f0", border:"1px solid #9fe1cb", borderRadius:"4px", padding:"16px 20px", marginBottom:"12px" }}>
              <div style={{ fontSize:"9px", color:"#0f6e56", letterSpacing:"0.5px", fontWeight:"600", marginBottom:"8px" }}>Primary Recommendation</div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"10px", flexWrap:"wrap", gap:"8px" }}>
                <div>
                  <div style={{ fontSize:"16px", color:"#1a2332", fontWeight:700 }}>{result.primary.approach}</div>
                ...
                <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
                  {[["adminBurden","burden",{"LOW":"#2db87a","MEDIUM":"#854f0b","HIGH":"#a32d2d"}],["commercialDetermination","commercial",{"COMMERCIAL":"#2db87a","NON-COMMERCIAL":"#a32d2d"}]].map(([field,suffix,colors])=>{
                    const val=result.primary[field]||"";
                    const col=colors[val]||"#6b7a99";
                  ...
                  <div style={{ padding:"3px 9px", borderRadius:"3px", fontSize:"9px", letterSpacing:"1px", background:"#f5f7fa", border:"1px solid #4a9eff", color:"#2a5aaa" }}>~{result.primary.timelineWeeks}wks</div>
                </div>
              </div>
              <div style={{ fontSize:"11px", color:"#1a2332", lineHeight:1.7, padding:"10px 12px", background:"#ffffff", borderRadius:"3px", marginBottom:"10px" }}>{result.primary.whyThisPath}</div>
              <div style={S.grid2}>
                <div style={{ background:"#f5f7fa", borderRadius:"6px", padding:"9px 11px", border:"1px solid #dde3ef" }}>
                  <div style={{ fontSize:"9px", color:"#1a3a6e", fontWeight:"600", letterSpacing:"0.5px", marginBottom:"4px" }}>LEGAL AUTHORITY</div>
                  <div style={{ fontSize:"10px", color:"#1a2332", lineHeight:1.5 }}>{result.primary.farAuthority}</div>
                </div>
                <div style={{ background:"#f5f7fa", borderRadius:"6px", padding:"9px 11px", border:"1px solid #dde3ef" }}>
                  <div style={{ fontSize:"9px", color:"#1a3a6e", fontWeight:"600", letterSpacing:"0.5px", marginBottom:"4px" }}>COMMERCIAL BASIS</div>
                  <div style={{ fontSize:"10px", color:"#1a2332", lineHeight:1.5 }}>{result.primary.commercialRationale}</div>
                </div>
              </div>
              <div style={{ ...S.grid2, marginTop:"8px" }}>
                <div style={{ background:"#f5f7fa", borderRadius:"6px", padding:"9px 11px", border:"1px solid #dde3ef" }}>
                  <div style={{ fontSize:"9px", color:"#6b7a99", letterSpacing:"1px", marginBottom:"4px" }}>ADMIN BURDEN</div>
                  <div style={{ fontSize:"10px", color:"#1a2332", lineHeight:1.5 }}>{result.primary.adminBurdenExplain}</div>
                </div>
                <div style={{ background:"#f5f7fa", borderRadius:"6px", padding:"9px 11px", border:"1px solid #dde3ef" }}>
                  <div style={{ fontSize:"9px", color:"#6b7a99", letterSpacing:"1px", marginBottom:"4px" }}>TIMELINE</div>
                  <div style={{ fontSize:"10px", color:"#1a2332", lineHeight:1.5 }}>{result.primary.timelineExplain}</div>
                </div>
              </div>
              <div style={{ ...S.grid2, marginTop:"8px" }}>
                <div>
                  <div style={{ fontSize:"9px", color:"#2db87a", letterSpacing:"1px", marginBottom:"6px" }}>ADVANTAGES</div>
                  {result.primary.pros?.map((p,i)=><div key={i} style={{ fontSize:"10px", color:"#1a2332", padding:"3px 0", borderBottom:"1px solid #dde3ef" }}>+ {p}</div>)}
                </div>
                <div>
                  <div style={{ fontSize:"9px", color:"#854f0b", letterSpacing:"1px", marginBottom:"6px" }}>TRADEOFFS</div>
                  {result.primary.cons?.map((c,i)=><div key={i} style={{ fontSize:"10px", color:"#1a2332", padding:"3px 0", borderBottom:"1px solid #dde3ef" }}>- {c}</div>)}
                </div>
              </div>
              {result.primary.keyDocs?.length>0 && <div style={{ marginTop:"8px" }}>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:"5px" }}>
                  {result.primary.keyDocs.map((d,i)=><span key={i} style={{ fontSize:"9px", padding:"2px 7px", border:"1px solid #1a3a6e", borderRadius:"2px", color:"#2a5aaa" }}>{d}</span>)}
                </div>
              </div>}
              <div style={{ marginTop:"10px" }}>
                <div style={{ fontSize:"9px", color:"#6b7a99", letterSpacing:"1px", marginBottom:"5px" }}>NEXT STEPS</div>
                {result.primary.nextSteps?.map((s,i)=><div key={i} style={{ fontSize:"10px", color:"#1a2332", padding:"3px 0" }}>{i+1}. {s}</div>)}
              </div>
              <button onClick={()=>onUseRecommendation&&onUseRecommendation(result.intakeSuggestions||{},result.extractedData||{})}
                style={{ marginTop:"12px", background:"#0f6e56", border:"none", color:"#fff", padding:"10px 20px", cursor:"pointer", borderRadius:"8px", fontSize:"12px", fontWeight:"500" }}>
            ...
            {(result.whyNotFAR15||result.whyNotTM) && (
              <div style={{ background:"#ffffff", border:"1px solid #1a3a6e", borderRadius:"4px", padding:"12px 14px", marginBottom:"12px" }}>
                ...
                {result.whyNotFAR15 && <div style={{ marginBottom:"6px" }}><span style={{ fontSize:"9px", color:"#854f0b", letterSpacing:"1px" }}>FAR PART 15: </span><span style={{ fontSize:"10px", color:"#1a2332" }}>{result.whyNotFAR15}</span></div>}
                {result.whyNotTM && <div><span style={{ fontSize:"9px", color:"#854f0b", letterSpacing:"1px" }}>T&M/COST-PLUS: </span><span style={{ fontSize:"10px", color:"#1a2332" }}>{result.whyNotTM}</span></div>}
              </div>
            )}
            ...
            {result.alternative?.approach && (
              <div style={{ background:"#ffffff", border:"1px solid #1a3a6e", borderRadius:"4px", padding:"12px 14px", marginBottom:"12px" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"6px", flexWrap:"wrap", gap:"5px" }}>
                  <div>
                    <div style={{ fontSize:"13px", color:"#1a2332", fontWeight:600 }}>{result.alternative.approach}</div>
                ...
                <div style={{ fontSize:"10px", color:"#1a2332", fontStyle:"italic", marginBottom:"6px" }}>{result.alternative.whyConsider}</div>
                <div style={S.grid2}>
                  <div>{result.alternative.pros?.map((p,i)=><div key={i} style={{ fontSize:"9px", color:"#2db87a", padding:"2px 0" }}>+ {p}</div>)}</div>
                  <div>{result.alternative.cons?.map((c,i)=><div key={i} style={{ fontSize:"9px", color:"#854f0b", padding:"2px 0" }}>- {c}</div>)}</div>
                </div>
              </div>
            )}
            ...
            {result.redFlags?.length>0 && (
              <div style={{ background:"#fff8e6", border:"1px solid #cc7733", borderRadius:"4px", padding:"12px 14px" }}>
                <div style={{ fontSize:"9px", color:"#854f0b", letterSpacing:"0.5px", marginBottom:"6px" }}>! VERIFY BEFORE PROCEEDING</div>
                {result.redFlags.map((f,i)=><div key={i} style={{ fontSize:"10px", color:"#1a2332", padding:"3px 0" }}>{i+1}. {f}</div>)}
              </div>
            )}
          </div>
        )}
...
function DocumentReview({ intake, initialDocType }) {
  ...
  const SEV_COLOR = { CRITICAL:"#a32d2d", MAJOR:"#854f0b", MINOR:"#854f0b", INFO:"#5ba3ff" };
  const RATING_COLOR = { ACCEPTABLE:"#2db87a", NEEDS_WORK:"#854f0b", MAJOR_ISSUES:"#a32d2d" };

  const inp = { background:"#ffffff", border:"1px solid #dde3ef", color:"#1a2332", padding:"8px 12px", borderRadius:"7px", fontSize:"12px", width:"100%", fontFamily:"system-ui,sans-serif" };

  return (
    <div style={{ padding:"20px", maxWidth:"800px", margin:"0 auto" }}>
      <div style={{ fontSize:"10px", color:"#6b7a99", letterSpacing:"0.5px",marginBottom:"4px"}}>DOCUMENT REVIEW</div>
      <div style={{ fontSize:"18px", color:"#1a2332", fontWeight:700,marginBottom:"4px"}}>AI-Powered Acquisition Document Review</div>
      ...
      <div style={{ marginBottom:"14px" }}>
        ...
        <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
          {DOC_TYPES.map(dt => (
            <button key={dt.key}
              style={{ background: docType===dt.key ? "#1a3a6e":"#ffffff", border:"1px solid "+(docType===dt.key?"#1a3a6e":"#dde3ef"), color: docType===dt.key?"#ffffff":"#6b7a99", padding:"6px 14px", cursor:"pointer", borderRadius:"7px", fontSize:"12px", fontWeight: docType===dt.key?"500":"400" }}
      ...
      <button
        style={{ background: loading?"#eef1f6":"#1a3a6e", border:"none", color: loading?"#6b7a99":"#ffffff", padding:"10px 20px", cursor: loading||!text||extracting?"not-allowed":"pointer", borderRadius:"8px", fontSize:"12px", fontWeight:"500", opacity: loading||!text||extracting?0.6:1, marginBottom:"20px"}}
        onClick={runReview}
      ...
      {result && (
        <div style={{ animation:"slideIn .3s ease" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"12px",marginBottom:"16px"}}>
            <div style={{ fontSize:"12px", fontWeight:"500", color:RATING_COLOR[result.overallRating]||"#1a2332", background:(RATING_COLOR[result.overallRating]||"#6b7a99")+"18", border:"1px solid "+(RATING_COLOR[result.overallRating]||"#dde3ef"), padding:"4px 12px", borderRadius:"20px" }}>
              {result.overallRating?.replace("_"," ")}
            </div>
            <div style={{ fontSize:"11px", color:"#6b7a99" }}>{result.docType}</div>
          </div>

          <div style={{ fontSize:"12px", color:"#1a2332", lineHeight:1.7, marginBottom:"16px", padding:"14px", background:"#f5f7fa", border:"1px solid #dde3ef", borderRadius:"8px" }}>
            {result.summary}
          </div>
          ...
          {result.findings?.length > 0 && (
            <div>
              ...
              {result.findings.map((f, i) => (
                <div key={i} style={{ marginBottom:"10px", border:"1px solid "+SEV_COLOR[f.severity]||"#1a3a6e"+"22", borderLeft:"3px solid "+(SEV_COLOR[f.severity]||"#1a3a6e"), borderRadius:"3px", padding:"10px 12px", background:"#ffffff" }}>
                  <div style={{ display:"flex", gap:"8px", alignItems:"center",marginBottom:"6px"}}>
                    ...
                    <span style={{ fontSize:"10px", color:"#1a3a6e", fontWeight:"600", letterSpacing:"0.5px" }}>{f.category}</span>
                  </div>
                  <div style={{ fontSize:"11px", color:"#1a2332", marginBottom:"6px", lineHeight:1.6 }}>{f.issue}</div>
                  <div style={{ fontSize:"10px", color:"#2db87a", lineHeight:1.5 }}>
                    <span style={{ color:"#0f6e56" }}>RECOMMENDATION: </span>{f.recommendation}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{background:"#ffffff",border:"1px solid #2a4a7a",borderRadius:8,width:500,maxWidth:"95vw",maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
            <div style={{padding:"14px 18px",borderBottom:"0.5px solid #dde3ef",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                {result && <span style={{color: passCount===total?"#0f6e56":"#854f0b", fontSize:11, marginLeft:10}}>{passCount}/{total} passed</span>}
              </div>
              ...
              {result && result.map((r,i) => (
                <div key={i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:"1px solid #0d1f3c"}}>
                  <div style={{width:18,height:18,borderRadius:"50%",flexShrink:0,marginTop:2,
                                background:r.pass?"#0f6e56":"#a32d2d",
                                display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"#fff"}}>
                    {r.pass?"✓":"✗"}
                  </div>
                  <div>
                    <div style={{color:r.pass?"#0f6e56":"#a32d2d",fontSize:11,fontWeight:"bold"}}>{r.criterion}</div>
                    {r.note && <div style={{color:"#6b7a99",fontSize:10,marginTop:2}}>{r.note}</div>}
                  </div>
                </div>
              ))}
            </div>
...
function SubmitButton({ docType, content, intake }) {
  ...
  if (submitted) return (
    <div style={{background:"#e8f7f0",border:"1px solid #9fe1cb",borderRadius:6,
                 padding:"5px 12px",fontSize:11,color:"#0f6e56",display:"inline-block"}}>
  ...
  return (
    <>
      <button style={{background:"#e8f7f0",border:"1px solid #9fe1cb",color:"#0f6e56",
                      padding:"5px 14px",borderRadius:4,cursor:"pointer",fontSize:11,fontWeight:"bold"}}
...
function SubmitModal({ docType, content, intake, onClose, onSubmitted }) {
  ...
  const inp = (ph, key) => (
    <input placeholder={ph} value={roles[key]||""}
      ...
      style={{width:"100%",background:"#ffffff",border:"1px solid #dde3ef",color:"#1a2332",
              padding:"7px 10px",borderRadius:4,fontSize:12,boxSizing:"border-box",marginBottom:8}} />
  );

  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#ffffff",border:"0.5px solid #dde3ef",borderRadius:12,padding:28,boxShadow:"0 8px 40px rgba(26,58,110,0.12)",width:420,maxWidth:"95vw"}}>
        <div style={{color:"#1a2332",fontWeight:"600",fontSize:14,marginBottom:4}}>Submit for Review</div>
        <div style={{color:"#6b7a99",fontSize:11,marginBottom:20}}>{docType} — {intake?.reqTitle}</div>

        <div style={{color:"#4a5a78",fontSize:11,fontWeight:"500",marginBottom:4}}>Contracting Officer Email *</div>
        {inp("co@nasa.gov","co")}
        <div style={{color:"#4a5a78",fontSize:11,fontWeight:"500",marginBottom:4}}>Tech Rep Email *</div>
        {inp("techrep@nasa.gov","techRep")}
        {needsCA && (<>
          <div style={{color:"#4a5a78",fontSize:11,fontWeight:"500",marginBottom:4}}>Competition Advocate Email</div>
          {inp("ca@nasa.gov","ca")}
        </>)}
        {needsHCA && (<>
          <div style={{color:"#4a5a78",fontSize:11,fontWeight:"500",marginBottom:4}}>Head of Contracting Activity Email</div>
          {inp("hca@nasa.gov","hca")}
        </>)}
        {docType!=="JOFOC" && docType!=="ACQ_PLAN" && (<>
          <div style={{color:"#4a5a78",fontSize:11,fontWeight:"500",marginBottom:4}}>Branch Chief / Supervisor Email</div>
        ...
        <div style={{display:"flex",gap:8,marginTop:4}}>
          <button onClick={submit} disabled={submitting}
            style={{flex:1,background:"#1a3a6e",border:"none",color:"#ffffff",
                    padding:"9px",borderRadius:4,cursor:"pointer",fontSize:12,fontWeight:"bold"}}>
            {submitting ? "SUBMITTING..." : "SUBMIT"}
          </button>
          <button onClick={onClose}
            style={{flex:1,background:"#ffffff",border:"1px solid #dde3ef",color:"#6b7a99",
  ...
  return (
    <div style={{display:"flex",flexWrap:"wrap",gap:6,margin:"12px 0"}}>
      {chain.map((step,i) => {
        ...
        const color = step.status==="approved" ? "#0f6e56"
                    : step.status==="returned"  ? "#a32d2d"
                    : isCurrent                 ? "#4a7abf"
                    : "#333";
        return (
...
function ReviewerDashboard({ onClose }) {
  ...
  const row = (sub) => {
    ...
    return (
      <div key={sub.id} onClick={()=>setSelected(sub===selected?null:sub)}
        style={{background: selected===sub?"#eef3fc":"#fafbfd",border:"1px solid "+(isMyTurn?"#4a7abf":"#dde3ef"),
                borderRadius:6,padding:"12px 14px",marginBottom:8,cursor:"pointer"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <span style={{color:"#1a3a6e",fontSize:11,fontWeight:"bold"}}>{sub.docType}</span>
            <span style={{color:"#6b7a99",fontSize:10,marginLeft:8}}>{sub.center}</span>
            {isMyTurn && <span style={{background:"#1a3a6e",color:"#1a3a6e",fontSize:9,padding:"1px 6px",borderRadius:10,marginLeft:8}}>YOUR TURN</span>}
          </div>
        ...
        <div style={{color:"#1a2332",fontSize:12,marginTop:4}}>{sub.title}</div>
        ...
        {selected===sub && (
          <div style={{marginTop:12,borderTop:"1px solid #dde3ef",paddingTop:12}}>
            <ChainView chain={sub.chain} currentStep={sub.currentStep} />

            {sub.history?.length > 0 && (
              <div style={{marginTop:8}}>
                <div style={{color:"#6b7a99",fontSize:10,marginBottom:4}}>HISTORY</div>
                {sub.history.map((h,i)=>(
                  <div key={i} style={{color:"#8896b0",fontSize:10,marginBottom:2}}>
                    {new Date(h.at).toLocaleString()} — <span style={{color:"#4a5a78"}}>{h.action.toUpperCase()}</span> by {h.role||h.by}
                    {h.comments && <span style={{color:"#854f0b"}}> — "{h.comments}"</span>}
                  </div>
                ))}
              </div>
            )}
            ...
            {isMyTurn && sub.status==="in_review" && (
              <div style={{marginTop:12,display:"flex",gap:8}}>
                <button onClick={e=>{e.stopPropagation();setAction({id:sub.id,type:"approve"});}}
                  style={{background:"#e8f7f0",border:"1px solid #1a7a3a",color:"#0f6e56",
                          padding:"7px 16px",borderRadius:4,cursor:"pointer",fontSize:11,fontWeight:"bold"}}>
                  ✓ APPROVE
                </button>
                <button onClick={e=>{e.stopPropagation();setAction({id:sub.id,type:"return"});}}
                  style={{background:"#fff0f0",border:"1px solid #f5a0a0",color:"#a32d2d",
                          padding:"7px 16px",borderRadius:4,cursor:"pointer",fontSize:11,fontWeight:"bold"}}>
                  ✗ RETURN
                </button>
              </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:998,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#ffffff",border:"1px solid #2a4a7a",borderRadius:8,width:560,maxWidth:"95vw",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"16px 20px",borderBottom:"0.5px solid #dde3ef",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{color:"#1a3a6e",fontWeight:"bold",fontSize:13}}>REVIEW DASHBOARD</div>
            {email && <div style={{color:"#6b7a99",fontSize:10,marginTop:2}}>{email}</div>}
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        <div style={{flex:1,overflow:"auto",padding:20}}>
          {!email ? (
            <div>
              <div style={{color:"#4a5a78",fontSize:12,marginBottom:12}}>Enter your NASA email to see pending reviews and submissions.</div>
              <input placeholder="your.name@nasa.gov" value={emailInput}
                onChange={e=>setEmailInput(e.target.value)}
                onKeyDown={e=>e.key==="Enter"&&login()}
                style={{width:"100%",background:"#ffffff",border:"1px solid #2a4a7a",color:"#1a2332",
                        padding:"9px 12px",borderRadius:4,fontSize:12,boxSizing:"border-box",marginBottom:10}} />
              <button onClick={login}
                style={{width:"100%",background:"#1a3a6e",border:"none",color:"#ffffff",
                        padding:"9px",borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:"500"}}>
                View My Queue
              </button>
            </div>
        ...
        {action && (
          <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:8}}>
            <div style={{background:"#ffffff",border:"1px solid #2a4a7a",borderRadius:8,padding:24,width:360}}>
              <div style={{color: action.type==="approve"?"#0f6e56":"#a32d2d",fontWeight:"bold",fontSize:13,marginBottom:12}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={doAction} disabled={acting||(action.type==="return"&&!comments.trim())}
                  style={{flex:1,background:action.type==="approve"?"#e8f7f0":"#fff0f0",
                          border:"1px solid "+(action.type==="approve"?"#9fe1cb":"#f5a0a0"),
                          color:action.type==="approve"?"#0f6e56":"#a32d2d",
                          padding:"9px",borderRadius:4,cursor:"pointer",fontSize:12,fontWeight:"bold",
                ...
                <button onClick={()=>{setAction(null);setComments("");}}
                  style={{flex:1,background:"#ffffff",border:"1px solid #2a4a7a",color:"#6b7a99",
                          padding:"9px",borderRadius:4,cursor:"pointer",fontSize:12}}>
                  CANCEL
                </button>
              </div>
...
function ToolsMenu({ onMod, onForms, onBPA, onUCF, onAward, onAdmin, onCompliance, onPreSol, onReview, onRequestor, onTechEval, onBranchChief, onHQDashboard, onContractRecord, onMyAcquisitions, onClauseMatrix, onDataRights, onAttachments, onRegSearch, onChecklist, onOfficeReadiness }) {
  ...
  const groups = {
    workflow: [
      { label:"My Acquisitions",            action: onMyAcquisitions },
      ...
      { label:"Contract Records",           action: onContractRecord },
    ...
    tools: [
      ...
      { label:"Clause Matrix & Solicitation", action: onClauseMatrix },
      { label:"Data Rights Attachment",        action: onDataRights },
      { label:"Attachments Builder",           action: onAttachments },
      { label:"Regulatory Search",             action: onRegSearch },
...
function CPAS() {
  ...
  const [acquisitionId, setAcquisitionId] = useState(() => localStorage.getItem("cpas_acq_id") || null);
  ...
  const [showMyAcquisitions, setShowMyAcquisitions] = useState(false);
  const [saveAsModal, setSaveAsModal] = useState(null); // { data, rm } when pending decision
  const [awardModal, setAwardModal] = useState(false);
  const [showClauseMatrix, setShowClauseMatrix] = useState(false);
  ...
  async function sbAcqUpsert(acqId, patch) {
    try {
      const method = acqId ? "PATCH" : "POST";
      const url = acqId
        ? `${SB_URL}/rest/v1/cpas_acquisitions?id=eq.${acqId}`
        : `${SB_URL}/rest/v1/cpas_acquisitions`;
      const res = await fetch(url, {
        method,
        headers: {
          "Content-Type": "application/json",
  ...
  function save(ni, nc) {
    const ni2 = ni || intake;
    const nc2 = nc || completedSteps;
    try {
      localStorage.setItem("cpas_v5_state", JSON.stringify({ intake: ni2, cs: Array.from(nc2) }));
    } catch(e) {}
    // Sync to Supabase if we have an acquisition record
    const acqId = acquisitionId || localStorage.getItem("cpas_acq_id");
    if (acqId && ni2) {
      sbAcqUpsert(acqId, {
        intake_data: ni2,
        completed_steps: Array.from(nc2),
        title: ni2.reqTitle || "",
        value: parseFloat(ni2.value) || null,
        contract_type: ni2.contractType || "",
        competition: (ni2.competitionStrategy || "").replace(/_/g, " "),
        naics: ni2.naics || "",
        status: nc2.size > 0 ? "In Progress" : "Draft",
      }).catch(() => {});
    }
  }
  ...
  function onIntakeComplete(data) {
    ...
    // If an acquisition is already loaded, ask whether to update it or start fresh
    if (acquisitionId) {
      setSaveAsModal({ data, rm });
      return;
    }
    applyIntakeComplete(data, rm, null);
  }

  function applyIntakeComplete(data, rm, existingAcqId) {
    setIntake(data); setRoadmap(rm); setScreen("ROADMAP"); save(data, new Set());

    // Create or update acquisition record in Supabase
    sbAcqUpsert(existingAcqId, {
      co_email: (data.coEmail || "").toLowerCase(),
      co_name: data.coName || "",
      center: data.center || "",
      title: data.reqTitle || "",
      status: "Draft",
      ...
      if (row?.id) {
        ...
        localStorage.setItem("cpas_acq_id", row.id);
      }
    }).catch(() => {});

    // Also create Pre-Award contract record (existing behavior)
    fetch(`${SB_URL}/rest/v1/cpas_contract_records`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "apikey": SB_KEY,
      ...
      body: JSON.stringify({
        ...
        co_name: data.coName || "",
        co_email: (data.coEmail || "").toLowerCase(),
        cor_name: data.techRepName || "",
        cor_email: data.techRepEmail || "",
        center: data.center || "",
        branch: "",
        contract_type: data.contractType || "",
        competition: (data.competitionStrategy || "").replace(/_/g, " "),
        description: data.reqTitle || "",
        naics: data.naics || "",
        psc: data.psc || "",
        current_value: parseFloat(data.value) || null,
        potential_value: parseFloat(data.value) || null,
        over_10m: parseFloat(data.value || 0) >= 10000000,
        intake_data: data,
      }),
    }).catch(() => {});
  }
  ...
  function startOver() {
    localStorage.removeItem("cpas_v5_state");
    localStorage.removeItem("cpas_acq_id");
  ...
  return (
      ...
      <div style={S.hdr}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
          ...
          <ToolsMenu
            ...
            onContractRecord={()=>setShowContractRecord(true)}
            onMyAcquisitions={()=>setShowMyAcquisitions(true)}
            onClauseMatrix={()=>setShowClauseMatrix(true)}
                ...
                <div style={{ background:"#e8f7f0", border:"1px solid #9fe1cb", borderRadius:8,
                  ...
                  {pending.map(p => (
                    <div key={p.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                      ...
                      <div>
                        <div style={{ fontSize:12, color:"#1a2332", fontWeight:600 }}>
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        ...
                        }} style={{ background:"none", border:"1px solid #2a4a3a", color:"#0f6e56",
        ...
        <div style={{ ...S.main, paddingRight: activeStep ? "604px" : "24px" }}>
          {(() => {
            try {
              const pending = pendingPackages;
              if (pending.length > 0) return (
                <div style={{ background:"#e8f7f0", border:"1px solid #9fe1cb", borderRadius:4,
                  ...
                  {pending.map(p => (
                    <div key={p.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                                             fontSize:11, color:"#1a2332", marginBottom:4 }}>
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        ...
                        }} style={{ background:"none", border:"1px solid #3a5a4a", color:"#6b7a99",
    ...
    {saveAsModal && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#fff",borderRadius:12,padding:"28px 32px",maxWidth:480,width:"90%",boxShadow:"0 8px 40px rgba(26,58,110,0.18)"}}>
          <div style={{fontSize:15,fontWeight:"600",color:"#1a2332",marginBottom:8}}>Save Acquisition</div>
          <div style={{fontSize:12,color:"#6b7a99",marginBottom:24,lineHeight:1.6}}>
            You have an existing acquisition open: <strong style={{color:"#1a2332"}}>{intake?.reqTitle || "Current acquisition"}</strong>.
            Would you like to update it with the new parameters, or save this as a new separate acquisition?
          </div>
          <div style={{display:"flex",gap:10,flexDirection:"column"}}>
            <button onClick={()=>{
              applyIntakeComplete(saveAsModal.data, saveAsModal.rm, acquisitionId);
              setSaveAsModal(null);
            }} style={{background:"#1a3a6e",border:"none",color:"#fff",padding:"11px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:"500",fontFamily:"-apple-system,sans-serif"}}>
              Update existing acquisition
            </button>
            <button onClick={()=>{
              const prevId = acquisitionId;
              setAcquisitionId(null);
              localStorage.removeItem("cpas_acq_id");
              applyIntakeComplete(saveAsModal.data, saveAsModal.rm, null);
              setSaveAsModal(null);
            }} style={{background:"#0f6e56",border:"none",color:"#fff",padding:"11px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:"500",fontFamily:"-apple-system,sans-serif"}}>
              Save as new acquisition
            </button>
            <button onClick={()=>setSaveAsModal(null)}
              style={{background:"none",border:"1px solid #dde3ef",color:"#6b7a99",padding:"9px",borderRadius:8,cursor:"pointer",fontSize:12,fontFamily:"-apple-system,sans-serif"}}>
              Cancel — go back
            </button>
          </div>
        </div>
    ...
    {awardModal && (
      <AwardRecordModal
        ...
        onSaved={(contractNumber)=>{
          const updated = { ...intake, contractNumber };
          setIntake(updated); save(updated, completedSteps);
          if (acquisitionId) sbAcqUpsert(acquisitionId, { status:"Awarded", contract_number: contractNumber });
          setAwardModal(false);
        }}
      />
    )}
    {showAttachments && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:990,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}}>
        <div style={{background:"#f5f7fa",width:"min(1300px,100vw)",height:"100%",display:"flex",flexDirection:"column",boxShadow:"-4px 0 24px rgba(26,58,110,0.12)"}}>
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
            <AttachmentsBuilder
              existingIntake={intake}
              onSaveToAcquisition={(docs) => {
                if (!intake) return;
                const sk = "cpas_docs_" + (intake.reqTitle || "x");
                try {
                  const ex = JSON.parse(localStorage.getItem(sk) || "[]");
                  const ATT_LABELS = {QASP:"QASP",CDRL:"CDRL/DRL",MILESTONES:"Payment Milestones",SUBK:"Subcontracting Plan",KEYPERSONNEL:"Key Personnel","508":"Section 508",CMR:"CMR",WD:"Wage Determination Cover",NDA:"NDA",PPQ:"PPQ",TRAVEL:"Travel Policy",SECURITY:"Security Requirements",GFP:"GFP List",OCI:"OCI Mitigation Plan"};
                  const entries = Object.entries(docs).map(([docType, content]) => ({
                    docType: "ATTACHMENT_" + docType,
                    label: "Attachment — " + (ATT_LABELS[docType] || docType),
                    content, ts: Date.now()
                  }));
                  const filtered = ex.filter(d => !entries.some(e => e.docType === d.docType));
                  localStorage.setItem(sk, JSON.stringify([...filtered, ...entries]));
                } catch(e) {}
                alert(Object.keys(docs).length + " attachment(s) saved to acquisition package.");
              }}
    ...
    {showDataRights && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:990,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}}>
        <div style={{background:"#f5f7fa",width:"min(1100px,100vw)",height:"100%",display:"flex",flexDirection:"column",boxShadow:"-4px 0 24px rgba(26,58,110,0.12)"}}>
          <div style={{background:"#fff",borderBottom:"1px solid #dde3ef",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            <div>
              <div style={{fontWeight:"600",fontSize:14,color:"#1a2332"}}>Data Rights Attachment Generator</div>
              <div style={{fontSize:11,color:"#6b7a99"}}>NFS 1827 / FAR 27 — Section J attachment and clause prescription</div>
            </div>
            <button onClick={()=>setShowDataRights(false)} style={{background:"none",border:"none",color:"#6b7a99",fontSize:22,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
            <DataRightsAttachment
              existingIntake={intake}
              onSaveToAcquisition={(generated) => {
                ...
                try {
                  const ex = JSON.parse(localStorage.getItem(sk) || "[]");
                  const entries = [
                    { docType:"DATA_RIGHTS_ATTACHMENT", label:"Data Rights Attachment (Section J)", content: generated.attachment, ts: Date.now() },
                    { docType:"DATA_RIGHTS_CLAUSES", label:"Data Rights Clause Prescription", content: JSON.stringify(generated.prescription), ts: Date.now() },
                  ];
                  localStorage.setItem(sk, JSON.stringify([
                    ...ex.filter(d => d.docType !== "DATA_RIGHTS_ATTACHMENT" && d.docType !== "DATA_RIGHTS_CLAUSES"),
                    ...entries
                  ]));
                } catch(e) {}
                setShowDataRights(false);
                alert("Data Rights Attachment saved to acquisition package.");
              }}
    ...
    {showClauseMatrix && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:990,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}}>
        <div style={{background:"#f5f7fa",width:"min(1200px,100vw)",height:"100%",display:"flex",flexDirection:"column",boxShadow:"-4px 0 24px rgba(26,58,110,0.12)"}}>
          <div style={{background:"#fff",borderBottom:"1px solid #dde3ef",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            <div>
              <div style={{fontWeight:"600",fontSize:14,color:"#1a2332"}}>Clause Matrix & Solicitation Drafting</div>
              <div style={{fontSize:11,color:"#6b7a99"}}>Standalone — generates clause list and full UCF solicitation draft</div>
            </div>
            <button onClick={()=>setShowClauseMatrix(false)} style={{background:"none",border:"none",color:"#6b7a99",fontSize:22,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
            <StandaloneClauseMatrix
              existingIntake={intake}
              onSaveToAcquisition={(prescribed, ucf) => {
                if (!intake) return;
                const sk = "cpas_docs_" + (intake.reqTitle || "x");
                try {
                  const ex = JSON.parse(localStorage.getItem(sk) || "[]");
                  const entries = [];
                  if (prescribed) entries.push({ docType:"CLAUSE_LIST", label:"Clause Recommendation List", content: JSON.stringify(prescribed), ts: Date.now() });
                  if (ucf) entries.push({ docType:"SOLICITATION_DRAFT", label:"Full Solicitation Draft (UCF)", content: JSON.stringify(ucf), ts: Date.now() });
                  localStorage.setItem(sk, JSON.stringify([...ex.filter(d => d.docType !== "CLAUSE_LIST" && d.docType !== "SOLICITATION_DRAFT"), ...entries]));
                } catch(e) {}
                setShowClauseMatrix(false);
                alert("Saved to acquisition package.");
              }}
              onClose={()=>setShowClauseMatrix(false)}
            />
          </div>
        </div>
      </div>
    ...
    {showRegSearch && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:990,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}}>
        <div style={{background:"#f5f7fa",width:"min(1100px,100vw)",height:"100%",display:"flex",flexDirection:"column",boxShadow:"-4px 0 24px rgba(26,58,110,0.12)"}}>
          <div style={{flex:1,overflow:"auto"}}>
            <RegulatorySearch onClose={()=>setShowRegSearch(false)} />
          </div>
        </div>
      </div>
    )}
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:990,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}}>
        <div style={{background:"#f5f7fa",width:"min(1100px,100vw)",height:"100%",display:"flex",flexDirection:"column",boxShadow:"-4px 0 24px rgba(26,58,110,0.12)"}}>
          ...
          <div style={{flex:1,overflow:"auto"}}>
            <MyAcquisitions
              onNew={() => {
                setShowMyAcquisitions(false);
                startOver();
              }}
              onOpen={(acq) => {
                // Load saved acquisition into CPAS state
                if (!acq.intake_data) return;
                const intake = acq.intake_data;
                const completed = new Set(Array.isArray(acq.completed_steps) ? acq.completed_steps : []);
                const rm = buildRoadmap(intake);
                setIntake(intake);
                setRoadmap(rm);
                setCompletedSteps(completed);
                setAcquisitionId(acq.id);
                localStorage.setItem("cpas_acq_id", acq.id);
                localStorage.setItem("cpas_v5_state", JSON.stringify({
                  intake, cs: Array.from(completed)
                }));
                setScreen("ROADMAP");
                setShowMyAcquisitions(false);
              }}
    ...
    {showContractRecord && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.25)",zIndex:990,display:"flex",alignItems:"stretch",justifyContent:"flex-end"}}>
        <div style={{background:"#f5f7fa",width:"min(1100px,100vw)",height:"100%",display:"flex",flexDirection:"column",boxShadow:"-4px 0 24px rgba(26,58,110,0.12)"}}>
          <div style={{background:"#fff",borderBottom:"1px solid #dde3ef",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
            <div>
              <div style={{fontWeight:"600",fontSize:14,color:"#1a2332"}}>Contract Records</div>
              <div style={{fontSize:11,color:"#6b7a99"}}>Post-award lifecycle — Supabase</div>
            </div>
            <button onClick={()=>setShowContractRecord(false)} style={{background:"none",border:"none",color:"#6b7a99",fontSize:22,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
            <ContractRecord intake={intake} onClose={()=>setShowContractRecord(false)} />
          </div>
        </div>
      </div>
    )}
      ...
      <div style={{position:"fixed",inset:0,background:"#ffffff",zIndex:990,overflow:"auto"}}>
        <RequestorPortal
          ...
          onSubmit={async (pkg) => {
            ...
            // Save to localStorage for CO pickup (existing behavior)
            try {
              ...
              setPendingPackages(p => [...p, newPkg]);
            } catch(e) { console.error("Failed to save package:", e); }

            // Also write to Supabase packages table and create submission status row
            try {
              const pkgRes = await fetch(`${SB_URL}/rest/v1/cpas_packages`, {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
                  "apikey": SB_KEY,
                ...
                body: JSON.stringify({
                  ...
                  cor_email: (pkg.reqEmail || "").toLowerCase(),
              ...
              if (pkgRes.ok) {
                const [pkgRow] = await pkgRes.json();
                if (pkgRow?.id) {
                  // Create submission status row for COR tracking
                  await fetch(`${SB_URL}/rest/v1/cpas_submission_status`, {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
                      "apikey": SB_KEY,
            ...
            } catch(e) { /* silent — localStorage copy already saved */ }
    ...
    {showMod && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:997,display:"flex",alignItems:"center",justifyContent:"center"}}>
    ...
    {showForms && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:996,display:"flex",alignItems:"center",justifyContent:"center"}}>
    ...
    {showBPA && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:995,display:"flex",alignItems:"center",justifyContent:"center"}}>
    ...
    {showPreSol && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:991,display:"flex",alignItems:"center",justifyContent:"center"}}>
    ...
    {showCompliance && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:992,display:"flex",alignItems:"center",justifyContent:"center"}}>
    ...
    {showAwardTools && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:993,display:"flex",alignItems:"center",justifyContent:"center"}}>
    ...
    {showUCF && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:994,display:"flex",alignItems:"center",justifyContent:"center"}}>
