...
function BranchChiefDashboard({ onClose }) {
  ...
  async function doApprove() {
    ...
    setSubmitting(true);
    try {
      await fetch(`${PKG_API}/approve/${action.pkg.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ approvedBy: email, assignedToName: assignName, assignedToEmail: assignEmail }),
      });
      setSuccessMsg(`Package assigned to ${assignName || assignEmail}. Email notification sent.`);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:997, display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      {action?.type === "approve" && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.7)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#0a1a3a", border:"1px solid #2a6a3a", borderRadius:8, padding:28, width:400, maxWidth:"95vw" }}>
            <div style={{ color:"#3aaa66", fontWeight:"bold", fontSize:14, marginBottom:4 }}>APPROVE & ASSIGN</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={doApprove} disabled={submitting || !assignEmail.includes("@")}
                style={{ flex:1, background:"#041a0e", border:"1px solid #3aaa66", color:"#3aaa66",
                  padding:"9px", borderRadius:4, cursor:"pointer", fontSize:12, fontWeight:"bold",
