                    ...
                    return (
                      <div key={i} style={{ background:"#eef1f6", border:"1px solid #1a3a6e", borderRadius:6,
                        ...
                        <div style={{ display:"flex", gap:16, fontSize:11 }}>
                          ...
                          <span><span style={{ color:"#6b7a99" }}>Pending: </span><span style={{ color:"#ffaa30" }}>{centerPending}</span></span>
                          <span><span style={{ color:"#6b7a99" }}>In Progress: </span><span style={{ color:"#0f6e56" }}>{centerInProgress}</span></span>
                        </div>
                        {/* Mini bar */}
                        <div style={{ marginTop:10, background:"#1a2d45", borderRadius:3, height:6, overflow:"hidden" }}>
...
function BranchChiefDashboard({ onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:997, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#152236", border:"1px solid #2a4a7a", borderRadius:8, width:700, maxWidth:"96vw", maxHeight:"92vh", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding:"16px 20px", borderBottom:"1px solid #1a2a4a", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
          <div>
            ...
            <div style={{ color:"#6080a0", fontSize:10, marginTop:2 }}>Review and assign acquisition packages to Contracting Officers</div>
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            ...
            <button onClick={onClose} style={{ background:"none", border:"none", color:"#6080a0", fontSize:20, cursor:"pointer" }}>×</button>
          </div>
        </div>
        ...
        <div style={{ flex:1, overflow:"auto", padding:20 }}>
          ...
          {!email ? (
            <div>
              ...
              <button onClick={login} style={{ width:"100%", background:"#1a4a8a", border:"1px solid #2a6aaa", color:"#5ba3ff",
                padding:"9px", borderRadius:4, cursor:"pointer", fontSize:12, fontWeight:"bold" }}>
                VIEW QUEUE
              </button>
            </div>
          ) : loading ? (
            <div style={{ color:"#6080a0", textAlign:"center", padding:40 }}>Loading packages...</div>
          ) : (
            <div>
              {successMsg && (
                <div style={{ background:"#041a0e", border:"1px solid #3aaa66", borderRadius:4, padding:"10px 14px", marginBottom:14, color:"#2db87a", fontSize:11 }}>
                  ✓ {successMsg}
              ...
              <div style={{ display:"flex", gap:12, marginBottom:16, flexWrap:"wrap" }}>
                {[
                  ...
                  <div key={s.label} style={{ background:"#152236", border:`1px solid ${s.color}44`, borderRadius:4, padding:"8px 14px" }}>
              ...
              {packages.length === 0 ? (
                <div style={{ color:"#6080a0", textAlign:"center", padding:40, border:"1px dashed #1a3a6e", borderRadius:4 }}>
                ...
                <div key={pkg.id} style={{ background: selected===pkg.id?"#1a3a5e":"#152236", border:`1px solid ${selected===pkg.id?"#5ba3ff":"#2255a4"}`,
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:6 }}>
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ color:"#7090b8", fontSize:10, marginTop:3 }}>
                  ...
                  {selected === pkg.id && (
                    <div style={{ marginTop:12, borderTop:"1px solid #1a3a6e", paddingTop:12 }}>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12, fontSize:11 }}>
                        <div><span style={{ color:"#7090b8" }}>Submitted: </span><span style={{ color:"#e8eef8" }}>{new Date(pkg.submitted_at||pkg.submittedAt).toLocaleDateString()}</span></div>
                        <div><span style={{ color:"#7090b8" }}>COR Email: </span><span style={{ color:"#e8eef8" }}>{pkg.cor_email||pkg.corEmail}</span></div>
                        {(pkg.package_data?.sowObjective||pkg.packageData?.sowObjective) && (
                          <div style={{ gridColumn:"1/-1" }}>
                            <span style={{ color:"#7090b8" }}>Objective: </span>
                        ...
                        {(pkg.assigned_to_name||pkg.assignedToName) && (
                          <div><span style={{ color:"#7090b8" }}>Assigned to: </span><span style={{ color:"#2db87a", fontWeight:"bold" }}>{pkg.assigned_to_name||pkg.assignedToName}</span></div>
                        )}
                      ...
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        <button onClick={e=>{e.stopPropagation();setViewPkg(pkg);}}
                          style={{ background:"#1a2d45", border:"1px solid #4a7aaa", color:"#a0b8d8",
                        ...
                        {pkg.status === "PENDING" && (<>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"approve",pkg});}}
                            style={{ background:"#041a0e", border:"1px solid #3aaa66", color:"#2db87a",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ✓ APPROVE & ASSIGN
                          </button>
                        ...
                        {pkg.status === "ASSIGNED" && (<>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reassign",pkg});}}
                            style={{ background:"#1a2d45", border:"1px solid #4a9eff", color:"#5ba3ff",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ↺ REASSIGN
                          </button>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reject",pkg});}}
              ...
              {bcTab === "active" && (
                <div>
                  {activePackages.length === 0 ? (
                    ...
                    return (
                      <div key={i} style={{ background:"#152236", border:"1px solid #1a3a6e", borderRadius:6, padding:"12px 16px", marginBottom:8 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
                          <div style={{ flex:1 }}>
                            <div style={{ color:"#e8eef8", fontWeight:"bold", fontSize:13 }}>{pkg.req_title}</div>
                            <div style={{ color:"#7090b8", fontSize:10, marginTop:3 }}>{pkg.center} · Submitted by {pkg.cor_name}</div>
                          </div>
                          <div style={{ textAlign:"right" }}>
                            <div style={{ color:"#2db87a", fontWeight:"bold", fontSize:12 }}>IN PROGRESS</div>
                            <div style={{ color:"#7090b8", fontSize:10 }}>${(parseFloat(pkg.value)||0).toLocaleString()}</div>
                          </div>
                        </div>
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginTop:10, fontSize:11 }}>
                          <div style={{ background:"#1a2d45", borderRadius:4, padding:"8px 10px" }}>
                            <div style={{ color:"#7090b8", fontSize:9, marginBottom:2 }}>ASSIGNED CO</div>
                            <div style={{ color:"#e8eef8", fontWeight:"bold" }}>{pkg.assigned_to_name||"—"}</div>
                            <div style={{ color:"#6080a0", fontSize:10 }}>{pkg.assigned_to_email||""}</div>
                          </div>
                          <div style={{ background:"#1a2d45", borderRadius:4, padding:"8px 10px" }}>
                            <div style={{ color:"#7090b8", fontSize:9, marginBottom:2 }}>DAYS SINCE ASSIGNMENT</div>
                          ...
                          <div style={{ background:"#1a2d45", borderRadius:4, padding:"8px 10px" }}>
                            <div style={{ color:"#7090b8", fontSize:9, marginBottom:2 }}>DAYS SINCE STARTED</div>
      ...
      {viewPkg && (() => {
        ...
        return (
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:10 }}>
            <div style={{ background:"#152236", border:"1px solid #2a4a7a", borderRadius:8, width:680, maxWidth:"96vw",
              maxHeight:"90vh", display:"flex", flexDirection:"column" }}>
              <div style={{ padding:"16px 20px", borderBottom:"1px solid #1a2a4a", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
                <div>
                  <div style={{ color:"#e8eef8", fontWeight:"bold", fontSize:14 }}>{viewPkg.req_title || p.reqtitle}</div>
                  <div style={{ color:"#6080a0", fontSize:11, marginTop:2 }}>
                ...
                <button onClick={()=>setViewPkg(null)} style={{ background:"none", border:"none", color:"#6080a0", fontSize:24, cursor:"pointer" }}>×</button>
              </div>
              ...
              <div style={{ flex:1, overflow:"auto", padding:20 }}>
                ...
                {p.tasks && p.tasks.length > 0 && (
                  <div style={{ marginBottom:10 }}>
                    ...
                    {p.tasks.map((t,i) => (
                      <div key={i} style={{ background:"#152236", border:"1px solid #1a3a6e", borderRadius:4, padding:"8px 12px", marginBottom:6 }}>
                        <div style={{ color:"#e8eef8", fontWeight:"bold", fontSize:12 }}>{t.title}</div>
                        <div style={{ color:"#a0b8d8", fontSize:11, marginTop:2 }}>{t.desc}</div>
                        {t.deliverable && <div style={{ color:"#7090b8", fontSize:11, marginTop:2 }}>Deliverable: {t.deliverable}</div>}
                        {t.frequency && <div style={{ color:"#7090b8", fontSize:11 }}>Frequency: {t.frequency}</div>}
                      </div>
                    ))}
                  </div>
                )}
                ...
                {p.igceMode === "CATALOG" && p.catalogClins && p.catalogClins.length > 0 && (
                  <div style={{ marginBottom:10 }}>
                    ...
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                      <thead>
                        <tr style={{ background:"#152236" }}>
                          {["DESCRIPTION","UNIT","RATE","QTY/YR","TOTAL"].map(h => (
                            <td key={h} style={{ padding:"5px 8px", color:"#5ba3ff", borderBottom:"1px solid #1a3a6e" }}>{h}</td>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
              ...
              <div style={{ padding:"12px 20px", borderTop:"1px solid #1a2a4a", display:"flex", gap:8, flexShrink:0 }}>
                ...
                <button onClick={()=>setViewPkg(null)}
                  style={{ background:"#1a2d45", border:"1px solid #1a2a4a", color:"#6a8ab0",
      ...
      {(action?.type === "approve" || action?.type === "reassign") && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.7)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#152236", border:`1px solid ${action.type === "reassign" ? "#2a5a8a" : "#2a6a3a"}`, borderRadius:8, padding:28, width:400, maxWidth:"95vw" }}>
            ...
            {action.type === "reassign" && (
              <div style={{ background:"#1a2d45", border:"1px solid #1a3a6e", borderRadius:4, padding:"8px 12px", marginBottom:12, fontSize:11, color:"#a0b8d8" }}>
            ...
            <div style={{ color:"#6080a0", fontSize:11, marginBottom:16 }}>{action.pkg.reqTitle || action.pkg.req_title}</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={doApprove} disabled={submitting || !assignEmail.includes("@")}
                style={{ flex:1, background: action.type === "reassign" ? "#1a2d45" : "#041a0e",
              ...
              <button onClick={()=>setAction(null)} style={{ flex:1, background:"#1a2d45", border:"1px solid #1a2a4a",
      ...
      {action?.type === "reject" && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.7)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#152236", border:"1px solid #7a2a2a", borderRadius:8, padding:28, width:400, maxWidth:"95vw" }}>
            <div style={{ color:"#e05555", fontWeight:"bold", fontSize:14, marginBottom:4 }}>RETURN TO COR</div>
            <div style={{ color:"#6080a0", fontSize:11, marginBottom:16 }}>{action.pkg.reqTitle}</div>
...
function ReviewChain({ docKey, value, center }) {
  ...
  return (
    <div style={{ marginBottom:"20px" }}>
      <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"0.5px",marginBottom:"10px"}}>
        RCPO-WI-5001 REVIEW CHAIN - SEQUENTIAL ORDER v
      </div>
      <div style={{ fontSize:"10px", color:"#7090b8", marginBottom:"12px", fontStyle:"italic" }}>
      ...
      {chain.map((item, i) => (
        <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:"10px",marginBottom:"6px"}}>
          ...
          <div style={{ flex:1 }}>
            ...
            {item.note && <div style={{ fontSize:"10px", color:"#7090b8", marginTop:"2px" }}>{item.note}</div>}
          </div>
        </div>
      ))}
      <div style={{ fontSize:"10px", color:"#7090b8", marginTop:"10px", padding:"8px", background:"#0f1c2d", border:"1px solid #1a3a6e", borderRadius:"3px" }}>
...
function NF1787Form({ intake, onSave, savedData }) {
  ...
  return (
    <div style={{ padding:"16px" }}>
      <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"0.5px",marginBottom:"12px"}}>NF 1787 - SMALL BUSINESS COORDINATION RECORD</div>
      <div style={{ background:"#152236", border:"1px solid #1a5aaa", borderRadius:"3px", padding:"12px 14px",marginBottom:"12px"}}>
        ...
        {fields.map((f,i) => f.val ? (
          <div key={i} style={{ display:"flex", gap:"12px", padding:"4px 0", borderBottom:"1px solid #061428" }}>
            <div style={{ fontSize:"10px", color:"#7090b8", minWidth:"140px" }}>{f.label}</div>
      ...
      <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
        <a href="https://www.nasa.gov/sites/default/files/atoms/files/nf1787.pdf" target="_blank" rel="noopener noreferrer"
          style={{ fontSize:"10px", padding:"8px 14px", borderRadius:"3px", background:"#152236", border:"1px solid #1a5aaa", color:"#5ba3ff", textDecoration:"none", letterSpacing:"1px" }}>
          DOWNLOAD NF 1787 PDF
        </a>
        <button onClick={() => { onSave && onSave({completed:true}); setSaved(true); }}
          style={{ fontSize:"10px", padding:"8px 14px", cursor:"pointer", borderRadius:"3px", background:"#041a0e", border:"1px solid #3aaa66", color:"#2db87a", letterSpacing:"1px" }}>
...
function NF1787AForm({ intake, onSave, savedData }) {
  ...
  return (
    <div style={{ padding:"16px" }}>
      <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"0.5px",marginBottom:"12px"}}>NF 1787A - MARKET RESEARCH REPORT</div>
      <div style={{ background:"#152236", border:"1px solid #1a5aaa", borderRadius:"3px", padding:"12px 14px",marginBottom:"12px"}}>
        <div style={{ fontSize:"10px", color:"#5ba3ff",marginBottom:"8px"}}>Acquisition Context</div>
        {fields.map((f,i) => f.val ? (
          <div key={i} style={{ display:"flex", gap:"12px", padding:"4px 0", borderBottom:"1px solid #061428" }}>
      ...
      <div style={{ background:"#152236", border:"1px solid #1a3a6e", borderRadius:"3px", padding:"10px 14px", marginBottom:"14px", fontSize:"10px", color:"#e8eef8" }}>
                  ...
                  return (
                    <div key={qi} onClick={()=>setFormData(d=>({...d,[step.id+"_qc_"+qi]:!ck}))}
                      style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",
                              background:ck?"#041a0e":"#080f1e",border:"1px solid "+(ck?"#1a6a3a":"#1a2d45"),
        ...
        {step.type === "FORM" && step.formType === "NAICS" && (
          <div style={S.panelSection}>
            ...
            <input placeholder="Search by code or keyword..." value={naicsSearch}
              ...
              style={{ background:"#152236", border:"1px solid #1a3a6e", color:"#e8eef8", padding:"7px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", marginBottom:"8px", fontFamily:"IBM Plex Mono,monospace" }} />
            ...
            <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>OR ENTER ANY CODE MANUALLY</div>
            <input placeholder="e.g. 481211" value={naicsCustom} onChange={e=>setNaicsCustom(e.target.value)}
              style={{ background:"#152236", border:"1px solid " + (naicsCustom?"#5ba3ff":"#2255a4"), color:"#e8eef8", padding:"7px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", marginBottom:"8px", fontFamily:"IBM Plex Mono,monospace" }} />
        ...
        {step.type === "FORM" && step.formType === "PSC" && (
          <div style={S.panelSection}>
            ...
            <input placeholder="e.g. V231" value={pscCustom} onChange={e=>setPscCustom(e.target.value)}
              style={{ background:"#152236", border:"1px solid " + (pscCustom?"#5ba3ff":"#2255a4"), color:"#e8eef8", padding:"7px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", marginBottom:"8px", fontFamily:"IBM Plex Mono,monospace" }} />
                ...
                {!docContent[curPkg.key] ? (
                  ...
                  <>
                    ...
                    <div style={{ display:"flex", gap:"8px", marginTop:"8px", flexWrap:"wrap" }}>
                      ...
                      <button style={{ ...S.copyBtn, background:"#152236", border:"1px solid #2a6aaa", color:"#5ba3ff" }}
    ...
    <div style={S.wWrap}>
      ...
      <div style={S.wCard}>
        ...
        <div style={{ display:"flex", gap:8, marginTop:8 }}>
          {step > 0 && (
            <button onClick={back}
              style={{ flex:1, background:"#152236", border:"1px solid #1a3a6e", color:"#7090b8",
...
function TemplatePanel({ stepId }) {
  ...
  return (
    <div style={{ marginTop:"12px", border:"1px solid #0d3a6e", borderRadius:"4px", overflow:"hidden" }}>
      <div style={{ padding:"7px 12px", background:"#04111f", fontSize:"9px", color:"#7090b8", letterSpacing:"0.5px" }}>
          ...
          return (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"5px 0", borderBottom:"1px solid #061428" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                {driveEntry && <div style={{ fontSize:"9px", color:"#7090b8", fontStyle:"italic" }}>{driveEntry.label}</div>}
              </div>
              {driveEntry ? (
                <a href={"https://docs.google.com/document/d/" + driveEntry.id + "/edit"}
                  target="_blank" rel="noopener noreferrer"
                  style={{ fontSize:"9px", padding:"3px 8px", borderRadius:"2px", background:"#152236", border:"1px solid #1a5aaa", color:"#5ba3ff", letterSpacing:"1px", textDecoration:"none", whiteSpace:"nowrap" }}>
                  OPEN
                </a>
              ) : (
...
function NEARPackage({ intake, roadmap }) {
  ...
  if (!intake) return <div style={{ padding:"40px", textAlign:"center", color:"#7090b8", fontSize:"12px" }}>Complete the intake wizard first.</div>;
  ...
  return (
    <div style={{ padding:"20px", maxWidth:"860px", margin:"0 auto" }}>
      <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"0.5px",marginBottom:"4px"}}>NEAR FILING PACKAGE</div>
      <div style={{ fontSize:"18px", color:"#e8eef8", fontWeight:700,marginBottom:"4px"}}>{intake.reqTitle}</div>
      <div style={{ fontSize:"11px", color:"#7090b8",marginBottom:"10px"}}>${(intake.value||0).toLocaleString()} | {intake.center} | {intake.contractType}</div>
      <div style={{ background:"#152236", border:"1px solid #1a3a6e", borderRadius:"4px", padding:"12px 16px",marginBottom:"20px"}}>
        <div style={{ display:"flex", justifyContent:"space-between",marginBottom:"6px"}}>
          <span style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px" }}>DOCUMENTS FILED TO NEAR</span>
          <span style={{ fontSize:"11px", color:"#e8eef8" }}>{genCount}/{expected.length}</span>
        </div>
        <div style={{ height:"5px", background:"#152236", borderRadius:"3px",marginBottom:"10px"}}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"5px" }}>
          {expected.map(e => { const done=genTypes.has(e.docType); const m=NEAR_MAP[e.docType]||{};
            return <div key={e.docType} style={{ fontSize:"9px", padding:"2px 6px", borderRadius:"2px", background:done?"#041a0e":"transparent", border:"1px solid "+(done?"#2db87a":"#2255a4"), color:done?"#2db87a":"#2a4a6a" }}>
      ...
      {docs.length > 0 && (
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          <button onClick={dlAll}
            style={{background:"#152236",border:"1px solid #2a6aaa",color:"#5ba3ff",
                    padding:"7px 16px",borderRadius:4,cursor:"pointer",fontSize:11,fontWeight:"bold"}}>
            ↓ DOWNLOAD ALL (.docx)
          </button>
      ...
      {docs.length===0 ? (
        <div style={{ padding:"32px", textAlign:"center", border:"1px dashed #1a3a6e", borderRadius:"4px", color:"#7090b8", fontSize:"11px" }}>
        ...
        return <div key={cat} style={{ marginBottom:"14px" }}>
          ...
          {cd.map((doc,i) => <div key={i} style={{ display:"flex", alignItems:"center", gap:"10px", padding:"8px 10px", marginBottom:"4px", background:"#152236", border:"1px solid #1a3a6e", borderRadius:"3px" }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:"9px", color:"#7090b8" }}>{doc.label||doc.docType}</div>
            </div>
            <button onClick={()=>dl(doc)} style={{ fontSize:"9px", padding:"3px 9px", cursor:"pointer", borderRadius:"2px", background:"#152236", border:"1px solid #1a5aaa", color:"#5ba3ff", letterSpacing:"1px" }}>DL</button>
          </div>)}
        </div>;
      })}
...
function RouteAdvisor({ onUseRecommendation }) {
  ...
  const inp = { background:"#152236", border:"1px solid #1a3a6e", color:"#e8eef8", padding:"8px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", fontFamily:"IBM Plex Mono,monospace", outline:"none" };
  ...
  return (
    <div style={{ padding:"24px", maxWidth:"820px", margin:"0 auto" }}>
      <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"0.5px",marginBottom:"4px"}}>ACQUISITION APPROACH COACH</div>
      <div style={{ fontSize:"20px", color:"#e8eef8", fontWeight:700,marginBottom:"4px"}}>Acquisition Approach Coach <span style={{fontSize:"13px",color:"#7090b8",fontWeight:400}}>(your acquisition life coach)</span></div>
      <div style={{ fontSize:"11px", color:"#7090b8",marginBottom:"20px"}}>Options, analysis, and alternatives — you decide. Think of this as your acquisition life coach. (The CO is still the CO.)</div>
      ...
      {mode === "QUICK" ? (
        <div>
          <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>DESCRIBE THE REQUIREMENT</div>
          ...
          <div style={S.grid2}>
            <div>
              <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>ESTIMATED TOTAL VALUE</div>
              <input value={dollars} onChange={e=>setDollars(e.target.value)} placeholder="e.g. 5000000" style={{ ...inp,marginBottom:"10px"}} />
            </div>
            <div>
              <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>PERIOD OF PERFORMANCE</div>
          ...
          <div style={S.grid2}>
            <div>
              <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>INCUMBENT CONTRACT?</div>
            ...
            <div>
              <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>URGENCY / NEED DATE</div>
          ...
          <div style={S.grid2}>
            <div>
              <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>EXISTING CONTRACT VEHICLE?</div>
            ...
            <div>
              <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>SMALL BUSINESS GOAL</div>
        ...
        <div style={{ marginBottom:"16px" }}>
          <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>UPLOAD PACKAGE DOCUMENTS (.docx, .txt, .pdf text)</div>
          <label style={{ display:"block", border:"2px dashed #1a3a6e", borderRadius:"4px", padding:"20px", textAlign:"center", cursor:"pointer", color:"#7090b8", fontSize:"11px" }}>
      ...
      {result && result.primary && (
          <div>
            <div style={{ background:"#041a0e", border:"1px solid #3aaa66", borderRadius:"4px", padding:"16px 20px", marginBottom:"12px" }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"10px", flexWrap:"wrap", gap:"8px" }}>
                ...
                <div style={{ display:"flex", gap:"6px", flexWrap:"wrap" }}>
                  ...
                  <div style={{ padding:"3px 9px", borderRadius:"3px", fontSize:"9px", letterSpacing:"1px", background:"#04111f", border:"1px solid #4a9eff", color:"#5ba3ff" }}>~{result.primary.timelineWeeks}wks</div>
                </div>
              </div>
              <div style={{ fontSize:"11px", color:"#e8eef8", lineHeight:1.7, padding:"10px 12px", background:"#0f1c2d", borderRadius:"3px", marginBottom:"10px" }}>{result.primary.whyThisPath}</div>
              <div style={S.grid2}>
                <div style={{ background:"#0f1c2d", borderRadius:"3px", padding:"9px 11px" }}>
              ...
              <div style={{ ...S.grid2, marginTop:"8px" }}>
                <div style={{ background:"#0f1c2d", borderRadius:"3px", padding:"9px 11px" }}>
                  <div style={{ fontSize:"9px", color:"#7090b8", letterSpacing:"1px", marginBottom:"4px" }}>ADMIN BURDEN</div>
                  <div style={{ fontSize:"10px", color:"#e8eef8", lineHeight:1.5 }}>{result.primary.adminBurdenExplain}</div>
                </div>
                <div style={{ background:"#0f1c2d", borderRadius:"3px", padding:"9px 11px" }}>
                  <div style={{ fontSize:"9px", color:"#7090b8", letterSpacing:"1px", marginBottom:"4px" }}>TIMELINE</div>
              ...
              {result.primary.keyDocs?.length>0 && <div style={{ marginTop:"8px" }}>
                <div style={{ fontSize:"9px", color:"#7090b8", letterSpacing:"1px", marginBottom:"5px" }}>REQUIRED DOCUMENTS</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:"5px" }}>
              ...
              <div style={{ marginTop:"10px" }}>
                <div style={{ fontSize:"9px", color:"#7090b8", letterSpacing:"1px", marginBottom:"5px" }}>NEXT STEPS</div>
            ...
            {(result.whyNotFAR15||result.whyNotTM) && (
              <div style={{ background:"#152236", border:"1px solid #1a3a6e", borderRadius:"4px", padding:"12px 14px", marginBottom:"12px" }}>
                <div style={{ fontSize:"9px", color:"#7090b8", letterSpacing:"0.5px", marginBottom:"8px" }}>WHY NOT THE MORE COMPLEX APPROACHES</div>
            ...
            {result.alternative?.approach && (
              <div style={{ background:"#152236", border:"1px solid #1a3a6e", borderRadius:"4px", padding:"12px 14px", marginBottom:"12px" }}>
                <div style={{ fontSize:"9px", color:"#7090b8", letterSpacing:"0.5px", marginBottom:"6px" }}>NEXT BEST ALTERNATIVE</div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"6px", flexWrap:"wrap", gap:"5px" }}>
                  <div>
                    ...
                    <div style={{ fontSize:"10px", color:"#7090b8" }}>{result.alternative.farAuthority} | ~{result.alternative.timelineWeeks}wks | {result.alternative.adminBurden} burden</div>
                  </div>
                </div>
...
function DocumentReview({ intake, initialDocType }) {
  ...
  const inp = { background:"#152236", border:"1px solid #1a3a6e", color:"#e8eef8", padding:"8px 10px", borderRadius:"3px", fontSize:"11px", width:"100%", fontFamily:"IBM Plex Mono,monospace" };

  return (
    <div style={{ padding:"20px", maxWidth:"800px", margin:"0 auto" }}>
      <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"0.5px",marginBottom:"4px"}}>DOCUMENT REVIEW</div>
      <div style={{ fontSize:"18px", color:"#e8eef8", fontWeight:700,marginBottom:"4px"}}>AI-Powered Acquisition Document Review</div>
      <div style={{ fontSize:"11px", color:"#7090b8",marginBottom:"20px"}}>FAR/NFS compliance check, completeness review, and issue flagging</div>

      {intake && (
        <div style={{ background:"#152236", border:"1px solid #1a3a6e", borderRadius:"4px", padding:"10px 14px", marginBottom:"16px", fontSize:"10px", color:"#7090b8" }}>
          CONTEXT: {ctx}
        </div>
      )}

      <div style={{ marginBottom:"14px" }}>
        <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>DOCUMENT TYPE</div>
        <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
          {DOC_TYPES.map(dt => (
            <button key={dt.key}
      ...
      <div style={{ marginBottom:"14px" }}>
        <div style={{ fontSize:"10px", color:"#7090b8", letterSpacing:"1px",marginBottom:"6px"}}>UPLOAD DOCUMENT</div>
        <label style={{ display:"block", border:"2px dashed #1a3a6e", borderRadius:"4px", padding:"20px", textAlign:"center", cursor:"pointer", color:"#7090b8", fontSize:"11px" }}>
          {file ? (
      ...
      {result && (
        <div style={{ animation:"slideIn .3s ease" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"12px",marginBottom:"16px"}}>
            ...
            <div style={{ fontSize:"11px", color:"#7090b8" }}>{result.docType}</div>
          </div>

          <div style={{ fontSize:"11px", color:"#e8eef8", lineHeight:1.7, marginBottom:"16px", padding:"12px", background:"#152236", border:"1px solid #1a3a6e", borderRadius:"4px" }}>
            {result.summary}
          </div>
          ...
          {result.findings?.length > 0 && (
            <div>
              ...
              {result.findings.map((f, i) => (
                <div key={i} style={{ marginBottom:"10px", border:"1px solid "+SEV_COLOR[f.severity]||"#2255a4"+"22", borderLeft:"3px solid "+(SEV_COLOR[f.severity]||"#2255a4"), borderRadius:"3px", padding:"10px 12px", background:"#152236" }}>
    ...
    <>
      ...
      {open && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{background:"#152236",border:"1px solid #2a4a7a",borderRadius:8,width:500,maxWidth:"95vw",maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
            <div style={{padding:"14px 18px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <button onClick={()=>setOpen(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
            </div>
            <div style={{flex:1,overflow:"auto",padding:16}}>
              {checking && <div style={{color:"#6080a0",textAlign:"center",padding:30}}>Analyzing document...</div>}
...
function SubmitModal({ docType, content, intake, onClose, onSubmitted }) {
  ...
  const inp = (ph, key) => (
    <input placeholder={ph} value={roles[key]||""}
      ...
      style={{width:"100%",background:"#1a2d45",border:"1px solid #2a4a7a",color:"#e8eef8",
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#152236",border:"1px solid #2a4a7a",borderRadius:8,padding:28,width:420,maxWidth:"95vw"}}>
        ...
        <div style={{display:"flex",gap:8,marginTop:4}}>
          <button onClick={submit} disabled={submitting}
            style={{flex:1,background:"#1a4a8a",border:"1px solid #2a6aaa",color:"#5ba3ff",
                    padding:"9px",borderRadius:4,cursor:"pointer",fontSize:12,fontWeight:"bold"}}>
            {submitting ? "SUBMITTING..." : "SUBMIT"}
          </button>
          <button onClick={onClose}
            style={{flex:1,background:"#1a2d45",border:"1px solid #2a4a7a",color:"#6a8ab0",
...
function ReviewerDashboard({ onClose }) {
  ...
  const row = (sub) => {
    ...
    return (
      <div key={sub.id} onClick={()=>setSelected(sub===selected?null:sub)}
        style={{background: selected===sub?"#1a3a5e":"#080f1e",border:"1px solid "+(isMyTurn?"#2a6aaa":"#1a2d45"),
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            ...
            {isMyTurn && <span style={{background:"#1a4a8a",color:"#5ba3ff",fontSize:9,padding:"1px 6px",borderRadius:10,marginLeft:8}}>YOUR TURN</span>}
          </div>
          ...
          <div style={{marginTop:12,borderTop:"1px solid #1a2a4a",paddingTop:12}}>
            ...
            {sub.history?.length > 0 && (
              <div style={{marginTop:8}}>
                ...
                {sub.history.map((h,i)=>(
                  <div key={i} style={{color:"#6080a0",fontSize:10,marginBottom:2}}>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:998,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:"#152236",border:"1px solid #2a4a7a",borderRadius:8,width:560,maxWidth:"95vw",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
        {/* Header */}
        <div style={{padding:"16px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <button onClick={onClose}
              style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer",lineHeight:1}}>×</button>
          </div>
        </div>
          ...
          {!email ? (
            <div>
              ...
              <button onClick={login}
                style={{width:"100%",background:"#1a4a8a",border:"1px solid #2a6aaa",color:"#5ba3ff",
                        padding:"9px",borderRadius:4,cursor:"pointer",fontSize:12,fontWeight:"bold"}}>
                VIEW MY QUEUE
              </button>
            </div>
          ) : loading ? (
            <div style={{color:"#6080a0",textAlign:"center",padding:40}}>Loading...</div>
          ) : submissions.length === 0 ? (
            <div style={{color:"#6080a0",textAlign:"center",padding:40}}>No documents in your queue.</div>
          ) : (
            submissions.map(row)
          )}
        </div>
        ...
        {action && (
          <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.7)",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:8}}>
            <div style={{background:"#152236",border:"1px solid #2a4a7a",borderRadius:8,padding:24,width:360}}>
...
function CPAS() {
  ...
  return (
    ...
    {showRequestorPortal && (
      <div style={{position:"fixed",inset:0,background:"#0f1c2d",zIndex:990,overflow:"auto"}}>
        <RequestorPortal
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:997,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #2a4a7a",borderRadius:8,width:700,maxWidth:"96vw",maxHeight:"92vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={()=>setShowMod(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
    ...
    {showForms && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:996,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #1a5a8a",borderRadius:8,width:860,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#4ac8ff",fontWeight:"bold",fontSize:13}}>SF FORMS BUILDER</div>
              <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>SF-1449 · SF-26 · SF-33 · SF-18 · SF-1442 · SF-1447 · OF-347 · OF-336</div>
            </div>
            <button onClick={()=>setShowForms(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showBPA && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:995,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #3a1a3a",borderRadius:8,width:680,maxWidth:"96vw",maxHeight:"92vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#e07ae0",fontWeight:"bold",fontSize:13}}>BPA CALL ORDER BUILDER</div>
              <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>FAR 13.303-5 · Blanket Purchase Agreement Call</div>
            </div>
            <button onClick={()=>setShowBPA(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showTechEval && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:989,display:"flex",flexDirection:"column"}}>
        <div style={{background:"#152236",borderBottom:"1px solid #1a2a4a",padding:"10px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div>
            <div style={{color:"#aa7aff",fontWeight:"bold",fontSize:13}}>TECHNICAL EVALUATION HELPER</div>
            <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>Source Selection Sensitive · For Technical Evaluators and CORs</div>
          </div>
          <button onClick={()=>setShowTechEval(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
        </div>
    ...
    {showPreSol && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:991,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #2a6a2a",borderRadius:8,width:1100,maxWidth:"98vw",maxHeight:"95vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#6aff6a",fontWeight:"bold",fontSize:13}}>PRE-SOLICITATION TOOLS</div>
              <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>IGCE Worksheet  ·  Market Research Report (FAR Part 10)</div>
            </div>
            <button onClick={()=>setShowPreSol(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showCompliance && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:992,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #1a5a5a",borderRadius:8,width:920,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#4affff",fontWeight:"bold",fontSize:13}}>COMPLIANCE TOOLS</div>
              <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>Responsibility Determination  ·  Closeout Checklist  ·  D&F Templates</div>
            </div>
            <button onClick={()=>setShowCompliance(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showAdminTools && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:992,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #3a1a1a",borderRadius:8,width:960,maxWidth:"98vw",maxHeight:"96vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#ff6a6a",fontWeight:"bold",fontSize:13}}>CONTRACT ADMINISTRATION TOOLS</div>
              <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>Closeout · CPARS · Debrief · Pre-Negotiation Objectives</div>
            </div>
            <button onClick={()=>setShowAdminTools(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showAwardTools && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:993,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #3a3a1a",borderRadius:8,width:920,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#f0b429",fontWeight:"bold",fontSize:13}}>AWARD TOOLS</div>
              <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>Price Analysis / PNM  ·  Option Exercise  ·  FPDS-NG Checklist</div>
            </div>
            <button onClick={()=>setShowAwardTools(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showUCF && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:994,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#152236",border:"1px solid #1a5a2a",borderRadius:8,width:920,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#4aff8a",fontWeight:"bold",fontSize:13}}>UCF SECTIONS D / E / F</div>
              <div style={{color:"#6080a0",fontSize:10,marginTop:2}}>Packaging · Inspection · Deliveries</div>
            </div>
            <button onClick={()=>setShowUCF(false)} style={{background:"none",border:"none",color:"#6080a0",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
