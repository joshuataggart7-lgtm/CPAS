...
async function generateDoc(docType, intake, roadmap) {
  ...
  const comm = intake?.isCommercial;
    ...
    JOFOC:"Write a NASA Justification for Other Than Full and Open Competition (JOFOC) per RFO FAR 6.104-1 [formerly FAR 6.303-2] and PCD 25-10. CRITICAL: This contract is for CONTRACTOR-OWNED aircraft services. The contractor owns and operates the modified aircraft. Do NOT mention NASA-owned aircraft (ER-2, DC-8, C-20A, Twin Otter, or any NASA fleet). The contractor provides King Air B-200 and A-90 aircraft they own or lease. Contract type is FFP IDIQ with pre-priced catalog rates — NOT CPFF. For: \""+(t)+"\", "+(c)+", "+(v)+", NAICS "+(n)+". Include all required elements per RFO FAR 6.104-1 [formerly FAR 6.303-2] labeled exactly: 1.IDENTIFICATION OF AGENCY AND CONTRACTING ACTIVITY 2.NATURE OF ACTION 3.DESCRIPTION OF SUPPLIES OR SERVICES 4.IDENTIFICATION OF STATUTORY AUTHORITY (cite FAR 6.302-X) 5.DEMONSTRATION THAT AUTHORITY APPLIES (specific technical facts, not conclusions) 6.EFFORTS TO OBTAIN COMPETITION 7.ANTICIPATED COST WILL BE FAIR AND REASONABLE 8.MARKET RESEARCH CONDUCTED 9.OTHER FACTS SUPPORTING USE 10.SOURCES THAT EXPRESSED INTEREST 11.ACTIONS TO REMOVE FUTURE BARRIERS. Section 2 NATURE OF ACTION must state contract type as Firm-Fixed-Price (FFP) Indefinite-Delivery Indefinite-Quantity (IDIQ) with pre-priced catalog rate task orders — not CPFF or cost-reimbursement. End with CO certification block and tiered approval signature block per RFO FAR 6.104-2 / NFS 1806.304 (PCD 25-10, PN 24-05): <$900K=CO approves; $900K-$20M=CA approves; $20M-$150M=CA concurs HCA approves; >$150M=CA and HCA concur Agency CA concurs SPE approves. Mark SOURCE SELECTION SENSITIVE.",
...
function BranchChiefDashboard({ onClose }) {
  ...
  const [action, setAction] = useState(null); // {type: "approve"|"reject", pkg}
                  ...
                  {selected === pkg.id && (
                    <div style={{ marginTop:12, borderTop:"1px solid #1a3a6e", paddingTop:12 }}>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12, fontSize:11 }}>
                        <div><span style={{ color:"#4a7aaa" }}>Submitted: </span><span style={{ color:"#c8d8f0" }}>{new Date(pkg.submittedAt).toLocaleDateString()}</span></div>
                        <div><span style={{ color:"#4a7aaa" }}>COR Email: </span><span style={{ color:"#c8d8f0" }}>{pkg.corEmail}</span></div>
                        {(pkg.packageData?.sowObjective || pkg.package_data?.sowObjective) && (
                          <div style={{ gridColumn:"1/-1" }}>
                            <span style={{ color:"#4a7aaa" }}>Objective: </span>
                            <span style={{ color:"#c8d8f0" }}>{(pkg.packageData?.sowObjective || pkg.package_data?.sowObjective || "").slice(0,120)}...</span>
                          </div>
                        )}
                        {pkg.assignedToName && (
                          <div><span style={{ color:"#4a7aaa" }}>Assigned to: </span><span style={{ color:"#3aaa66", fontWeight:"bold" }}>{pkg.assignedToName}</span></div>
                        )}
                        {pkg.rejectionReason && (
                          <div style={{ gridColumn:"1/-1" }}><span style={{ color:"#cc4a4a" }}>Rejection reason: </span><span style={{ color:"#c8d8f0" }}>{pkg.rejectionReason}</span></div>
                        )}
                      </div>
      ...
      fetch(`${PKG_API}/package/${pkgId}`)
        ...
        .then(pkg => {
          if (pkg && (pkg.packageData || pkg.package_data)) {
            ...
            // Switch to INTAKE screen first, then fire event — wizard must be mounted to receive it
            setScreen("INTAKE");
            setTimeout(() => window.dispatchEvent(new Event("cpas_prefill")), 150);
          }
        }).catch(()=>{});
    }
