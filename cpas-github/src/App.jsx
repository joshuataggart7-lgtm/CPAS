...
async function downloadAsWord(docType, text, intake) {
  try {
    ...
    const title = intake?.reqTitle || "NASA Acquisition Document";
    const center = intake?.center || "NASA";
    const label = docType || "DOC";
    const date = new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"});

    const bodyParas = (text||"").split("\n").map(line => {
      const trimmed = line.trim();
      if (!trimmed) return new Paragraph({ text: "" });
      if (/^#{1,2}\s/.test(trimmed)) return new Paragraph({
        text: trimmed.replace(/^#+\s/,""),
        heading: HeadingLevel.HEADING_1,
      });
      if (/^###\s/.test(trimmed)) return new Paragraph({
        text: trimmed.replace(/^###\s/,""),
    ...
    const doc = new Document({
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            children: [new TextRun({ text: center, bold: true, size: 28 })],
            alignment: AlignmentType.CENTER,
            spacing: { after: 200 },
          }),
          new Paragraph({
          ...
          new Paragraph({
            children: [new TextRun({ text: date, italics: true, size: 22 })],
            alignment: AlignmentType.CENTER,
            spacing: { after: 400 },
          }),
