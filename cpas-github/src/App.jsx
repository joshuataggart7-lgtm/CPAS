...
async function downloadAsWord(docType, text, intake) {
  try {
    ...
    const { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType,
      BorderStyle, WidthType, ShadingType, LevelFormat, Header,
      TabStopType, Table, TableRow, TableCell } = window.docx;
    ...
    for (const line of text.split("\n")) {
      ...
      if (line.startsWith("# ")) {
        bodyChildren.push(new Paragraph({heading:HeadingLevel.HEADING_1,spacing:{before:300,after:100},
          children:[new TextRun({text:line.slice(2).trim(),bold:true,font:"Arial",size:28,color:"1F4E79"})]}));
      } else if (line.startsWith("## ")) {
        bodyChildren.push(new Paragraph({heading:HeadingLevel.HEADING_2,spacing:{before:200,after:80},
          children:[new TextRun({text:line.slice(3).trim(),bold:true,font:"Arial",size:24,color:"2E75B6"})]}));
      } else if (line.startsWith("### ")) {
        bodyChildren.push(new Paragraph({heading:HeadingLevel.HEADING_3,spacing:{before:160,after:60},
          children:[new TextRun({text:line.slice(4).trim(),bold:true,font:"Arial",size:22,color:"2E75B6"})]}));
      } else if (line.trim().match(/^\|/)) {
        bodyChildren.push(new Paragraph({spacing:{before:40,after:40},
          children:[new TextRun({text:line.replace(/\|/g," | ").trim(),font:"Courier New",size:18})]}));
      } else if (line.match(/^\s*[-*] /)) {
        bodyChildren.push(new Paragraph({numbering:{reference:"bullets",level:0},spacing:{before:40,after:40},
          children:parseInline(line.replace(/^\s*[-*] /,""))}));
      } else if (line.trim()) {
        bodyChildren.push(new Paragraph({spacing:{before:60,after:60},children:parseInline(line)}));
      }
    }
    ...
    a.download = label.replace(/[^a-zA-Z0-9]/g,"_").substring(0,25)+"_"+safeTitle+".docx";
    a.href = url;
    a.click();
    URL.revokeObjectURL(url);
    return null;
  } catch(e) { return "Word export error: "+e.message; }
}
