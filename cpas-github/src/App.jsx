...
const STEP_CHECKS = {
  ...
  P2S5: [
    ...
    "JOFOC approved at correct level per RFO FAR 6.104-2 / NFS 1806.304 (PCD 25-10, PN 24-05)? — <$900K CO; $900K-$20M CA; $20M-$150M HCA (PO+CA concur); >$150M SPE (PO+CA+HCA+Agency CA concur)",
...
// ── Tools Menu Dropdown ───────────────────────────────────────────
function ToolsMenu({ onMod, onForms, onBPA, onUCF, onAward, onAdmin, onCompliance, onPreSol, onReview, onRequestor, onTechEval }) {
  const [open, setOpen] = React.useState(false);
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        <button onClick={()=>{
                          try {
                            ...
                            // Pre-fill intake wizard from portal package
                            window.__cpas_prefill = {
                              reqTitle:            p.reqtitle || "",
                              value:               parseFloat(p.valueEstimate) || 0,
                              reqType:             p.workType === "rd" ? "RD" : p.workType === "it" ? "IT" : "SERVICES",
                              isCommercial:        p.commerciallyAvailable === "yes" ? "YES" : "TBD",
                              ...
                              pop:                 p.popMonths ? p.popMonths + " months" : "",
                            };
                          } catch(e) {}
                          setScreen("INTAKE");
                ...
                <div style={{ background:"#0a2a1a", border:"1px solid #3aaa66", borderRadius:4,
                  ...
                  {pending.map(p => (
                    <div key={p.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                      ...
                      <span>📋 {p.requirement?.title || "Untitled"} — ${((p.requirement?.estimatedValue||0)).toLocaleString()}</span>
                      <div style={{ display:"flex", gap:8 }}>
                        <button onClick={()=>{
                          // Pre-fill intake from portal package
                          const prefill = {
                            reqTitle: p.requirement?.title || "",
                            value: p.requirement?.estimatedValue || 0,
                            reqType: p.requirement?.workType === "research" ? "RD" : "SERVICES",
                            isCommercial: p.requirement?.commerciallyAvailable === "yes" ? "YES" : "TBD",
                            competitionStrategy: "FULL_OPEN",
                            contractType: "FFP",
                            center: p.requestor?.center || "",
                          };
                          window.__cpas_prefill = prefill;
                          window.dispatchEvent(new Event("cpas_prefill"));
                          setScreen("INTAKE");
                          // Remove from pending
                          try {
                            const updated = JSON.parse(localStorage.getItem("cpas_pending_packages") || "[]").filter(x => x.id !== p.id);
                            localStorage.setItem("cpas_pending_packages", JSON.stringify(updated));
                          } catch(e) {}
