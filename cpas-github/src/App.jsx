...
function HQDashboard({ onClose }) {
  ...
  const s = { background:"#0a1a3a", border:"1px solid #2a4a7a", borderRadius:8,
    width:900, maxWidth:"97vw", maxHeight:"93vh", display:"flex", flexDirection:"column" };

  const inp = { background:"#08182e", border:"1px solid #1a3a6e", color:"#c8d8f0",
    padding:"10px 14px", borderRadius:4, fontSize:14, fontFamily:"'IBM Plex Mono',monospace",
    textAlign:"center", letterSpacing:6, width:160 };

  const tab = (id, label) => (
    <button onClick={()=>setActiveTab(id)} style={{
      background: activeTab===id ? "#1a3a6e" : "none",
      border: `1px solid ${activeTab===id ? "#4a9eff" : "#1a3a6e"}`,
      color: activeTab===id ? "#4a9eff" : "#4a6a8a",
  ...
  const statBox = (label, value, color="#4a9eff", sub) => (
    <div style={{ background:"#08182e", border:`1px solid ${color}33`, borderRadius:6, padding:"14px 18px", flex:1, minWidth:120 }}>
      <div style={{ fontSize:9, color:color, letterSpacing:2, textTransform:"uppercase", marginBottom:4 }}>{label}</div>
  ...
  const barChart = (items, colorFn) => {
    ...
    return items.map((item,i) => (
      <div key={i} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
        <div style={{ width:120, color:"#8ab0d0", fontSize:11, textAlign:"right", flexShrink:0 }}>{item.label}</div>
        <div style={{ flex:1, background:"#08182e", borderRadius:3, height:22, overflow:"hidden" }}>
        ...
        <div style={{ width:80, color:"#4a6a8a", fontSize:10, flexShrink:0 }}>
  ...
  if (!authed) return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:998, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#0a1a3a", border:"1px solid #2a4a7a", borderRadius:8, padding:40, width:360, textAlign:"center" }}>
        <div style={{ color:"#f4c542", fontWeight:"bold", fontSize:13, letterSpacing:2, marginBottom:4 }}>HQ LEADERSHIP DASHBOARD</div>
        <div style={{ color:"#4a6a8a", fontSize:11, marginBottom:24 }}>Restricted Access — Enter PIN to continue</div>
        <div style={{ color:"#8ab0d0", fontSize:10, letterSpacing:2, marginBottom:8 }}>ACCESS PIN</div>
        ...
        {pinError && <div style={{ color:"#cc4a4a", fontSize:11, marginTop:8 }}>Incorrect PIN</div>}
        <div style={{ display:"flex", gap:8, marginTop:16 }}>
          <button onClick={tryPin} style={{ flex:1, background:"#1a4a8a", border:"1px solid #2a6aaa",
            color:"#4a9eff", padding:"10px", borderRadius:4, cursor:"pointer", fontSize:12, fontWeight:"bold" }}>
            ENTER
          </button>
          <button onClick={onClose} style={{ flex:1, background:"#0d1f3c", border:"1px solid #1a2a4a",
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:998, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={s}>
        {/* Header */}
        <div style={{ padding:"14px 20px", borderBottom:"1px solid #1a2a4a", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
          <div>
            <div style={{ color:"#f4c542", fontWeight:"bold", fontSize:14 }}>HQ LEADERSHIP DASHBOARD</div>
            <div style={{ color:"#4a6a8a", fontSize:10, marginTop:2 }}>NASA Procurement Pipeline — All Centers</div>
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <button onClick={loadData} style={{ background:"none", border:"1px solid #1a3a6e", color:"#4a7aaa",
              padding:"4px 10px", borderRadius:4, cursor:"pointer", fontSize:10 }}>↻ REFRESH</button>
            <button onClick={onClose} style={{ background:"none", border:"none", color:"#4a6a8a", fontSize:22, cursor:"pointer" }}>×</button>
          </div>
        </div>

        {/* Tabs */}
        <div style={{ padding:"10px 20px", borderBottom:"1px solid #1a2a4a", display:"flex", gap:8, flexShrink:0 }}>
        ...
        <div style={{ flex:1, overflow:"auto", padding:20 }}>
          {loading ? (
            <div style={{ color:"#4a6a8a", textAlign:"center", padding:40 }}>Loading data...</div>
          ) : data.length === 0 ? (
            <div style={{ color:"#4a6a8a", textAlign:"center", padding:40, border:"1px dashed #1a3a6e", borderRadius:4 }}>
            ...
            <>
              ...
              {activeTab === "overview" && (
                <div>
                  ...
                  <div style={{ display:"flex", gap:12, marginBottom:20, flexWrap:"wrap" }}>
                    {statBox("Total Packages", total, "#4a9eff")}
                    {statBox("Pipeline Value", `$${(totalValue/1000000).toFixed(1)}M`, "#f4c542")}
                    {statBox("Pending Review", pending, "#ffaa30", "awaiting Branch Chief")}
                    {statBox("In Progress", inProgress, "#3aaa66", "active acquisitions")}
                    {statBox("Avg Days to Assign", avgDaysToAssign||"—", "#8a70d0", "submission to CO assignment")}
                  </div>

                  {/* Status breakdown */}
                  <div style={{ color:"#4a9eff", fontSize:10, letterSpacing:2, marginBottom:10 }}>STATUS BREAKDOWN</div>
                  {barChart([
                  ...
                  ], item => item.label==="PENDING"?"#ffaa30":item.label==="ASSIGNED"?"#4a9eff":item.label==="IN PROGRESS"?"#3aaa66":"#cc4a4a")}

                  {/* Urgency breakdown */}
                  <div style={{ color:"#4a9eff", fontSize:10, letterSpacing:2, marginBottom:10, marginTop:20 }}>URGENCY BREAKDOWN</div>
                  {barChart([
                  ...
                  ], item => item.label==="URGENT"?"#cc3333":item.label==="MODERATE"?"#cc8800":"#3aaa66")}
                </div>
              )}
              ...
              {activeTab === "pipeline" && (
                <div>
                  <div style={{ color:"#4a9eff", fontSize:10, letterSpacing:2, marginBottom:12 }}>ALL PACKAGES — ${(totalValue/1000000).toFixed(1)}M TOTAL PIPELINE VALUE</div>
                  {data.sort((a,b)=>new Date(b.submitted_at)-new Date(a.submitted_at)).map((pkg,i) => {
                    ...
                    const statusColor = {PENDING:"#ffaa30",ASSIGNED:"#4a9eff",IN_PROGRESS:"#3aaa66",REJECTED:"#cc4a4a"}[pkg.status]||"#4a6a8a";
                    return (
                      <div key={i} style={{ background:"#08182e", border:"1px solid #1a3a6e", borderRadius:6,
                        padding:"10px 14px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
                        <div style={{ flex:1, minWidth:200 }}>
                          <div style={{ color:"#c8d8f0", fontSize:12, fontWeight:"bold" }}>{pkg.req_title}</div>
                          <div style={{ color:"#4a7aaa", fontSize:10, marginTop:2 }}>
                            {pkg.cor_name} · {pkg.center} · {new Date(pkg.submitted_at).toLocaleDateString()}
                          </div>
                        </div>
                        <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                          <span style={{ color:"#c8d8f0", fontSize:11, fontWeight:"bold" }}>${(parseFloat(pkg.value)||0).toLocaleString()}</span>
                          {pkg.assigned_to_name && <span style={{ color:"#3aaa66", fontSize:10 }}>→ {pkg.assigned_to_name}</span>}
                          ...
                          <span style={{ color: days>=5?"#cc3333":"#4a6a8a", fontSize:10 }}>{days}d</span>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div>
                  {highValue.length > 0 && (
                    <div style={{ marginBottom:20 }}>
                      <div style={{ color:"#f4c542", fontSize:10, letterSpacing:2, marginBottom:10 }}>
                      ...
                      {highValue.map((pkg,i) => (
                        <div key={i} style={{ background:"#1a1000", border:"1px solid #f4c54244", borderRadius:6,
                          ...
                          <div>
                            <div style={{ color:"#c8d8f0", fontSize:12, fontWeight:"bold" }}>{pkg.req_title}</div>
                            <div style={{ color:"#4a7aaa", fontSize:10 }}>{pkg.center} · {pkg.cor_name}</div>
                          </div>
                          <div style={{ textAlign:"right" }}>
                            <div style={{ color:"#f4c542", fontWeight:"bold" }}>${(parseFloat(pkg.value)||0).toLocaleString()}</div>
                            <div style={{ color:"#4a7aaa", fontSize:10 }}>{pkg.status}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {stalled.length > 0 && (
                    <div style={{ marginBottom:20 }}>
                      <div style={{ color:"#cc3333", fontSize:10, letterSpacing:2, marginBottom:10 }}>
                        🔴 STALLED — PENDING 5+ DAYS WITHOUT ASSIGNMENT
                      </div>
                      {stalled.map((pkg,i) => {
                        const days = Math.floor((Date.now()-new Date(pkg.submitted_at).getTime())/86400000);
                        return (
                          <div key={i} style={{ background:"#1a0404", border:"1px solid #cc333344", borderRadius:6,
                            padding:"10px 14px", marginBottom:6, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                            <div>
                              <div style={{ color:"#c8d8f0", fontSize:12, fontWeight:"bold" }}>{pkg.req_title}</div>
                              <div style={{ color:"#4a7aaa", fontSize:10 }}>{pkg.center} · {pkg.cor_name}</div>
                            ...
                            <div style={{ textAlign:"right" }}>
                              <div style={{ color:"#cc3333", fontWeight:"bold" }}>{days} days waiting</div>
                              <div style={{ color:"#4a7aaa", fontSize:10 }}>${(parseFloat(pkg.value)||0).toLocaleString()}</div>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  {highValue.length === 0 && stalled.length === 0 && (
                    <div style={{ color:"#3aaa66", textAlign:"center", padding:40, border:"1px dashed #1a3a6e", borderRadius:4 }}>
              ...
              {activeTab === "centers" && (
                <div>
                  <div style={{ color:"#4a9eff", fontSize:10, letterSpacing:2, marginBottom:12 }}>ACQUISITION ACTIVITY BY CENTER</div>
                    ...
                    return (
                      <div key={i} style={{ background:"#08182e", border:"1px solid #1a3a6e", borderRadius:6,
                        ...
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                          <div style={{ color:"#c8d8f0", fontWeight:"bold", fontSize:13 }}>{center}</div>
                          <div style={{ color:"#f4c542", fontWeight:"bold" }}>${(centerValue/1000000).toFixed(1)}M total</div>
                        </div>
                        <div style={{ display:"flex", gap:16, fontSize:11 }}>
                          <span><span style={{ color:"#4a7aaa" }}>Total: </span><span style={{ color:"#c8d8f0" }}>{count}</span></span>
                          <span><span style={{ color:"#4a7aaa" }}>Pending: </span><span style={{ color:"#ffaa30" }}>{centerPending}</span></span>
                          <span><span style={{ color:"#4a7aaa" }}>In Progress: </span><span style={{ color:"#3aaa66" }}>{centerInProgress}</span></span>
                        </div>
                        {/* Mini bar */}
                        <div style={{ marginTop:10, background:"#0d1f38", borderRadius:3, height:6, overflow:"hidden" }}>
                          <div style={{ width:`${(count/total)*100}%`, background:"#4a9eff", height:"100%", borderRadius:3 }} />
                        </div>
                      </div>
                    );
                  })}
...
function BranchChiefDashboard({ onClose }) {
  ...
  const statusColor = { PENDING: "#ffaa30", ASSIGNED: "#3aaa66", REJECTED: "#cc4a4a", IN_PROGRESS: "#4a9eff" };
  const urgencyColor = { URGENT: "#cc3333", MODERATE: "#cc8800", NORMAL: "#3aaa66" };

  const inp = { background: "#08182e", border: "1px solid #1a3a6e", color: "#c8d8f0", padding: "8px 10px",
    borderRadius: 4, fontSize: 12, width: "100%", boxSizing: "border-box", fontFamily: "'IBM Plex Mono',monospace" };
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.85)", zIndex:997, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#0a1a3a", border:"1px solid #2a4a7a", borderRadius:8, width:700, maxWidth:"96vw", maxHeight:"92vh", display:"flex", flexDirection:"column" }}>

        {/* Header */}
        <div style={{ padding:"16px 20px", borderBottom:"1px solid #1a2a4a", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
          <div>
            <div style={{ color:"#f4c542", fontWeight:"bold", fontSize:14 }}>BRANCH CHIEF QUEUE</div>
            <div style={{ color:"#4a6a8a", fontSize:10, marginTop:2 }}>Review and assign acquisition packages to Contracting Officers</div>
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
        ...
        <div style={{ flex:1, overflow:"auto", padding:20 }}>
          ...
          {!email ? (
            <div>
              <div style={{ color:"#8ab0d0", fontSize:12, marginBottom:12 }}>Enter your email to see the acquisition package queue.</div>
              ...
              <button onClick={login} style={{ width:"100%", background:"#1a4a8a", border:"1px solid #2a6aaa", color:"#4a9eff",
                padding:"9px", borderRadius:4, cursor:"pointer", fontSize:12, fontWeight:"bold" }}>
                VIEW QUEUE
              </button>
            </div>
          ) : loading ? (
            <div style={{ color:"#4a6a8a", textAlign:"center", padding:40 }}>Loading packages...</div>
          ) : (
            <div>
              {successMsg && (
                <div style={{ background:"#041a0e", border:"1px solid #3aaa66", borderRadius:4, padding:"10px 14px", marginBottom:14, color:"#3aaa66", fontSize:11 }}>
                  ✓ {successMsg}
                  <button onClick={()=>setSuccessMsg("")} style={{ float:"right", background:"none", border:"none", color:"#3aaa66", cursor:"pointer", fontSize:14 }}>×</button>
                </div>
              )}
              ...
              <div style={{ display:"flex", gap:8, marginBottom:16 }}>
                {[["queue","PENDING QUEUE"],["active","ACTIVE ACQUISITIONS"]].map(([id,label])=>(
                  <button key={id} onClick={()=>setBcTab(id)} style={{
                    background: bcTab===id?"#1a3a6e":"none",
                    border:`1px solid ${bcTab===id?"#4a9eff":"#1a3a6e"}`,
                    color: bcTab===id?"#4a9eff":"#4a6a8a",
              ...
              <div style={{ display:"flex", gap:12, marginBottom:16, flexWrap:"wrap" }}>
                {[
                  { label:"PENDING", count: packages.filter(p=>p.status==="PENDING").length, color:"#ffaa30" },
                  { label:"ASSIGNED", count: packages.filter(p=>p.status==="ASSIGNED").length, color:"#3aaa66" },
                  { label:"REJECTED", count: packages.filter(p=>p.status==="REJECTED").length, color:"#cc4a4a" },
                ].map(s => (
                  <div key={s.label} style={{ background:"#08182e", border:`1px solid ${s.color}44`, borderRadius:4, padding:"8px 14px" }}>
                    <div style={{ fontSize:9, color:s.color, letterSpacing:2 }}>{s.label}</div>
              ...
              {packages.length === 0 ? (
                ...
                <div key={pkg.id} style={{ background: selected===pkg.id?"#0d2040":"#08182e", border:`1px solid ${selected===pkg.id?"#4a9eff":"#1a3a6e"}`,
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:6 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ color:"#c8d8f0", fontWeight:"bold", fontSize:13 }}>{pkg.reqTitle}</div>
                      <div style={{ color:"#4a7aaa", fontSize:10, marginTop:3 }}>
                        ${(pkg.value||0).toLocaleString()} · {pkg.center} · Submitted by {pkg.corName}
                      </div>
                    </div>
                    <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
                      <span style={{ background: urgencyColor[pkg.urgency]+"22", color: urgencyColor[pkg.urgency]||"#4a7aaa",
                        border:`1px solid ${urgencyColor[pkg.urgency]||"#1a3a6e"}`, fontSize:9, padding:"2px 7px", borderRadius:10 }}>
                        {pkg.urgency}
                      </span>
                      <span style={{ background: statusColor[pkg.status]+"22", color: statusColor[pkg.status]||"#4a7aaa",
                        border:`1px solid ${statusColor[pkg.status]||"#1a3a6e"}`, fontSize:9, padding:"2px 7px", borderRadius:10 }}>
                        {pkg.status}
                      </span>
                      <span style={{ background: pkg.daysWaiting >= 5 ? "#cc333322" : "#08182e",
                        color: pkg.daysWaiting >= 5 ? "#cc3333" : "#4a7aaa",
                        border:`1px solid ${pkg.daysWaiting >= 5 ? "#cc3333" : "#1a3a6e"}`,
                  ...
                  {selected === pkg.id && (
                    <div style={{ marginTop:12, borderTop:"1px solid #1a3a6e", paddingTop:12 }}>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12, fontSize:11 }}>
                        <div><span style={{ color:"#4a7aaa" }}>Submitted: </span><span style={{ color:"#c8d8f0" }}>{new Date(pkg.submitted_at||pkg.submittedAt).toLocaleDateString()}</span></div>
                        <div><span style={{ color:"#4a7aaa" }}>COR Email: </span><span style={{ color:"#c8d8f0" }}>{pkg.cor_email||pkg.corEmail}</span></div>
                        {(pkg.package_data?.sowObjective||pkg.packageData?.sowObjective) && (
                          <div style={{ gridColumn:"1/-1" }}>
                            <span style={{ color:"#4a7aaa" }}>Objective: </span>
                            <span style={{ color:"#c8d8f0" }}>{(pkg.package_data?.sowObjective||pkg.packageData?.sowObjective||"").slice(0,120)}...</span>
                          </div>
                        )}
                        {(pkg.assigned_to_name||pkg.assignedToName) && (
                          <div><span style={{ color:"#4a7aaa" }}>Assigned to: </span><span style={{ color:"#3aaa66", fontWeight:"bold" }}>{pkg.assigned_to_name||pkg.assignedToName}</span></div>
                        )}
                        {(pkg.rejection_reason||pkg.rejectionReason) && (
                          <div style={{ gridColumn:"1/-1" }}><span style={{ color:"#cc4a4a" }}>Rejection reason: </span><span style={{ color:"#c8d8f0" }}>{pkg.rejection_reason||pkg.rejectionReason}</span></div>
                        )}
                      </div>

                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        <button onClick={e=>{e.stopPropagation();setViewPkg(pkg);}}
                          style={{ background:"#0d1f3c", border:"1px solid #4a7aaa", color:"#8ab0d0",
                        ...
                        {pkg.status === "PENDING" && (<>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"approve",pkg});}}
                            style={{ background:"#041a0e", border:"1px solid #3aaa66", color:"#3aaa66",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ✓ APPROVE & ASSIGN
                          </button>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reject",pkg});}}
                            style={{ background:"#1a0404", border:"1px solid #cc4a4a", color:"#cc4a4a",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ✗ RETURN TO COR
                          </button>
                        </>)}
                        {pkg.status === "ASSIGNED" && (<>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reassign",pkg});}}
                            style={{ background:"#0d1f3c", border:"1px solid #4a9eff", color:"#4a9eff",
                              padding:"8px 16px", borderRadius:4, cursor:"pointer", fontSize:11, fontWeight:"bold" }}>
                            ↺ REASSIGN
                          </button>
                          <button onClick={e=>{e.stopPropagation();setAction({type:"reject",pkg});}}
              ...
              {bcTab === "active" && (
                <div>
                  {activePackages.length === 0 ? (
                    ...
                    return (
                      <div key={i} style={{ background:"#08182e", border:"1px solid #1a3a6e", borderRadius:6, padding:"12px 16px", marginBottom:8 }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
                          <div style={{ flex:1 }}>
                            <div style={{ color:"#c8d8f0", fontWeight:"bold", fontSize:13 }}>{pkg.req_title}</div>
                            <div style={{ color:"#4a7aaa", fontSize:10, marginTop:3 }}>{pkg.center} · Submitted by {pkg.cor_name}</div>
                          </div>
                          <div style={{ textAlign:"right" }}>
                            <div style={{ color:"#3aaa66", fontWeight:"bold", fontSize:12 }}>IN PROGRESS</div>
                            <div style={{ color:"#4a7aaa", fontSize:10 }}>${(parseFloat(pkg.value)||0).toLocaleString()}</div>
                          </div>
                        </div>
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginTop:10, fontSize:11 }}>
                          <div style={{ background:"#0d1f38", borderRadius:4, padding:"8px 10px" }}>
                            <div style={{ color:"#4a7aaa", fontSize:9, marginBottom:2 }}>ASSIGNED CO</div>
                            <div style={{ color:"#c8d8f0", fontWeight:"bold" }}>{pkg.assigned_to_name||"—"}</div>
                            <div style={{ color:"#4a6a8a", fontSize:10 }}>{pkg.assigned_to_email||""}</div>
                          </div>
                          <div style={{ background:"#0d1f38", borderRadius:4, padding:"8px 10px" }}>
                            <div style={{ color:"#4a7aaa", fontSize:9, marginBottom:2 }}>DAYS SINCE ASSIGNMENT</div>
                            <div style={{ color: daysSinceAssign>=14?"#cc3333":daysSinceAssign>=7?"#cc8800":"#c8d8f0",
                              fontWeight:"bold", fontSize:18 }}>{daysSinceAssign??0}</div>
                          </div>
                          <div style={{ background:"#0d1f38", borderRadius:4, padding:"8px 10px" }}>
                            <div style={{ color:"#4a7aaa", fontSize:9, marginBottom:2 }}>DAYS SINCE STARTED</div>
                            <div style={{ color:"#c8d8f0", fontWeight:"bold", fontSize:18 }}>{daysSinceStart??0}</div>
                          </div>
                        </div>
                      </div>
                    );
      ...
      {viewPkg && (() => {
        ...
        const field = (label, val) => val ? (
          <div style={{ marginBottom:10 }}>
            <div style={{ color:"#4a9eff", fontSize:10, letterSpacing:2, textTransform:"uppercase", marginBottom:3 }}>{label}</div>
            <div style={{ color:"#c8d8f0", fontSize:12, lineHeight:1.5, whiteSpace:"pre-wrap" }}>{val}</div>
          </div>
        ) : null;
        const sect = (title) => (
          <div style={{ color:"#f4c542", fontWeight:"bold", fontSize:11, letterSpacing:2, textTransform:"uppercase",
            borderBottom:"1px solid #1a3a6e", paddingBottom:6, marginBottom:12, marginTop:16 }}>{title}</div>
        );
        return (
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:10 }}>
            <div style={{ background:"#0a1a3a", border:"1px solid #2a4a7a", borderRadius:8, width:680, maxWidth:"96vw",
              maxHeight:"90vh", display:"flex", flexDirection:"column" }}>
              <div style={{ padding:"16px 20px", borderBottom:"1px solid #1a2a4a", display:"flex", justifyContent:"space-between", alignItems:"center", flexShrink:0 }}>
                <div>
                  <div style={{ color:"#c8d8f0", fontWeight:"bold", fontSize:14 }}>{viewPkg.req_title || p.reqtitle}</div>
                ...
                <button onClick={()=>setViewPkg(null)} style={{ background:"none", border:"none", color:"#4a6a8a", fontSize:24, cursor:"pointer" }}>×</button>
              </div>
              ...
              <div style={{ flex:1, overflow:"auto", padding:20 }}>
                ...
                {p.tasks && p.tasks.length > 0 && (
                  <div style={{ marginBottom:10 }}>
                    <div style={{ color:"#4a9eff", fontSize:10, letterSpacing:2, textTransform:"uppercase", marginBottom:6 }}>WORK TASKS</div>
                    {p.tasks.map((t,i) => (
                      <div key={i} style={{ background:"#08182e", border:"1px solid #1a3a6e", borderRadius:4, padding:"8px 12px", marginBottom:6 }}>
                        <div style={{ color:"#c8d8f0", fontWeight:"bold", fontSize:12 }}>{t.title}</div>
                        <div style={{ color:"#8ab0d0", fontSize:11, marginTop:2 }}>{t.desc}</div>
                        {t.deliverable && <div style={{ color:"#4a7aaa", fontSize:11, marginTop:2 }}>Deliverable: {t.deliverable}</div>}
                        {t.frequency && <div style={{ color:"#4a7aaa", fontSize:11 }}>Frequency: {t.frequency}</div>}
                      </div>
                    ))}
                  </div>
                )}
                ...
                {p.igceMode === "CATALOG" && p.catalogClins && p.catalogClins.length > 0 && (
                  <div style={{ marginBottom:10 }}>
                    <div style={{ color:"#4a9eff", fontSize:10, letterSpacing:2, textTransform:"uppercase", marginBottom:6 }}>CATALOG LINE ITEMS</div>
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                      <thead>
                        <tr style={{ background:"#08182e" }}>
                          {["DESCRIPTION","UNIT","RATE","QTY/YR","TOTAL"].map(h => (
                            <td key={h} style={{ padding:"5px 8px", color:"#4a9eff", borderBottom:"1px solid #1a3a6e" }}>{h}</td>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {p.catalogClins.filter(c=>c.qty).map((c,i) => (
                          <tr key={i} style={{ background: i%2===0?"#08182e":"#0a1f38" }}>
                            <td style={{ padding:"5px 8px", color:"#c8d8f0", borderBottom:"1px solid #0d1f38" }}>{c.title}</td>
                            <td style={{ padding:"5px 8px", color:"#8ab0d0", borderBottom:"1px solid #0d1f38" }}>{c.unit}</td>
                            <td style={{ padding:"5px 8px", color:"#8ab0d0", borderBottom:"1px solid #0d1f38" }}>${parseFloat(c.rate||0).toFixed(2)}</td>
                            <td style={{ padding:"5px 8px", color:"#8ab0d0", borderBottom:"1px solid #0d1f38" }}>{c.qty}</td>
                            <td style={{ padding:"5px 8px", color:"#c8d8f0", borderBottom:"1px solid #0d1f38" }}>${(parseFloat(c.rate||0)*parseFloat(c.qty||0)).toLocaleString(undefined,{maximumFractionDigits:0})}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {(action?.type === "approve" || action?.type === "reassign") && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.7)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#0a1a3a", border:`1px solid ${action.type === "reassign" ? "#2a5a8a" : "#2a6a3a"}`, borderRadius:8, padding:28, width:400, maxWidth:"95vw" }}>
            <div style={{ color: action.type === "reassign" ? "#4a9eff" : "#3aaa66", fontWeight:"bold", fontSize:14, marginBottom:4 }}>
              {action.type === "reassign" ? "↺ REASSIGN TO DIFFERENT CO" : "APPROVE & ASSIGN"}
            </div>
            {action.type === "reassign" && (
              <div style={{ background:"#0d1f3c", border:"1px solid #1a3a6e", borderRadius:4, padding:"8px 12px", marginBottom:12, fontSize:11, color:"#8ab0d0" }}>
                Currently assigned to: <strong style={{ color:"#c8d8f0" }}>{action.pkg.assignedToName || action.pkg.assignedToEmail || action.pkg.assigned_to_name || action.pkg.assigned_to_email}</strong>
              </div>
            )}
            <div style={{ color:"#4a6a8a", fontSize:11, marginBottom:16 }}>{action.pkg.reqTitle || action.pkg.req_title}</div>
            <div style={{ color:"#8ab0d0", fontSize:11, marginBottom:4 }}>CO / CS NAME</div>
            <input placeholder="Joshua Taggart" value={assignName} onChange={e=>setAssignName(e.target.value)}
              style={{ ...inp, marginBottom:10 }} />
            <div style={{ color:"#8ab0d0", fontSize:11, marginBottom:4 }}>CO / CS EMAIL *</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={doApprove} disabled={submitting || !assignEmail.includes("@")}
                style={{ flex:1, background: action.type === "reassign" ? "#0d1f3c" : "#041a0e",
                  border:`1px solid ${action.type === "reassign" ? "#4a9eff" : "#3aaa66"}`,
                  color: action.type === "reassign" ? "#4a9eff" : "#3aaa66",
                  padding:"9px", borderRadius:4, cursor:"pointer", fontSize:12, fontWeight:"bold",
              ...
              <button onClick={()=>setAction(null)} style={{ flex:1, background:"#0d1f3c", border:"1px solid #1a2a4a",
      ...
      {action?.type === "reject" && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.7)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#0a1a3a", border:"1px solid #7a2a2a", borderRadius:8, padding:28, width:400, maxWidth:"95vw" }}>
            <div style={{ color:"#cc4a4a", fontWeight:"bold", fontSize:14, marginBottom:4 }}>RETURN TO COR</div>
            <div style={{ color:"#4a6a8a", fontSize:11, marginBottom:16 }}>{action.pkg.reqTitle}</div>
            <div style={{ color:"#8ab0d0", fontSize:11, marginBottom:4 }}>REASON FOR RETURN *</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={doReject} disabled={submitting || !rejectReason.trim()}
                style={{ flex:1, background:"#1a0404", border:"1px solid #cc4a4a", color:"#cc4a4a",
                  padding:"9px", borderRadius:4, cursor:"pointer", fontSize:12, fontWeight:"bold",
    ...
    <div style={S.wWrap}>
      ...
      <div style={S.wCard}>
        ...
        <div style={{ display:"flex", gap:8, marginTop:8 }}>
          {step > 0 && (
            <button onClick={back}
              style={{ flex:1, background:"#08182e", border:"1px solid #1a3a6e", color:"#4a7aaa",
...
function RouteAdvisor({ onUseRecommendation }) {
  ...
  return (
    <div style={{ padding:"24px", maxWidth:"820px", margin:"0 auto" }}>
      <div style={{ fontSize:"10px", color:"#4a7aaa", letterSpacing:"2px",marginBottom:"4px"}}>ACQUISITION APPROACH COACH</div>
      <div style={{ fontSize:"20px", color:"#c8d8f0", fontWeight:700,marginBottom:"4px"}}>Acquisition Approach Coach <span style={{fontSize:"13px",color:"#4a7aaa",fontWeight:400}}>(your acquisition life coach)</span></div>
      <div style={{ fontSize:"11px", color:"#4a7aaa",marginBottom:"20px"}}>Options, analysis, and alternatives — you decide. Think of this as your acquisition life coach. (The CO is still the CO.)</div>
...
function ToolsMenu({ onMod, onForms, onBPA, onUCF, onAward, onAdmin, onCompliance, onPreSol, onReview, onRequestor, onTechEval, onBranchChief, onHQDashboard }) {
  ...
  const items = [
    { label:"REQUESTOR / COR PORTAL", color:"#f0a040", action: onRequestor },
    { label:"BRANCH CHIEF QUEUE", color:"#f4c542", action: onBranchChief },
    { label:"HQ LEADERSHIP DASHBOARD", color:"#f4c542", action: onHQDashboard },
    { label:"TECH EVAL HELPER", color:"#aa7aff", action: onTechEval },
    { label:"IGCE / MARKET RESEARCH",  color:"#6aff6a", action: onPreSol  },
    { label:"SF FORMS",                color:"#4ac8ff", action: onForms   },
    { label:"UCF D / E / F",           color:"#4aff8a", action: onUCF     },
    { label:"CLAUSE MATRIX",           color:"#4a9eff", action: null, note:"(in roadmap step)" },
    { label:"MOD BUILDER — SF-30",     color:"#c07aff", action: onMod     },
    { label:"BPA CALL ORDER",          color:"#e07ae0", action: onBPA     },
    { label:"AWARD TOOLS (PNM / Opts/FPDS)", color:"#f4c542", action: onAward   },
    { label:"CONTRACT ADMIN (CPARS/Debrief)", color:"#ff6a6a", action: onAdmin   },
    { label:"COMPLIANCE (Resp/Closeout/D&F)", color:"#4affff", action: onCompliance },
    { label:"REVIEW QUEUE",            color:"#4aba6a", action: onReview  },
    { label:"— NSSC SIMPLIFIED (COMING SOON)", color:"#3a5a3a", action: null, note:"roadmap" },
  ];
  ...
  return (
    <div ref={ref} style={{ position:"relative" }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ background: open ? "#0a2a4a" : "#061020",
                 border:"1px solid #2a4a7a", color:"#4a9eff",
      ...
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 4px)", right:0,
                      background:"#061020", border:"1px solid #1a3a6e",
                      borderRadius:5, zIndex:999, minWidth:260,
                      boxShadow:"0 4px 20px rgba(0,0,0,.6)", overflow:"hidden" }}>
          {items.map((item, i) => (
            <div key={i}
              onClick={() => { if (item.action) { item.action(); setOpen(false); } }}
              style={{ padding:"9px 14px", cursor: item.action ? "pointer" : "default",
                       borderBottom:"1px solid #0d1f3c",
                       background:"transparent",
                       display:"flex", alignItems:"center", justifyContent:"space-between",
                       opacity: item.action ? 1 : 0.5 }}
              onMouseEnter={e => { if(item.action) e.currentTarget.style.background="#0a1a3a"; }}
              onMouseLeave={e => { e.currentTarget.style.background="transparent"; }}>
              <span style={{ fontSize:11, color: item.color, fontWeight:"bold" }}>{item.label}</span>
              {item.note && <span style={{ fontSize:9, color:"#3a5a7a" }}>{item.note}</span>}
            </div>
          ))}
        </div>
      )}
...
function CPAS() {
  ...
  return (
    <>
    {showApiSetup && (
      <div style={{position:"fixed",inset:0,background:"rgba(4,13,26,.97)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'IBM Plex Mono',monospace"}}>
        <div style={{background:"#061020",border:"1px solid #2a4a7a",borderRadius:10,padding:40,width:480,maxWidth:"95vw",textAlign:"center"}}>
          <div style={{fontSize:11,color:"#4a7aaa",letterSpacing:3,marginBottom:6}}>NATIONAL AERONAUTICS AND SPACE ADMINISTRATION</div>
          <div style={{fontSize:28,color:"#4a9eff",fontWeight:"bold",marginBottom:4}}>CPAS</div>
          <div style={{fontSize:11,color:"#4a6a8a",marginBottom:28}}>Contracting Process Automation System — v5</div>
          <div style={{fontSize:11,color:"#8ab0d0",marginBottom:8,textAlign:"left"}}>ANTHROPIC API KEY</div>
          <div style={{fontSize:10,color:"#4a6a8a",marginBottom:12,textAlign:"left"}}>Required for AI document generation. Get your key at console.anthropic.com. Stored locally in your browser only — never sent to any server other than Anthropic.</div>
          <input
            type="password"
            placeholder="sk-ant-..."
            value={apiKeyInput}
            ...
            style={{width:"100%",background:"#08182e",border:"1px solid #2a6aaa",color:"#c8d8f0",padding:"10px 12px",borderRadius:4,fontSize:12,boxSizing:"border-box",marginBottom:12,fontFamily:"'IBM Plex Mono',monospace"}}
          />
          <button
            ...
            style={{width:"100%",background:apiKeyInput.startsWith("sk-ant")?"#1a4a8a":"#0a1a3a",border:"1px solid #2a6aaa",color:apiKeyInput.startsWith("sk-ant")?"#4a9eff":"#2a4a6a",padding:"11px",borderRadius:4,cursor:apiKeyInput.startsWith("sk-ant")?"pointer":"default",fontSize:12,fontWeight:"bold",marginBottom:8}}>
            ENTER CPAS
          </button>
          <div style={{background:"#0a1a0a",border:"1px solid #1a4a2a",borderRadius:6,padding:"8px 12px",marginBottom:10,fontSize:9,color:"#3a8a4a",textAlign:"left"}}>
            <span style={{fontWeight:"bold",color:"#4aba6a"}}>🚀 Coming Soon — NSSC Expansion:</span> CPAS roadmap includes a dedicated simplified acquisition lane for NSSC buyers (micro-purchases, credit card, simplified orders) with appropriate guardrails. One platform for all of NASA procurement.
          </div>
          <button onClick={()=>setShowApiSetup(false)} style={{background:"none",border:"none",color:"#2a4a6a",fontSize:10,cursor:"pointer"}}>
            Skip — use existing key or enter later via browser console
          </button>
        </div>
      </div>
    )}
    <div style={S.app}>
      ...
      <div style={S.hdr}>
        ...
        <button style={{...S.startOver}} onClick={()=>setScreen(screen==="ROUTE"?"INTAKE":"ROUTE")}>
          {screen==="ROUTE" ? "< BACK" : "ACQ COACH"}</button>
        ...
        <button title="API Key Settings"
          onClick={()=>setShowApiSetup(true)}
          style={{background:"none",border:"1px solid #1a2a4a",color:"#2a4a6a",padding:"4px 8px",
                  borderRadius:3,cursor:"pointer",fontSize:12,marginRight:4}}>
          ⚙
        </button>
        <ToolsMenu
          ...
          {(() => {
            try {
              ...
              if (pending.length > 0) return (
                <div style={{ background:"#0a2a1a", border:"2px solid #3aaa66", borderRadius:6,
                              padding:"12px 18px", marginBottom:16 }}>
                  <div style={{ fontSize:11, color:"#3aaa66", fontWeight:700, letterSpacing:2, marginBottom:8 }}>
                  ...
                  {pending.map(p => (
                    <div key={p.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:10, color:"#8a9ab0", marginTop:2 }}>
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        ...
                        }} style={{ background:"#3aaa66", border:"none", color:"#fff",
        ...
        <div style={{ ...S.main, paddingRight: activeStep ? "604px" : "24px" }}>
          {(() => {
            try {
              const pending = pendingPackages;
              if (pending.length > 0) return (
                <div style={{ background:"#0a2a1a", border:"1px solid #3aaa66", borderRadius:4,
                              padding:"10px 16px", marginBottom:12 }}>
                  <div style={{ fontSize:10, color:"#3aaa66", fontWeight:700, letterSpacing:2, marginBottom:6 }}>
                    📥 {pending.length} PENDING PACKAGE{pending.length > 1 ? "S" : ""} FROM REQUESTOR PORTAL
                  </div>
                  {pending.map(p => (
                    <div key={p.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
                                             fontSize:11, color:"#c8d8f0", marginBottom:4 }}>
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        ...
                        }} style={{ background:"#3aaa66", border:"none", color:"#fff", padding:"3px 10px",
    ...
    {showRequestorPortal && (
      <div style={{position:"fixed",inset:0,background:"#040d1a",zIndex:990,overflow:"auto"}}>
        <RequestorPortal
    ...
    {showMod && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:997,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #2a4a7a",borderRadius:8,width:700,maxWidth:"96vw",maxHeight:"92vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{color:"#c07aff",fontWeight:"bold",fontSize:13}}>MOD BUILDER — SF-30</div>
            <button onClick={()=>setShowMod(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
    ...
    {showForms && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:996,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #1a5a8a",borderRadius:8,width:860,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#4ac8ff",fontWeight:"bold",fontSize:13}}>SF FORMS BUILDER</div>
              <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>SF-1449 · SF-26 · SF-33 · SF-18 · SF-1442 · SF-1447 · OF-347 · OF-336</div>
            </div>
            <button onClick={()=>setShowForms(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showBPA && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:995,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #3a1a3a",borderRadius:8,width:680,maxWidth:"96vw",maxHeight:"92vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#e07ae0",fontWeight:"bold",fontSize:13}}>BPA CALL ORDER BUILDER</div>
              <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>FAR 13.303-5 · Blanket Purchase Agreement Call</div>
            </div>
            <button onClick={()=>setShowBPA(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showTechEval && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:989,display:"flex",flexDirection:"column"}}>
        <div style={{background:"#0a1a3a",borderBottom:"1px solid #1a2a4a",padding:"10px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div>
            <div style={{color:"#aa7aff",fontWeight:"bold",fontSize:13}}>TECHNICAL EVALUATION HELPER</div>
            <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>Source Selection Sensitive · For Technical Evaluators and CORs</div>
          </div>
          <button onClick={()=>setShowTechEval(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
        </div>
    ...
    {showPreSol && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:991,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #2a6a2a",borderRadius:8,width:1100,maxWidth:"98vw",maxHeight:"95vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#6aff6a",fontWeight:"bold",fontSize:13}}>PRE-SOLICITATION TOOLS</div>
              <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>IGCE Worksheet  ·  Market Research Report (FAR Part 10)</div>
            </div>
            <button onClick={()=>setShowPreSol(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showCompliance && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:992,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #1a5a5a",borderRadius:8,width:920,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#4affff",fontWeight:"bold",fontSize:13}}>COMPLIANCE TOOLS</div>
              <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>Responsibility Determination  ·  Closeout Checklist  ·  D&F Templates</div>
            </div>
            <button onClick={()=>setShowCompliance(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showAdminTools && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:992,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #3a1a1a",borderRadius:8,width:960,maxWidth:"98vw",maxHeight:"96vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#ff6a6a",fontWeight:"bold",fontSize:13}}>CONTRACT ADMINISTRATION TOOLS</div>
              <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>Closeout · CPARS · Debrief · Pre-Negotiation Objectives</div>
            </div>
            <button onClick={()=>setShowAdminTools(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showAwardTools && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:993,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #3a3a1a",borderRadius:8,width:920,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#f4c542",fontWeight:"bold",fontSize:13}}>AWARD TOOLS</div>
              <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>Price Analysis / PNM  ·  Option Exercise  ·  FPDS-NG Checklist</div>
            </div>
            <button onClick={()=>setShowAwardTools(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
    ...
    {showUCF && (
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:994,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{background:"#0a1a3a",border:"1px solid #1a5a2a",borderRadius:8,width:920,maxWidth:"97vw",maxHeight:"94vh",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"14px 20px",borderBottom:"1px solid #1a2a4a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{color:"#4aff8a",fontWeight:"bold",fontSize:13}}>UCF SECTIONS D / E / F</div>
              <div style={{color:"#4a6a8a",fontSize:10,marginTop:2}}>Packaging · Inspection · Deliveries</div>
            </div>
            <button onClick={()=>setShowUCF(false)} style={{background:"none",border:"none",color:"#4a6a8a",fontSize:18,cursor:"pointer"}}>×</button>
          </div>
          <div style={{flex:1,overflow:"auto"}}>
