...
const DEFAULT_CATALOG_CLINS = [
  { id: 1, title: "Pilot — Pilot-in-Command",          unit: "Pilot Day",       rate: "", qty: "", basis: "Market/historical rates" },
  { id: 2, title: "Pilot — Safety Pilot/Co-Pilot",     unit: "Pilot Day",       rate: "", qty: "", basis: "Market/historical rates" },
  { id: 3, title: "Aircraft Daily Availability Rate",  unit: "Aircraft Day",    rate: "", qty: "", basis: "Based on aircraft type/mission" },
  { id: 4, title: "Aircraft Flight Hour Rate",         unit: "Flight Hour",     rate: "", qty: "", basis: "Historical flight ops data" },
  { id: 5, title: "Mission Support Engineer",          unit: "Engineer Day",    rate: "", qty: "", basis: "GSA/market labor rates" },
  { id: 6, title: "Fuel (EPA — govt-furnished or EIA index)", unit: "N/A", rate: "", qty: "", basis: "EIA Kerosene-Type Jet Fuel Index" },
];
...
export function IGCEWorksheet({ intake, onGenerated }) {
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono', monospace", color: C.text, background: C.bg }}>
      ...
      <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
          <div onClick={() => setMetaField("includeProfit", !meta.includeProfit)}
            style={{ width: 13, height: 13, border: `1px solid ${meta.includeProfit ? C.green : "#2a4a6a"}`, borderRadius: 2,
      ...
      {/* Grand total banner */}
      <div style={{ background: "#041a0e", borderBottom: `1px solid #1a6a3a`, padding: "10px 16px", display: "flex", gap: 24, flexWrap: "wrap" }}>
        <div>
            ...
            {["LABOR","CATALOG"].map(mode => (
              <button key={mode} onClick={() => setMetaField("igceMode", mode)}
                style={{ padding:"4px 12px", borderRadius:3, cursor:"pointer", fontSize:10, fontWeight:"bold",
                         background: meta.igceMode === mode ? (mode==="CATALOG"?"#0a2a4a":"#0a2a1a") : "#08182e",
                         border: `1px solid ${meta.igceMode === mode ? (mode==="CATALOG"?"#4a9eff":"#3aaa66") : "#1a3a6e"}`,
                         color: meta.igceMode === mode ? (mode==="CATALOG"?"#4a9eff":"#3aaa66") : "#4a7aaa" }}>
                {mode === "LABOR" ? "Standard Labor-Hour" : "Catalog / Rate Card (IDIQ)"}
              </button>
            ))}
            {meta.igceMode === "CATALOG" && (
              <span style={{fontSize:9,color:"#4a7aaa",alignSelf:"center"}}>
        ...
        <button onClick={() => onGenerated && onGenerated(text)}
          style={{ background: "#0a2a1a", border: "1px solid #1a6a3a", color: C.green, padding: "6px 16px", borderRadius: 3, cursor: "pointer", fontSize: 11, fontWeight: "bold", alignSelf: "center" }}>
          SAVE TO PACKAGE
        </button>
      ...
      <div style={{ overflow: "auto", maxHeight: "62vh" }}>
          ...
          {periods.map(p => (
            <button key={p.id} onClick={() => setPeriods(pp => pp.map(pp2 => pp2.id === p.id ? { ...pp2, active: !pp2.active } : pp2))}
              style={{ padding: "3px 10px", borderRadius: 3, cursor: "pointer", fontSize: 10,
                       background: p.active ? "#0a2a4a" : C.bg3,
                    ...
                    {activePeriods.map((period, pidx) => {
                      ...
                      return (
                        <div key={period.id} style={{ position: "relative" }}>
                          ...
                          {pidx > 0 && rate > 0 && (
                            <div style={{ fontSize: 8, color: "#3a5a7a", position: "absolute", bottom: -10, left: 4 }}>
        ...
        <div style={{ padding: "12px 16px 0" }}>
          ...
          <div style={{ border: `1px solid ${C.border}`, borderRadius: 4, overflow: "hidden" }}>
                  ...
                  {activePeriods.map((period, pidx) => {
                    ...
                    return (
                      <div key={period.id} style={{ position: "relative" }}>
                        ...
                        {pidx > 0 && rate > 0 && (
                          <div style={{ fontSize: 8, color: "#3a5a7a", position: "absolute", bottom: -12, left: 4 }}>
            ...
            <div style={{ display: "grid",
                          gridTemplateColumns: `180px 80px 90px ${activePeriods.map(() => "90px").join(" ")} 110px`,
                          padding: "6px 8px", gap: 4, background: "#041a0e", borderTop: `1px solid ${C.border}` }}>
          ...
          <div style={{ border: `1px solid ${C.border}`, borderRadius: 4, overflow: "hidden" }}>
            ...
            {[
              ...
              <div key={row.key} style={{ display: "grid", gridTemplateColumns: `160px repeat(${periodTotals.length + 1}, 1fr)`,
                                          ...
                                          background: row.key === "total" ? "#041a0e" : "transparent",
          ...
          {MR_METHODS.map(m => (
            <div key={m.id} onClick={() => toggleMethod(m.id)}
              style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "4px 6px", cursor: "pointer",
                       background: r.methodsUsed.includes(m.id) ? "#041a0e" : "transparent",
                       border: `1px solid ${r.methodsUsed.includes(m.id) ? "#1a4a2a" : "transparent"}`,
                       borderRadius: 3, marginBottom: 3 }}>
              <div style={{ width: 12, height: 12, border: `1px solid ${r.methodsUsed.includes(m.id) ? C.green : "#2a4a6a"}`,
            ...
            {[["YES","Commercial available"], ["PARTIAL","Partially commercial"], ["NO","Non-commercial"]].map(([val, lbl2]) => (
              <button key={val} onClick={() => set("commercialAvail", val)}
                style={{ flex: 1, padding: "5px 4px", borderRadius: 3, cursor: "pointer", fontSize: 9,
                         background: r.commercialAvail === val ? "#0a2a4a" : C.bg3,
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
            {[["YES","Reasonable expectation of 2+ SB offers"], ["NO","Rule of Two not met"], ["UNABLE","Unable to determine"]].map(([val, lbl2]) => (
              <button key={val} onClick={() => set("ruleOfTwo", val)}
                style={{ flex: 1, padding: "5px 4px", borderRadius: 3, cursor: "pointer", fontSize: 9,
                         background: r.ruleOfTwo === val ? "#0a2a4a" : C.bg3,
