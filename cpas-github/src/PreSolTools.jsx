                ...
                return (
                  <div key={cl.id} style={{ display: "grid", gridTemplateColumns: `220px 110px 90px ${activePeriods.map(() => "80px").join(" ")} 80px 28px`, padding: "4px 8px", gap: 4, borderTop: `1px solid ${C.border}`, background: i % 2 === 0 ? "transparent" : C.bg2 }}>
                    ...
                    <button onClick={() => setCatalogClins(cs => cs.filter(c => c.id !== cl.id))}
                      style={{ background: "none", border: "none", color: "#6b7a99", cursor: "pointer", fontSize: 14 }}>×</button>
                  </div>
                );
              })}
              {/* Catalog subtotal */}
              <div style={{ display: "grid", gridTemplateColumns: `220px 110px 90px ${activePeriods.map(() => "80px").join(" ")} 80px 28px`, padding: "6px 8px", gap: 4, background: "#eef3fc", borderTop: `1px solid ${C.border}` }}>
                ...
                <div key={lc.id} style={{ display: "grid",
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    ...
                    <button onClick={() => removeLC(lc.id)}
                      style={{ background: "none", border: "none", color: "#6b7a99", cursor: "pointer", fontSize: 13, marginLeft: "auto" }}>×</button>
                  </div>
                </div>
              );
            })}
          ...
          {MR_METHODS.map(m => (
            <div key={m.id} onClick={() => toggleMethod(m.id)}
              style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "4px 6px", cursor: "pointer",
                       ...
                       border: `1px solid ${r.methodsUsed.includes(m.id) ? "#9fe1cb" : "transparent"}`,
