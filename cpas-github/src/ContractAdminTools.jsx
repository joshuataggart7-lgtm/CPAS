...
export function CloseoutChecklist({ intake, onGenerated }) {
  ...
  return (
    <div style={{ fontFamily:"FONT_COMP", color:C.text, padding:16, overflow:"auto", maxHeight:"72vh" }}>
      <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:10 }}>CLOSEOUT CHECKLIST — FAR 4.804</div>
      ...
      {cats.map(cat => {
        ...
        return (
          <div key={cat} style={{ marginBottom:12 }}>
            ...
            {items.map(item => (
              <div key={item.id} style={{ background: checked[item.id] ? "#e8f7f0" : C.bg3,
                border:`1px solid ${checked[item.id] ? "#9fe1cb" : C.border}`,
                borderRadius:4, padding:"8px 10px", marginBottom:4 }}>
                <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                  <div onClick={() => setChecked(c => ({...c,[item.id]:!c[item.id]}))}
                    style={{ width:16, height:16, border:`1px solid ${checked[item.id]?C.green:"#8896b0"}`,
      ...
      <div style={{ display:"flex", gap:8, marginTop:8 }}>
        ...
        <button onClick={() => { const t = generateDoc(); onGenerated && onGenerated(t); }}
          style={{ background: allDone ? "#e8f7f0" : C.bg3, border:`1px solid ${allDone ? C.green : C.border}`,
...
const CPARS_RATINGS = [
  { value:"EX", label:"Exceptional",   color:"#0f6e56", desc:"Performance meets contractual requirements and exceeds many to the Government's benefit." },
  { value:"VG", label:"Very Good",     color:"#1a3a6e", desc:"Performance meets contractual requirements and exceeds some to the Government's benefit." },
  { value:"S",  label:"Satisfactory",  color:"#4a5a78", desc:"Performance meets contractual requirements." },
  { value:"M",  label:"Marginal",      color:"#854f0b", desc:"Performance does not meet some contractual requirements. CO has taken action." },
  { value:"U",  label:"Unsatisfactory",color:"#c05a00", desc:"Performance does not meet contractual requirements and recovery is not likely." },
  { value:"NA", label:"Not Applicable",color:"#6b7a99", desc:"Not applicable to this contract." },
];
    ...
    <div style={{ fontFamily:"FONT_COMP", color:C.text }}>
      <div style={{ display:"grid", gridTemplateColumns:"380px 1fr", minHeight:520 }}>
        <div style={{ padding:16, borderRight:`1px solid ${C.border}`, overflow:"auto", maxHeight:"72vh" }}>
          <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:10 }}>CPARS STRUCTURED INPUT</div>
                ...
                {CPARS_RATINGS.map(r => (
                  <button key={r.value} onClick={() => setRating(area.id, r.value)}
                    style={{ padding:"3px 8px", borderRadius:3, cursor:"pointer", fontSize:9, fontWeight:"bold",
                             ...
                             border:`1px solid ${c.ratings[area.id]===r.value ? r.color : "#8896b0"}`,
    ...
    <div style={{ fontFamily:"FONT_COMP", color:C.text }}>
      <div style={{ display:"grid", gridTemplateColumns:"340px 1fr", minHeight:480 }}>
        <div style={{ padding:16, borderRight:`1px solid ${C.border}`, overflow:"auto", maxHeight:"70vh" }}>
          <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:10 }}>POST-AWARD DEBRIEF — FAR 15.506</div>
...
export function PreNegotiationMemo({ intake, onGenerated }) {
  ...
  return (
    <div style={{ fontFamily:"FONT_COMP", color:C.text }}>
      <div style={{ display:"grid", gridTemplateColumns:"340px 1fr", minHeight:480 }}>
        <div style={{ padding:16, borderRight:`1px solid ${C.border}`, overflow:"auto", maxHeight:"70vh" }}>
          <div style={{ fontSize:9, color:C.muted, letterSpacing:"0.5px", marginBottom:10 }}>PRE-NEGOTIATION OBJECTIVES — FAR 15.406-1</div>
          ...
          {variance !== null && (
            <div style={{ background:"#ffffff", border:`1px solid ${C.border}`, borderRadius:3, padding:"7px 10px", marginTop:4 }}>
