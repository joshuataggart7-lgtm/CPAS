...
export function ResponsibilityDetermination({ intake, onGenerated }) {
  ...
  const leftContent = (
    <>
            ...
            {["SATISFACTORY","UNSATISFACTORY","N/A"].map(status => (
              <button key={status} onClick={() => setFinding(s.id, "status", status)}
                style={{ flex: 1, padding: "3px 4px", borderRadius: 2, cursor: "pointer", fontSize: 9, fontWeight: "bold",
                         background: r.findings[s.id]?.status === status ? (status === "SATISFACTORY" ? "#e8f7f0" : status === "UNSATISFACTORY" ? "#fff0f0" : "#0a0a1a") : "transparent",
      ...
      {r.nonresponsible && (<>
        <div style={{ background: "#fff0f0", border: "1px solid #4a1a1a", borderRadius: 4, padding: "8px 10px", marginBottom: 8, fontSize: 10, color: "#f07070" }}>
...
export function ContractCloseout({ intake, onGenerated }) {
  ...
  const leftContent = (
    <>
      ...
      {/* Progress */}
      <div style={{ background: "#e8f7f0", border: `1px solid ${allRequiredDone ? "#1a6a3a" : "#1a3a6e"}`, borderRadius: 4, padding: "8px 10px", marginTop: 10, marginBottom: 10 }}>
  ...
  const specificFields = () => {
    switch (selectedDF) {
      ...
      case "URGENCY": return (<>
        ...
        <div style={{ background: "#e8f7f0", border: "1px solid #1a4a2a", borderRadius: 4, padding: "8px 10px", marginBottom: 8, fontSize: 10, color: "#4aba6a" }}>
