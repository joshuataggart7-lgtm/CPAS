...
function TwoPane({ leftContent, text, panelLabel, onSave }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", minHeight: 480, fontFamily: "FONT_COMP", color: C.text }}>
    ...
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <div onClick={() => set("samCheck", !r.samCheck)}
          style={{ width: 13, height: 13, border: `1px solid ${r.samCheck ? C.green : "#8896b0"}`, borderRadius: 2,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        <div onClick={() => set("cparsCheck", !r.cparsCheck)}
          style={{ width: 13, height: 13, border: `1px solid ${r.cparsCheck ? C.blue : "#8896b0"}`, borderRadius: 2,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        <div onClick={() => set("presAwardSurvey", !r.presAwardSurvey)}
          style={{ width: 13, height: 13, border: `1px solid ${r.presAwardSurvey ? C.yellow : "#8896b0"}`, borderRadius: 2,
      ...
      {RESPONSIBILITY_STANDARDS.map(s => (
        <div key={s.id} style={{ background: C.bg3, border: `1px solid ${r.findings[s.id]?.status === "SATISFACTORY" ? "#9fe1cb" : r.findings[s.id]?.status === "UNSATISFACTORY" ? "#fff0f0" : C.border}`, borderRadius: 3, padding: "7px 9px", marginBottom: 5 }}>
            ...
            {["SATISFACTORY","UNSATISFACTORY","N/A"].map(status => (
              <button key={status} onClick={() => setFinding(s.id, "status", status)}
                style={{ flex: 1, padding: "3px 4px", borderRadius: 2, cursor: "pointer", fontSize: 9, fontWeight: "bold",
                         background: r.findings[s.id]?.status === status ? (status === "SATISFACTORY" ? "#e8f7f0" : status === "UNSATISFACTORY" ? "#fff0f0" : "#ffffff") : "transparent",
                         border: `1px solid ${r.findings[s.id]?.status === status ? (status === "SATISFACTORY" ? C.green : status === "UNSATISFACTORY" ? C.red : "#6b7a99") : "#1a3a6e"}`,
                         color: r.findings[s.id]?.status === status ? (status === "SATISFACTORY" ? C.green : status === "UNSATISFACTORY" ? C.red : "#5a3a9e") : C.dim }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
        <button onClick={() => set("nonresponsible", false)}
          style={{ flex: 1, padding: "7px", borderRadius: 3, cursor: "pointer", fontSize: 10, fontWeight: "bold",
                   background: !r.nonresponsible ? "#e8f7f0" : C.bg3,
        ...
        <button onClick={() => set("nonresponsible", true)}
          style={{ flex: 1, padding: "7px", borderRadius: 3, cursor: "pointer", fontSize: 10, fontWeight: "bold",
                   background: r.nonresponsible ? "#fff0f0" : C.bg3,
      ...
      {r.nonresponsible && (<>
        <div style={{ background: "#fff0f0", border: "1px solid #4a1a1a", borderRadius: 4, padding: "8px 10px", marginBottom: 8, fontSize: 10, color: "#e05555" }}>
...
export function ContractCloseout({ intake, onGenerated }) {
  ...
  const leftContent = (
    <>
      ...
      {/* Progress */}
      <div style={{ background: "#e8f7f0", border: `1px solid ${allRequiredDone ? "#e8f7f0" : "#1a3a6e"}`, borderRadius: 4, padding: "8px 10px", marginTop: 10, marginBottom: 10 }}>
        ...
        <div style={{ height: 4, background: "#ffffff", borderRadius: 3 }}>
      ...
      {CLOSEOUT_PHASES.map(phase => {
        ...
        return (
          <div key={phase} style={{ marginBottom: 10 }}>
            ...
            {items.map(item => (
              <div key={item.id} style={{ background: c.checks[item.id]?.done ? "#e8f7f0" : C.bg3,
                                           border: `1px solid ${c.checks[item.id]?.done ? "#9fe1cb" : item.required ? "#6b7a99" : C.border}`,
                                           borderRadius: 3, padding: "6px 8px", marginBottom: 4 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  <div onClick={() => setCheck(item.id, "done", !c.checks[item.id]?.done)}
                    style={{ width: 14, height: 14, border: `1px solid ${c.checks[item.id]?.done ? C.green : item.required ? C.yellow : "#8896b0"}`,
  ...
  const specificFields = () => {
    switch (selectedDF) {
      case "TM": return (<>
        ...
        <div style={{ background: "#fff8e6", border: "1px solid #4a4a1a", borderRadius: 4, padding: "8px 10px", marginBottom: 8, fontSize: 10, color: "#854f0b" }}>
      ...
      case "CONSOLIDATION": return (<>
        {secHead("CONSOLIDATION — FAR 7.107-4", "#a32d2d")}
        ...
        {DF_TYPES.map(df => (
          <button key={df.id} onClick={() => setSelectedDF(df.id)}
            style={{ display: "block", width: "100%", textAlign: "left", marginBottom: 4, padding: "7px 10px", borderRadius: 3, cursor: "pointer", fontSize: 10,
                     background: selectedDF === df.id ? "#eef3fc" : C.bg3,
